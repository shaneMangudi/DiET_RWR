...
final class JsonTokenizer implements Closeable {
	...
	enum JsonToken {
		CURLYOPEN(Event.START_OBJECT, false), SQUAREOPEN(Event.START_ARRAY, false), COLON(null, false), COMMA(null,
				false), STRING(Event.VALUE_STRING, true), NUMBER(Event.VALUE_NUMBER, true), TRUE(Event.VALUE_TRUE,
						true), FALSE(Event.VALUE_FALSE, true), NULL(Event.VALUE_NULL, true), CURLYCLOSE(
								Event.END_OBJECT, false), SQUARECLOSE(Event.END_ARRAY, false), EOF(null, false);
	...
	/*
	 * Could be optimized if the parser uses separate methods to match colon etc
	 * (that would avoid the switch statement cost in certain cases)
	 */
	JsonToken nextToken() {
		reset();
		int ch = read();
