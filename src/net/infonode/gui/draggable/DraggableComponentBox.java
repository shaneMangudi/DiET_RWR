...
public class DraggableComponentBox extends SimplePanel {
	...
	public DraggableComponentBox(int iconSize, boolean useDefaultScrollButtons) {
		...
		final DirectionLayout layout = new DirectionLayout(componentDirection == Direction.UP ? Direction.RIGHT
			...
			public Dimension minimumLayoutSize(Container parent) {
				...
				return componentDirection.isHorizontal() ? new Dimension(pref.width, min.height)
	...
	public void setComponentDirection(Direction componentDirection) {
		if (componentDirection != this.componentDirection) {
			...
			getDirectionLayout().setDirection(componentDirection == Direction.UP ? Direction.RIGHT
					: componentDirection == Direction.LEFT ? Direction.DOWN
							: componentDirection == Direction.DOWN ? Direction.RIGHT : Direction.DOWN);
			if (scrollEnabled) {
	...
	public void dragDraggableComponent(DraggableComponent component, Point p) {
		...
		// component.drag(SwingUtilities.convertPoint(this, p,
	...
	// Prevents focus problems when adding/removing focused component while
	// sorting when spacing < 0
	...
	private void doSort() {
		if (mustSort && getComponentSpacing() < 0) {
			...
			/*
			 * System.out.print("  Box:   "); for (int i = 0; i <
			 * componentBox.getComponentCount(); i++)
			 * System.out.print(componentBox.getComponent(i) + "  ");
			 * System.out.println(); System.out.print("  Order: "); for (int i =
			 * 0; i < layoutOrderList.size(); i++)
			 ...
			 * 
			 * if (tc != null) componentBox.add(tc);
			 * 
			 * int size = layoutOrderList.size(); for (int i = 0; i < size; i++)
			 * { c = (Component) layoutOrderList.get(doReverseSort ? size - i -
			 * 1 : i); if (c != tc) componentBox.add(c); }
			 */

			// componentBoxEnabled = true;

			// System.out.println("Sorting " + scount++ + " time: " +
			// (System.currentTimeMillis() - millis) + " Sorted: " + switc);
		}
	}
	...
	private int getComponentIndexAtPoint(Point p) {
		...
		if (outerParentArea.contains(p3))
			c = (JComponent) ComponentUtil.getChildAtLine(componentBox, p2,
	...
	private void initialize() {
		...
		if (scrollEnabled) {
			...
			final ScrollableBox scrollableBox = new ScrollableBox(componentBox, componentDirection.isHorizontal(),
					scrollOffset);
