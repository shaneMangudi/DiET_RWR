...
public class GradientComponentPainter extends AbstractComponentPainter implements Serializable {
	 ...
	 * @param topRightColor
	 *            the top right corner color
	 * @param bottomLeftColor
	 *            the bottom left corner color
	...
	public GradientComponentPainter(Color topLeftColor, Color topRightColor, Color bottomLeftColor,
		...
		this(new FixedColorProvider(topLeftColor), new FixedColorProvider(topRightColor),
				new FixedColorProvider(bottomLeftColor), new FixedColorProvider(bottomRightColor));
	}
	 ...
	 *
	 * @param topLeftColor
	 *            the top left corner color provider
	 * @param topRightColor
	 *            the top right corner color provider
	 * @param bottomLeftColor
	 *            the bottom left corner color provider
	 * @param bottomRightColor
	 *            the bottom right corner color provider
	 */
	public GradientComponentPainter(ColorProvider topLeftColor, ColorProvider topRightColor,
			ColorProvider bottomLeftColor, ColorProvider bottomRightColor) {
	 ...
	 * BufferedImage image = new BufferedImage(direction.isHorizontal() ? size :
	 * 1, direction.isHorizontal() ? 1 : size, BufferedImage.TYPE_INT_RGB);
	 * 
	 * int dx = direction == Direction.RIGHT ? 1 : direction == Direction.LEFT ?
	 * -1 : 0; int dy = direction == Direction.DOWN ? 1 : direction ==
	 * Direction.UP ? -1 : 0;
	 * 
	 * int x = direction == Direction.LEFT ? size - 1 : 0; int y = direction ==
	 * Direction.UP ? size - 1 : 0;
	 * 
	 * for (int i = 0; i < size; i++) { Color c = ColorUtil.blend(c1, c2,
	 * (double) i / size); image.setRGB(x, y, c.getRGB()); x += dx; y += dy; }
	 * 
	 * return image; }
	 */
	/*
	 * private static void drawLines(Direction direction, Graphics g, int x, int
	 * y, int width, int height, Color c1, Color c2) { int size =
	 * direction.isHorizontal() ? width : height; Int4 c =
	 * ImageUtils.toInt4(c1); Int4 dc =
	 * ImageUtils.toInt4(c2).sub(ImageUtils.toInt4(c1)).div(size);
	 * 
	 * for (int i = 0; i < size; i++) { g.setColor(ImageUtils.toColor(c));
	 * 
	 * if (direction == Direction.RIGHT) g.drawLine(x + i, y, x + i, y + height
	 * - 1); else if (direction == Direction.DOWN) g.drawLine(x, y + i, x +
	 * width - 1, y + i); else if (direction == Direction.LEFT) g.drawLine(x +
	 * width - 1 - i, y, x + width - 1 - i, y + height - 1); else g.drawLine(x,
	 * y + height - 1 - i, x + width - 1, y + height - 1 - i);
	 * 
	 * c.add(dc); } }
	 */
	...
	private Image createGradientImage(Color[] colors) {
		return Toolkit.getDefaultToolkit().createImage(
				new MemoryImageSource(size, size, ImageUtils.createGradientPixels(colors, size, size), 0, size));
	}
	...
	public Color getColor(Component component) {
		updateColors(component);
		return ColorUtil.blend(ColorUtil.blend(colors[0], colors[1], 0.5), ColorUtil.blend(colors[2], colors[3], 0.5),
				0.5);
	}
}
