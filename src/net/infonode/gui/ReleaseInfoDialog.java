...
public class ReleaseInfoDialog {
	...
	public static void showDialog(ReleaseInfo[] info, String[] text) {
		...
		JScrollPane scrollPane = new JScrollPane(message, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER) {
			public Dimension getPreferredSize() {
				Dimension d = message.getPreferredSize();
				int height = (int) d.getHeight();
				return new Dimension((int) d.getWidth() + 50,
						height < 300 ? (int) super.getPreferredSize().getHeight() : 400);
			}
		};
	...
	private static JComponent constructMessage(ReleaseInfo[] info, String[] text) {
		...
		for (int i = 0; i < info.length; i++) {
			JLabel l = new JLabel("<html><body>" + (text == null || text[i] == null ? "" : text[i] + "<br>") + "<table>"
					+ "<tr><td style='font-weight: bold;'>Product Name:</td><td>" + info[i].getProductName()
					+ "</td></tr>" + "<tr><td style='font-weight: bold;'>Version:</td><td>"
					+ info[i].getProductVersion().toString() + "</td></tr>"
					+ "<tr><td style='font-weight: bold;'>Build Time:</td><td>"
					+ DATE_FORMAT.format(new Date(info[i].getBuildTime())) + "</td></tr>"
