...
public class DirectionLayout implements LayoutManager2 {
	...
	public void layoutContainer(Container parent) {
		...
		/*
		 * System.out.println("Parent: " + parent.getComponentCount() +
		 * "  List: " + components.length); for (int i = 0; i <
		 * components.length; i++) { System.out.println("Parent: " +
		 * parent.getComponent(i) + "  List: " + components[i]); }
		 */
		...
		// int otherSize = Math.min(getOtherSize(parentInteriorSize),
		// getOtherSize(LayoutUtil.getMaxPreferredSize(components)));

		// First, set componentsTotalSize of all components that fit inside the
		// limit
		do {
			lastCount = count;
		...
		for (int i = 0; i < components.length; i++) {
			...
			int yPos = yOffset + (int) ((otherSize - getOtherSize(components[i].getSize()))
					* (direction == Direction.DOWN || direction == Direction.LEFT ? 1 - components[i].getAlignmentY()
			...
			// System.out.println("\n" + components[i] + " " +
			// components[i].getLocation() + " " + components[i].getSize() + " "
			// + ((JComponent)components[i]).getInsets());
		}
	}
	...
	public Dimension maximumLayoutSize(Container parent) {
		...
		for (int i = 0; i < c.length; i++) {
			...
			// maxHeight = Math.min(getOtherSize(c[i].getMaximumSize()),
			// maxHeight);
		}
