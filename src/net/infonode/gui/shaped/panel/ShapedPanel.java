...
/**
 * <p>
 * A panel that has support for a {@link ComponentPainter} and a
 * {@link ShapedBorder}. The background of the panel is painted as normal and
 * then the {@link ComponentPainter} paints the area inside the
 * {@link ShapedBorder} or the complete component area if the its border isn't a
 * {@link ShapedBorder}.
 * </p>
 *
 * <p>
 * If a {@link ShapedBorder} is applied to this panel, mouse events etc. are
 * only triggered for this panel if the point is inside the {@link Shape} of the
 * {@link ShapedBorder}. Child components of this panel can optionally be
 * clipped using the {@link Shape}.
 * </p>
 *
 * <p>
 * A {@link ShapedBorder} wrapped inside {@link CompoundBorder}'s will be used
 * by the ShapedPanel, but a {@link ShapedBorder} wrapped inside other border
 * types can't be found and is hence not used by the panel.
 * </p>
 *
 * @author johan
 */
public class ShapedPanel extends BaseContainer implements BackgroundPainter {
	...
	public Shape getShape() {
		...
		return b == null ? null
				: b.getShape(this, shapedInsets.left, shapedInsets.top,
	...
	protected void paintChildren(Graphics g) {
		if (clipChildren) {
			...
			if (shape != null) {
				 ...
				 * 
				 * if (sb != null) sb.paintBorder(this, g, shapedInsets.left,
				 * shapedInsets.top, getWidth() - shapedInsets.left
				 * -shapedInsets.right, getHeight() - shapedInsets.top -
				 * shapedInsets.bottom);
				 */
				return;
			}
	...
	protected void paintComponent(Graphics g) {
		...
		if (painter != null) {
			...
			if (shape != null) {
				...
				g.clipRect(shapedInsets.left, shapedInsets.top, getWidth() - shapedInsets.left - shapedInsets.right,
						getHeight() - shapedInsets.top - shapedInsets.bottom);
