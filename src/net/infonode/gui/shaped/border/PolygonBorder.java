...
public class PolygonBorder extends AbstractPolygonBorder {
	...
	public PolygonBorder(ColorProvider lineColor, ColorProvider highlightColor, int[] coords, float[] widthFactors,
			float[] heightFactors) {
	...
	public PolygonBorder(ColorProvider lineColor, ColorProvider highlightColor, ColorProvider middleColor,
			ColorProvider shadowColor, int[] coords, float[] widthFactors, float[] heightFactors) {
	...
	protected Polygon createPolygon(Component c, int width, int height) {
		...
		for (int i = 0; i < xc.length; i++) {
			xc[i] = (int) (widthFactors[n] * width) + (int) (heightFactors[n] * height) + coords[n];// (coords[n]
																									// >=
																									// 0
																									// ?
																									// coords[n]
																									...
																									// coords[n]);
			n++;
			yc[i] = (int) (widthFactors[n] * width) + (int) (heightFactors[n] * height) + coords[n];// (coords[n]
																									// >=
																									// 0
																									// ?
																									// coords[n]
