...
abstract public class AbstractPolygonBorder extends AbstractShapedBorder {
	...
	protected AbstractPolygonBorder(ColorProvider lineColor, ColorProvider highlightColor, ColorProvider middleColor,
	...
	protected void paintPolygon(Component c, Graphics2D g, Polygon polygon, int width, int height) {
		...
		while (i < polygon.npoints) {
			if (lineIsDrawn(i, polygon)) {
				...
				GraphicsUtil.drawOptimizedLine(g, polygon.xpoints[i], polygon.ypoints[i], polygon.xpoints[ni],
						polygon.ypoints[ni]);
			}
	...
	protected void paintHighlight(Component c, Graphics2D g, Polygon polygon, int width, int height) {
		...
		for (int i = 0; i < polygon.npoints; i++) {
			...
			if (lineIsDrawn(i, polygon)) {
				HighlightPainter.drawLine(g, polygon.xpoints[i], polygon.ypoints[i], polygon.xpoints[ni],
						polygon.ypoints[ni], clockWise, true, c1, c2, c3);
			}
		}
	}
	...
	/*
	 * protected int getWidth(Component c) { Insets i = getOuterInsets(c);
	 * Direction d = ShapedUtil.getDirection(c); if (d == Direction.UP || d ==
	 * Direction.DOWN) return c.getHeight() - i.top - i.bottom;
	 * 
	 * return c.getWidth() - i.left - i.right; }
	 * 
	 * protected int getHeight(Component c) { Insets i = getOuterInsets(c);
	 * Direction d = ShapedUtil.getDirection(c); if (d == Direction.UP || d ==
	 * Direction.DOWN) return c.getWidth() - i.left - i.right;
	 * 
	...
	protected int getHighlightOffsetX(int deltaX, int deltaY) {
		return deltaY - deltaX > 0 ? (deltaX + deltaY > 0 ? -1 : 0) : (deltaX + deltaY > 0 ? 0 : 1); // -deltaY
																										// >
																										// deltaX
																										// ?
																										// 1
	...
	protected int getHighlightOffsetY(int deltaX, int deltaY) {
		return deltaY - deltaX > 0 ? (deltaX + deltaY > 0 ? 0 : -1) : (deltaX + deltaY > 0 ? 1 : 0); // -deltaY
																										// >
																										// deltaX
																										// ?
																										// 1
