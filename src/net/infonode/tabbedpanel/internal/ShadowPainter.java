...
public class ShadowPainter {
	...
	public ShadowPainter(Component component, JComponent componentsPanel, JComponent highlightedTab,
			JComponent contentPanel, JComponent tabAreaComponentsPanel, JComponent tabAreaContainer, JComponent tabBox,
			Direction tabOrientation, boolean paintTabAreaShadow, int shadowSize, int shadowBlendSize,
			Color shadowColor, float shadowStrength, boolean highlightedTabIsLast) {
		this.component = component;
		this.componentsPanel = componentsPanel;
		this.highlightedTab = highlightedTab == null ? null
				: !highlightedTab.isVisible() || !tabAreaContainer.isVisible() ? null : highlightedTab;
	...
	public void paint(Graphics g) {
		...
		panelBackgroundColor = panelBackgroundColor == null ? UIManagerUtil.getColor("Panel.background", "control")
		...
		if (paintTabAreaShadow) {
			...
			tabBackgroundColor = highlightedTab == null ? panelBackgroundColor
			...
			if (tabOrientation != Direction.RIGHT || highlightedTab == null)
				drawBottomRightEdgeShadow(g, contentPanelBounds.y - (tabOrientation == Direction.UP ? len : 0),
						...
						contentPanelBounds.height + (!tabOrientation.isHorizontal() ? len : 0), true,
						highlightedTab == null ? null : panelBackgroundColor);

			if (tabOrientation != Direction.DOWN || highlightedTab == null)
				drawBottomRightEdgeShadow(g, contentPanelBounds.x - (tabOrientation == Direction.LEFT ? len : 0),
						contentPanelBounds.y + contentPanelBounds.height,
						contentPanelBounds.width + (tabOrientation.isHorizontal() ? len : 0), false,
						highlightedTab == null ? null : panelBackgroundColor);
	...
	private int paintHighlightedTabShadow(Graphics g, Direction tabOrientation, Rectangle contentPanelBounds) {
		...
		Rectangle bounds = tabAreaComponentsPanel.isVisible()
				? SwingUtilities.convertRectangle(tabAreaComponentsPanel.getParent(),
						tabAreaComponentsPanel.getBounds(), component)
				: new Rectangle(contentPanelBounds.x + contentPanelBounds.width,
						contentPanelBounds.y + contentPanelBounds.height, 0, 0);
		...
		if (tabOrientation == Direction.DOWN)
			drawBottomRightTabShadow(g, contentPanelBounds.x, contentPanelBounds.y + contentPanelBounds.height,
					contentPanelBounds.width, p.x, highlightedTab.getWidth(), highlightedTab.getHeight(), bounds.x,
					bounds.width, bounds.height, false, highlightedTabIsLast);
		else if (tabOrientation == Direction.RIGHT)
			drawBottomRightTabShadow(g, contentPanelBounds.y, contentPanelBounds.x + contentPanelBounds.width,
					contentPanelBounds.height, p.y, highlightedTab.getHeight(), highlightedTab.getWidth(), bounds.y,
					bounds.height, bounds.width, true, highlightedTabIsLast);
		else if (tabOrientation == Direction.UP) {
			drawTopLeftTabShadow(g, p.x + highlightedTab.getWidth(), p.y, highlightedTab.getHeight(), bounds,
					contentPanelBounds.width, false, highlightedTabIsLast);
		} else
			drawTopLeftTabShadow(g, p.y + highlightedTab.getHeight(), p.x, highlightedTab.getWidth(),
					flipRectangle(bounds), contentPanelBounds.height, true, highlightedTabIsLast);

		if (highlightedTabIsLast) {
			return tabOrientation.isHorizontal()
					? (p.y + highlightedTab.getHeight() >= contentPanelBounds.height
							&& contentPanelBounds.height == height ? highlightedTab.getWidth() : 0)
					: (p.x + highlightedTab.getWidth() >= contentPanelBounds.width && contentPanelBounds.width == width
							? highlightedTab.getHeight() : 0);
		} else
			return 0;
	}
	...
	private void drawTopLeftTabShadow(Graphics g, int x, int y, int height, Rectangle componentsBounds, int totalWidth,
			boolean flip, boolean isLast) {
		...
		if (!connected || y + shadowSize + shadowBlendSize <= componentsBounds.y) {
			...
			drawEdgeShadow(g, y, connected ? componentsBounds.y : y + height, Math.min(x, componentsBounds.x), false,
					!flip, isLast ? tabBackgroundColor : null);
		}
		...
		if (endX < totalWidth) {
			...
			drawEdgeShadow(g, componentsBounds.y, componentsBounds.y + componentsBounds.height, endX, false, !flip,
					tabBackgroundColor);
		}
	}

	private void drawBottomRightTabShadow(Graphics g, int x, int y, int width, int tabX, int tabWidth, int tabHeight,
			int componentsX, int componentsWidth, int componentsHeight, boolean flip, boolean isLast) {
		...
		if (tabX < componentsX) {
			...
			if (endX < x + width && (!connected || componentsHeight < tabHeight)) {
				...
				drawEdgeShadow(g, y + (connected ? componentsHeight : 0), y + tabHeight, endX, true, !flip,
		...
		if (componentsHeight > 0 && (!connected || tabHeight != componentsHeight)) {
			if (!connected || componentsHeight > tabHeight) {
				...
				drawEdgeShadow(g, componentsX, componentsX + componentsWidth, y + componentsHeight, false, flip,
						panelBackgroundColor);
			} else
				drawEdgeShadow(g, componentsX, componentsX + componentsWidth, y + componentsHeight, true, flip,
						panelBackgroundColor);
		}
		...
		if (componentsX + componentsWidth < x + width) {
			...
			drawEdgeShadow(g, y, y + componentsHeight, componentsX + componentsWidth, true, !flip,
					panelBackgroundColor);
	...
	private void drawEdgeShadow(Graphics g, int startX, int endX, int y, boolean cornerStart, boolean vertical,
		...
		for (int i = 0; i < shadowSize - shadowBlendSize; i++) {
			drawLine(g, startX + (cornerStart ? i + (vertical ? 1 : 0) : shadowSize + shadowBlendSize), y + i, endX - 1,
					y + i, vertical);
		}
		...
		for (int i = 0; i < shadowBlendSize; i++) {
			g.setColor(getShadowBlendColor(i, backgroundColor));
			int d = shadowSize - shadowBlendSize + i;
			drawLine(g, startX + (cornerStart ? d + (vertical ? 1 : 0) : shadowSize + shadowBlendSize), y + d, endX - 1,
					y + d, vertical);
		}
	}

	/*
	 * private void drawShadowLine(Graphics g, int startX, int endX, int y,
	 * boolean vertical, Color backgroundColor) { if (startX >= endX) return;
	 * 
	 * g.setColor(getShadowColor(backgroundColor));
	 * 
	 * for (int i = 0; i < shadowSize - shadowBlendSize; i++) { drawLine(g,
	 * startX, y + i, endX - 1, y + i, vertical); }
	 * 
	 * for (int i = 0; i < shadowBlendSize; i++) {
	 * g.setColor(getShadowBlendColor(i, backgroundColor)); int d = shadowSize -
	 * shadowBlendSize + i; drawLine(g, startX, y + d, endX - 1, y + d,
	 * vertical); } }
	 */
	...
	private Color getShadowBlendColor(int offset, Color backgroundColor) {
		return backgroundColor == null
				? new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(),
						(int) (255F * shadowStrength * (shadowBlendSize - offset) / (shadowBlendSize + 1)))
				: ColorUtil.blend(backgroundColor, shadowColor,
						shadowStrength * (shadowBlendSize - offset) / (shadowBlendSize + 1));
	}

	private Color getShadowColor(Color backgroundColor) {
		return backgroundColor == null ? new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(),
				(int) (255F * shadowStrength)) : ColorUtil.blend(backgroundColor, shadowColor, shadowStrength);
	}
}
