...
public class TabbedHoverUtil {
		...
		while (c != null) {
			...
			if (tp2 != null)
				if (tp2 == tp)
					...
					return false;
			/*
			 * if (c instanceof TabbedPanel) if (c == tp) return true; else
			 * return false;
			 * 
			 * if (c instanceof TabbedPanelContentPanel) if
			 * (((TabbedPanelContentPanel) c).getTabbedPanel() == tp) return
	...
	public static boolean hasVisibleTabbedPanelChild(Component c) {
		if (c instanceof TabbedPanel
				&& ((TabbedPanel) c).getProperties().getHoverPolicy() != TabbedPanelHoverPolicy.ALWAYS_AND_EXCLUDE)
			return true;
	...
	public static boolean acceptTabbedPanelHover(TabbedPanelHoverPolicy policy, ArrayList enterables, TabbedPanel tp,
			Component c) {
