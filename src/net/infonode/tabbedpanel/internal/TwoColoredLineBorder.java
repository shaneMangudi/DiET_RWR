...
/**
 * TwoColoredLineBorder draws a 1 pixel wide line. The border can have one color
 * for the top and left line and another color for the bottom and right line.
 *
...
public class TwoColoredLineBorder extends RoundedCornerBorder {
	 ...
	 *
	 * @param topLeftColor
	 *            the colorprovider for the top and left lines
	 * @param bottomRightColor
	 *            the colorprovider for the bottom and right lines
	 * @param roundCorners
	 *            true for round corners on the side facing away from the tab
	 *            area
	 * @param open
	 *            true for no border on the side towards the tab area
	 */
	public TwoColoredLineBorder(ColorProvider topLeftColor, ColorProvider bottomRightColor, boolean roundCorners,
			boolean open) {
		super(topLeftColor, null, roundCorners ? 2 : 0, roundCorners ? 2 : 0, roundCorners && !open ? 2 : 0,
				roundCorners && !open ? 2 : 0, true, true, !open, true);
		...
		if (tp != null) {
			...
			if (d == Direction.UP) {
				...
				while (i < (roundCorners ? 3 : 1)) {
					GraphicsUtil.drawOptimizedLine(g, polygon.xpoints[i], polygon.ypoints[i], polygon.xpoints[i + 1],
							polygon.ypoints[i + 1]);
					i++;
				}
				...
				g.setColor(c1);
				GraphicsUtil.drawOptimizedLine(g, polygon.xpoints[i], polygon.ypoints[i], polygon.xpoints[0],
						polygon.ypoints[0]);
