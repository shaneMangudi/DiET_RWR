	...
	private void initTheme() {
		PropertyMapManager.runBatch(new Runnable() {
			public void run() {
				...
				{
					tpProps.setShadowEnabled(false).setTabSpacing(ui.getTabSpacing())
							.setTabScrollingOffset(ui.getScrollOffset()).setEnsureSelectedTabVisible(true);
					tpProps.getTabAreaComponentsProperties().getComponentProperties().setBorder(null)
							.setInsets(InsetsUtil.EMPTY_INSETS);

					tpProps.getTabAreaProperties().getComponentProperties().setInsets(InsetsUtil.EMPTY_INSETS)
						...
						public Insets getBorderInsets(Component c) {
							...
							return tp.isTabAreaVisible()
									? ui.getTabAreaInsets(tp.getProperties().getTabAreaOrientation())
					...
					tpProps.getTabAreaComponentsProperties().getShapedPanelProperties()
							.setOpaque(ui.isTabAreaComponentsOpaque());
					...
					tpProps.getTabAreaProperties().getShapedPanelProperties().setOpaque(ui.isTabAreaOpaque())
						...
						public void paint(Component component, Graphics g, int x, int y, int width, int height,
								Direction direction, boolean horizontalFlip, boolean verticalFlip) {
					...
					tpProps.getContentPanelProperties().getComponentProperties().setInsets(InsetsUtil.EMPTY_INSETS)
							.setBorder(new Border() {
						public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
						}
						...
						public Insets getBorderInsets(Component c) {
							...
							return ui.getContentInsets(tp.getProperties().getTabAreaOrientation(),
									tp.isTabAreaVisible());
						}
				...
				{
					tabProps.setSizePolicy(TitledTabSizePolicy.INDIVIDUAL_SIZE)
					...
					tabProps.getNormalProperties().setIconTextGap(ui.getTextIconGap())
							.setTextTitleComponentGap(ui.getTextIconGap());
					...
					tabProps.getNormalProperties().getShapedPanelProperties().setOpaque(false)
							.setComponentPainter(null);

					tabProps.setMinimumSizeProvider(new DimensionProvider() {
						public Dimension getDimension(Component c) {
							return ui.getTabExternalMinSize(TabbedUtils.getParentTab(c).getTabbedPanel().getProperties()
	...
	private Border createTabInsetsBorder(final boolean selected) {
		return new Border() {
			...
			public Insets getBorderInsets(Component c) {
				...
				Direction tabDirection = selected ? tab.getProperties().getHighlightedProperties().getDirection()
						: tab.getProperties().getNormalProperties().getDirection();

				return selected ? ui.getSelectedTabInsets(areaOrientation, tabDirection)
