...
public class SmallFlatTheme extends TabbedPanelTitledTabTheme {
	...
	public SmallFlatTheme() {
		...
		tabProperties.getNormalProperties().getComponentProperties().setBorder(border).setFont(font)
				.setInsets(new Insets(0, 4, 0, 4));
		tabProperties.getHighlightedProperties().getComponentProperties().setBorder(border);
		tabProperties.setHighlightedRaised(0);

		tabbedPanelProperties.getContentPanelProperties().getComponentProperties().setBorder(new OpenContentBorder());
