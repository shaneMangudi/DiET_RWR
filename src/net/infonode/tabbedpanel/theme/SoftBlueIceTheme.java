...
public class SoftBlueIceTheme extends TabbedPanelTitledTabTheme {
	...
	public SoftBlueIceTheme(ColorProvider darkColor, ColorProvider lightColor, int cornerType) {
		...
		Border roundedBorder = new RoundedCornerBorder(dark3, light, cornerType, cornerType, cornerType, cornerType,
				true, true, true, true);

		Border tabNormalBorder = roundedBorder;

		Border contentBorder = new RoundedCornerBorder(dark3, light, cornerType, cornerType, cornerType, cornerType,
		...
		tabbedPanelProperties.setPaintTabAreaShadow(true).setTabSpacing(2).setShadowEnabled(false);

		tabbedPanelProperties.getTabAreaProperties().getComponentProperties().setBorder(roundedBorder)
				.setInsets(new Insets(2, 2, 3, 3));

		tabbedPanelProperties.getTabAreaProperties().getShapedPanelProperties().setClipChildren(true)
				.setComponentPainter(areaPainter).setOpaque(false);

		tabbedPanelProperties.getTabAreaComponentsProperties().setStretchEnabled(true).getComponentProperties()
				.setBorder(null).setInsets(new Insets(0, 0, 0, 0));

		tabbedPanelProperties.getTabAreaComponentsProperties().getShapedPanelProperties().setOpaque(false);

		tabbedPanelProperties.getContentPanelProperties().getComponentProperties().setBorder(contentBorder)
				.setInsets(new Insets(3, 3, 4, 4));

		tabbedPanelProperties.getContentPanelProperties().getShapedPanelProperties().setComponentPainter(contentPainter)
		...
		titledTabProperties.getNormalProperties().getComponentProperties().setBorder(tabNormalBorder)
				.setInsets(new Insets(1, 4, 2, 5))
				.setBackgroundColor(
						titledTabProperties.getHighlightedProperties().getComponentProperties().getBackgroundColor())
				.setFont(font);

		titledTabProperties.getNormalProperties().getShapedPanelProperties().setComponentPainter(normalPainter)
				.setOpaque(false);

		Property[] linkedProperties = { ComponentProperties.BORDER, ComponentProperties.INSETS,
				ComponentProperties.FONT };

		for (int i = 0; i < linkedProperties.length; i++) {
			titledTabProperties.getHighlightedProperties().getComponentProperties().getMap().createRelativeRef(
					linkedProperties[i], titledTabProperties.getNormalProperties().getComponentProperties().getMap(),
					linkedProperties[i]);
		}
