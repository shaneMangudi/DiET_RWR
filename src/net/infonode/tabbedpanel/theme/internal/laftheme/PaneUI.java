...
public class PaneUI {
	...
	// Order is IMPORTANT!!!
	private static final Direction[] DIRECTIONS = new Direction[] { Direction.UP, Direction.LEFT, Direction.DOWN,
			Direction.RIGHT };
	...
	private void initValues(PanePainter pane, int index, Direction direction) {
...

		{
			...
			minimumSizes[index] = new Dimension(bounds.width - tabInsets[index].left - tabInsets[index].right,
					bounds.height - tabInsets[index].top - tabInsets[index].bottom);
		}
	...
	private void calculateAreaInsets(PanePainter pane, int index, Direction direction) {
		{
			...
			if (direction == Direction.UP) {
				...
				left = Math.min(selectedBounds.x, normalBounds.x);
				top = 0;
				bottom = pane.getHeight()
						- Math.max(selectedBounds.y + selectedBounds.height, normalBounds.y + normalBounds.height);
				// right = left;
			} else if (direction == Direction.LEFT) {
				top = Math.min(selectedBounds.y, normalBounds.y);
				left = Math.min(selectedBounds.x, normalBounds.x);
				...
				left = 0;
				right = pane.getWidth()
						- Math.max(selectedBounds.x + selectedBounds.width, normalBounds.x + normalBounds.width);
				// bottom = top;
			}
	...
	public Insets getContentInsets(Direction d, boolean tabAreaVisible) {
		return tabAreaVisible ? adjustedContentInsets[getDirectionIndex(d)]
				: adjustedContentInsetsTabAreaHidden[getDirectionIndex(d)];
	}
			...
			if (tp.isTabAreaVisible()) {
				...
				if (tp.getTabCount() > 0) {
					...
					if (PAINT_TAB_AREA && !(pane.getTabCount() == 0 && tabData.getTabCount() > 0)) {
						...
						Rectangle lastVisibleRect = (Rectangle) tabData.getVisibleTabRects()
								.get(tabData.getTabCount() - 1);
						Tab lastTab = (Tab) tabData.getTabList().get(tabData.getTabCount() - 1);

						if (tabData.isHorizontalLayout()) {
							int extraWidth = lastTab.getWidth() == lastVisibleRect.width ? 0
							...
							g.clipRect(aInsets.left + tabData.getTabAreaWidth() - clipExtraWidth, 0,
									width - aInsets.left - tabData.getTabAreaWidth() + clipExtraWidth, height);
							pane.paint(g, tx, ty);
							g.setClip(originalClip);
						} else {
							int extraHeight = lastTab.getHeight() == lastVisibleRect.height ? 0
							...
							g.clipRect(0, aInsets.top + tabData.getTabAreaHeight() - clipExtraHeight, width,
									height - aInsets.top - tabData.getTabAreaHeight() + clipExtraHeight);
							pane.paint(g, tx, ty);
							g.setClip(originalClip);
						}
					}
	...
	private void paintTabs(PanePainter pane, TabData tabData, Graphics g, int x, int y, int width, int height,
		...
		if (enabled) {
			if (PAINT_TAB) {
				...
				if (tabData.isHorizontalLayout()) {
					int w = aInsets.left + aInsets.right
							+ Math.max(0, tabData.getTabAreaWidth() - l.x - lastVisibleRect.width) + EXTRA_SIZE;
					...
					int h = aInsets.top + aInsets.bottom
							+ Math.max(0, tabData.getTabAreaHeight() - l.y - lastVisibleRect.height) + EXTRA_SIZE;
				...
				if (tabData.isHorizontalLayout()) {
					tx -= (tabData.getPreTab() != null ? tab.getX() - tabData.getPreTab().getX() + visibleRect.x
							: visibleRect.x);
					g.clipRect(aInsets.left, 0, tabData.getTabAreaWidth(), height);
				} else {
					ty -= (tabData.getPreTab() != null ? tab.getY() - tabData.getPreTab().getY() + visibleRect.y
							: visibleRect.y);
		...
		if (enabled) {
			if (PAINT_CONTENT_AREA) {
				...
				if (tabData.getTabbedPanel().hasContentArea()) {
					...
					if (/* !tabData.getTabbedPanel().hasContentArea() || */(pane.getTabCount() == 0
							&& tabData.getTabCount() > 0)) {
					...
					if (tabAreaNotVisibleFix && !tabData.getTabbedPanel().isTabAreaVisible()) {
						extraWidth = !tabData.isHorizontalLayout()
								...
								: 0;
						extraHeight = tabData.isHorizontalLayout()
					...
					pane.setSize(tabData.getTabbedPanelSize().width - Math.abs(xComp) + extraWidth,
	...
	private void initPaintableTabLocations(PanePainter pane) {
		...
		for (int i = 0; i < tabData.getTabCount(); i++) {
			...
			SizeIcon icon = new SizeIcon(getInternalTabWidth(tab) - getWidthCompensate(tabData.getAreaOrientation()),
	...
	private void initTabLocations(PanePainter pane) {
		...
		if (tabData.getTabbedPanel().isTabAreaVisible()) {
			for (int i = 0; i < tabData.getTabCount(); i++) {
				...
				if ((tabData.isHorizontalLayout() && (visibleRect.width >= minSize.width
						|| minSize.width < tabData.getTabbedPanelWidth() - l.x - aInsets.right))
						|| (!tabData.isHorizontalLayout() && (visibleRect.height >= minSize.height
								|| minSize.height < tabData.getTabbedPanelHeight() - l.y - aInsets.bottom))) {
					final int iWidth = iconWidth;
					final int iHeight = iconHeight;

					SizeIcon icon = new SizeIcon(iWidth - getWidthCompensate(tabData.getAreaOrientation()),
	...
	private void findPaintableTabs() {
		...
		if (tabData.getTabbedPanel().isTabAreaVisible()) {
			while (i < tabData.getTabbedPanel().getTabCount()) {
				...
				if (tabsFound && r.x == 0 && r.y == 0 && ((tabData.isHorizontalLayout() && r.width < tab.getWidth())
