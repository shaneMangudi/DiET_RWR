	...
	public ClassicTheme(final int raised) {
		...
		shadowBorder = new Border() {
			...
			public Insets getBorderInsets(Component c) {
				...
				if (tp != null) {
					Direction d = tp.getProperties().getTabAreaOrientation();
					return new Insets(0, 0, (d == Direction.LEFT || d == Direction.RIGHT) ? 1 : 0,
		...
		tabbedPanelProperties.getTabAreaComponentsProperties().getComponentProperties()
				.setBorder(doCreateTabBorder(false, false, true, true));
		...
		Insets highlightInsets = new Insets(0, (raised + 1) / 2 + (((raised + 1) & 1) == 1 ? 1 : 0) + normalInsets.left,
				1, (raised + 1) / 2 + normalInsets.right);

		titledTabProperties.setHighlightedRaised(raised).setBorderSizePolicy(TitledTabBorderSizePolicy.INDIVIDUAL_SIZE);
		titledTabProperties.getNormalProperties().getComponentProperties().setBorder(normalBorder)
				.setInsets(normalInsets);
		titledTabProperties.getHighlightedProperties().getComponentProperties().setBorder(highlightBorder)
				.setInsets(highlightInsets);
	}
	...
	public Border createInsetsTabBorder(boolean roundEdges, boolean open, boolean highlight) {
		Border insetsBorder = new Border() {
			...
			public Insets getBorderInsets(Component c) {
				...
				if (tp != null) {
					Direction d = tp.getProperties().getTabAreaOrientation();
					return new Insets(d == Direction.RIGHT || d == Direction.LEFT ? raised : 0,
							d == Direction.UP || d == Direction.DOWN ? raised : 0,
							d == Direction.RIGHT || d == Direction.LEFT ? 1 : 0,
							d == Direction.UP || d == Direction.DOWN ? 1 : 0);
				}
				return new Insets(0, 0, 0, 0);
			}
	...
	private ColorProvider createNormalHighlightColorProvider() {
		return new ColorProvider() {
			public Color getColor() {
				...
				highlightBackground = new Color(highlightBackground.getRed() == 0 ? 1 : highlightBackground.getRed(),
						highlightBackground.getGreen() == 0 ? 1 : highlightBackground.getGreen(),
