...
public class GradientTheme extends TabbedPanelTitledTabTheme {
	...
	/**
	 * The tab area background color used if no color is specified in the
	 * constructor.
	 */
	 ...
	 *
	 * @param opaqueTabArea
	 *            if true a gradient background is drawn for the tab area,
	 ...
	 * @param borderColor
	 *            the border color, null means default border color
	 * @param tabAreaBackgroundColor
	 *            the background color for the tab area and normal tabs, null
	 *            means use the default tab background
	 */
	public GradientTheme(boolean opaqueTabArea, boolean shadowEnabled, Color borderColor,
	...
	private void initTabbedPanelProperties() {
		...
		tabbedPanelProperties.getContentPanelProperties().getComponentProperties().setInsets(new Insets(3, 3, 3, 3))
				.setBorder(new OpenContentBorder(borderColor, opaqueTabArea ? 0 : 1));

		tabbedPanelProperties.setShadowEnabled(shadowEnabled).setPaintTabAreaShadow(opaqueTabArea)
		...
		tabbedPanelProperties.getTabAreaComponentsProperties().setStretchEnabled(opaqueTabArea)

				.getComponentProperties()
				.setBorder(new CompoundBorder(new TabAreaLineBorder(opaqueTabArea ? null : borderColor, !opaqueTabArea,
						true, !opaqueTabArea, true), TAB_AREA_COMPONENTS_GRADIENT_BORDER))
	...
	private void initTitledTabProperties() {
		...
		titledTabProperties.getNormalProperties().getComponentProperties()
				.setBorder(opaqueTabArea ? (Border) new TabAreaLineBorder(false, false, true, true)
						: new CompoundBorder(new TabAreaLineBorder(), normalTabGradientBorder));

		if (opaqueTabArea)
			titledTabProperties.getNormalProperties().getComponentProperties().setBackgroundColor(null);
		...
		titledTabProperties.getHighlightedProperties().setIconVisible(true)
				...
				.getComponentProperties()
				.setBorder(new CompoundBorder(opaqueTabArea ? (Border) new TabAreaLineBorder(false, false, true, true)
						: new TabAreaLineBorder(borderColor), HIGHLIGHTED_TAB_GRADIENT_BORDER));
	}
	...
	/**
	 * Returns the gradient border for the normal tab or the tab area if it's
	 * opaque.
	 *
	 * @return the gradient border for the normal tab or the tab area if it's
	 *         opaque
	 */
