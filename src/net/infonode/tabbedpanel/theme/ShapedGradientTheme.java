...
public class ShapedGradientTheme extends TabbedPanelTitledTabTheme {
	...
	private static class TabBorder extends SlopedTabLineBorder {
		...
		TabBorder(ColorProvider lineColor, ColorProvider highlightColor, float leftSlope, float rightSlope,
				int leftHeight, int rightHeight, boolean bottomLeftRounded, boolean topLeftRounded,
				boolean topRightRounded, boolean bottomRightRounded, boolean isNormal,
				boolean highlightBottomLeftRounded, int raised) {
			super(lineColor, highlightColor, false, leftSlope, rightSlope, leftHeight, rightHeight,
					isNormal ? false : bottomLeftRounded, topLeftRounded, topRightRounded, bottomRightRounded);
	...
	/**
	 * Creates a theme with the given slopes on the left and right side of the
	 * tab and with colors based on the active look and feel
	 *
	 * @param leftSlope
	 *            leaning of left slope defined as left slope width divided by
	 *            left slope height
	 * @param rightSlope
	 *            leaning of right slope defined as right slope width divided by
	 *            right slope height
	 */
	public ShapedGradientTheme(float leftSlope, float rightSlope) {
		this(leftSlope, rightSlope, UIManagerColorProvider.TABBED_PANE_DARK_SHADOW,
				UIManagerColorProvider.TABBED_PANE_HIGHLIGHT);
	}

	/**
	 * Creates a theme with the given slopes on the left and right side of the
	 * tab and with the given colors
	 *
	 * @param leftSlope
	 *            leaning of left slope defined as left slope width divided by
	 *            left slope height
	 ...
	 *            right slope height
	 * @param lineColor
	 *            color provider for the lines
	 * @param highlightColor
	 *            color provider for the highlighting, null for no highlighting
	 */
	public ShapedGradientTheme(float leftSlope, float rightSlope, ColorProvider lineColor,
	 ...
	 *            right slope height
	 * @param slopeHeight
	 *            slope height in pixels, used when estimating slope width
	 * @param lineColor
	 *            color provider for the lines
	 * @param highlightColor
	 *            color provider for the highlighting, null for no highlighting
	 */
	public ShapedGradientTheme(float leftSlope, float rightSlope, int slopeHeight, ColorProvider lineColor,
			ColorProvider highlightColor) {
		...
		alternateHighlight = highlightColor != null ? new ColorBlender(highlightColor, controlColor, 0.3f)
		...
		GradientComponentPainter blendedComponentPainter = new GradientComponentPainter(alternateHighlight,
				alternateHighlight, controlColor, controlColor);
		...
		titledTabProperties.setHighlightedRaised(highlightedRaised)
		...
		Border normalBorder = new TabBorder(lineColor, null, leftSlope, rightSlope, leftSlopeHeight, rightSlopeHeight,
				false, topLeftRounded, topRightRounded, false, true, true, highlightedRaised);
		Border highlightBorder = new TabBorder(lineColor, highlightColor, leftSlope, rightSlope, leftSlopeHeight,
				rightSlopeHeight, bottomLeftRounded, topLeftRounded, topRightRounded, bottomRightRounded, false, true,
				highlightedRaised);
		...
		GradientComponentPainter normalComponentPainter = new GradientComponentPainter(darkControlColor1,
				darkControlColor1, darkControlColor2, darkControlColor2);
		...
		int tabSpacing = 1 + insets.left + insets.right - (topLeftRounded ? CORNER_INSET : 0)
				- (topRightRounded ? CORNER_INSET : 0) - (int) (0.2 * (leftSlopeWidth + rightSlopeWidth));
		tabbedPanelProperties.setTabSpacing(-tabSpacing).setShadowEnabled(false);

		tabbedPanelProperties
				.getTabAreaComponentsProperties().getComponentProperties().setBorder(new SlopedTabLineBorder(lineColor,
						highlightColor, false, 0f, 0f, 0, 0, false, topLeftRounded, topRightRounded, false))
				.setInsets(new Insets(0, 0, 0, 0));

		tabbedPanelProperties.getTabAreaComponentsProperties().getShapedPanelProperties()
				.setComponentPainter(blendedComponentPainter);
		...
		tabbedPanelProperties.getContentPanelProperties().getComponentProperties()
				.setBorder(new OpenContentBorder(lineColor, lineColor,
						highlightColor == null ? null
								: new ColorBlender(highlightColor, BackgroundPainterColorProvider.INSTANCE,
										HighlightPainter.getBlendFactor(1, 0)),
						1));
	}
	...
	/**
	 * Gets the alternate highlight color provider used for tab area components
	 * gradient background and highlighted tab background (when no highlight
	 * color is specified)
	 *
	...
	/**
	 * Gets the dark control background color used for gradient for normal tab
	 * and disabled tab
	 *
	 ...
	 *            line color provider
	 * @param highlightColor
	 *            highlight color provider, null for no highlight
	 * @param leftSlope
	 *            left slope
	 * @param rightSlope
	 *            right slope
	 * @param bottomLeftRounded
	 *            true if bottom left should be rounded
	 * @param topLeftRounded
	 *            true if top left should be rounded
	 * @param topRightRounded
	 *            true if top right should be rounded
	 * @param bottomRightRounded
	 *            true if bottom right should be rounded
	 * @param isNormal
	 *            true if this is a normal rendered border
	 * @param highlightBottomLeftRounded
	 *            true if highlight has bottom left rounded
	 * @param raised
	 *            raised
	 * @return the created border
	 */
	public Border createTabBorder(ColorProvider lineColor, ColorProvider highlightColor, float leftSlope,
			float rightSlope, boolean bottomLeftRounded, boolean topLeftRounded, boolean topRightRounded,
			boolean bottomRightRounded, boolean isNormal, boolean highlightBottomLeftRounded, int raised) {
		return new TabBorder(lineColor, highlightColor, leftSlope, rightSlope, 25, 25, bottomLeftRounded,
				topLeftRounded, topRightRounded, bottomRightRounded, isNormal, highlightBottomLeftRounded, raised);
	}
}