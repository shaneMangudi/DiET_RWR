...
/**
 * TabAreaLineBorder draws a 1 pixel wide border on all sides except the side
 * towards the content area of a tabbed panel.
 *
...
public class TabAreaLineBorder implements Border, Serializable {
	 ...
	 *            draw the right line
	 * @param flipLeftRight
	 *            if true the left line is rotated so that it is always to the
	 *            left or at the top and vice versa for the right line, if false
	 *            the left and right lines are rotated the same way as the other
	 *            lines
	 */
	...
	public TabAreaLineBorder(Color color, boolean drawTop, boolean drawLeft, boolean drawRight, boolean flipLeftRight) {
		this(ColorProviderUtil.getColorProvider(color,
				new ColorProviderList(UIManagerColorProvider.TABBED_PANE_DARK_SHADOW,
						UIManagerColorProvider.CONTROL_DARK_SHADOW, FixedColorProvider.BLACK)),
	...
	private boolean drawLeft(Direction orientation) {
		return orientation == Direction.UP ? drawLeft
				: orientation == Direction.LEFT ? drawTop
						: orientation == Direction.DOWN ? (flipLeftRight ? drawLeft : drawRight) : false;
	}

	private boolean drawRight(Direction orientation) {
		return orientation == Direction.UP ? drawRight
				: orientation == Direction.LEFT ? false
						: orientation == Direction.DOWN ? (flipLeftRight ? drawRight : drawLeft) : drawTop;
	}
	...
	public Insets getBorderInsets(Component c) {
		...
		return orientation != null ? new Insets(drawTop(orientation) ? 1 : 0, drawLeft(orientation) ? 1 : 0,
				drawBottom(orientation) ? 1 : 0, drawRight(orientation) ? 1 : 0) : new Insets(0, 0, 0, 0);
	}
