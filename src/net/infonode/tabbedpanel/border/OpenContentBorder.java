 ...
 * component that is used as content area component in a tabbed panel. The
 * border also optionally draws a highlight inside the line on the top and left
 * sides of the component. It is open, i.e. no content border will be drawn
 * where the highlighted tab in the tabbed panel is located.
 * </p>
 *
 * <p>
 * If the highlighted tab has a
 * {@link net.infonode.gui.shaped.border.ShapedBorder} its shape will be used to
 * calculate where the OpenContentBorder should be open.
 * </p>
 *
 * @author $Author: johan $
...
public class OpenContentBorder implements Border, Serializable {
	...
	/**
	 * Constructor. Uses the TabbedPane.darkShadow color from the UIManager as
	 * line color.
	 */
	...
	/**
	 * Constructs a OpenContentBorder without highlight and with the given color
	 * as line color.
	 *
	 * @param color
	 *            the line color
	...
	/**
	 * Constructs a OpenContentBorder with highlight and with the given colors
	 * as line color and highlight color.
	 *
	 * @param color
	 *            the line color
	...
	public OpenContentBorder(Color color, Color highlightColor) {
		this(ColorProviderUtil.getColorProvider(color, UIManagerColorProvider.TABBED_PANE_DARK_SHADOW),
				highlightColor == null ? null : new FixedColorProvider(highlightColor), 1);
	}
	 ...
	 * @param highlightColorProvider
	 *            the highlight color provider
	 * @param tabLeftInset
	 *            the left border inset of the tab
	 */
	 ...
	 * as line color and highlight color.
	 *
	 * @param topLeftLineColor
	 *            the line color provider for the top and left lines
	 * @param bottomRightLineColor
	 *            the line color provider for the bottom and right lines
	 * @param highlightColorProvider
	 *            the highlight color provider
	 * @param tabLeftInset
	 *            the left border inset of the tab
	...
	private static int getLineIntersection(int edge, float x1, float y1, float x2, float y2, Direction orientation) {
		...
		((x1 <= edge && x2 >= edge) || (x1 >= edge && x2 <= edge)
				? Math.round(x2 == x1 ? y2 : y1 + (edge - x1) * (y2 - y1) / (x2 - x1)) : Integer.MAX_VALUE) :

		((y1 <= edge && y2 >= edge) || (y1 >= edge && y2 <= edge)
				? Math.round(y2 == y1 ? x2 : x1 + (edge - y1) * (x2 - x1) / (y2 - y1)) : Integer.MAX_VALUE));
	}

	private static Point getTabBounds(Component c, Tab tab, Direction orientation, int x, int y, int width,
			int height) {
		...
		if (shape != null) {
			int edge = orientation == Direction.UP ? tab.getHeight()
					: orientation == Direction.RIGHT ? -1 : orientation == Direction.DOWN ? -1 : tab.getWidth();
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
		...
		if (c != null && tabbedPanel != null) {
			...
			if (tab != null) {
				...
				clipOffset = (orientation.isHorizontal() ? tab.getHeight() > tabHeight : tab.getWidth() > tabWidth) ? -1
						: 0;
			}
			...
			if (orientation == Direction.UP && tab != null) {
				...
				if (highlightColorProvider != null) {
					g.setColor(hc1);
					drawLine(g, x + 1, y + 1, x + width - (orientation == Direction.RIGHT && tabStart == 0 ? 1 : 3),
							y + 1);
				}
			}
			...
			if (orientation == Direction.LEFT && tab != null) {
				...
				if (highlightColorProvider != null) {
					g.setColor(hc1);
					drawLine(g, x + 1, y + 2, x + 1,
							y + height - (orientation == Direction.DOWN && tabStart == 0 ? 1 : 3));
				}
			}
