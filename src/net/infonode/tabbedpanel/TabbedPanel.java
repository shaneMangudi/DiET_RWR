 ...
 * <a href="http://www.infonode.net/index.html?itpdemo" target="_blank"> www.
 * infonode.net</a> demonstrates and explains most of the tabbed panel's
 * features.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * It is possible to display a button in the tab area next to the tabs that
 * shows a drop down list (called tab drop down list) with all the tabs where it
 * is possible to select a tab. This is for example useful when the tabbed panel
 * contains a large amount of tabs or if some tabs are scrolled out. The drop
 * down list can show a text and an icon for a tab. The text is retrieved by
 * calling toString() on the tab and the icon is only retrieved if the tab
 * implements the {@link net.infonode.gui.icon.IconProvider} interface.
 * </p>
 *
 * <p>
 ...
 * {@link HoverEvent} and the source for the event is always the hovered tabbed
 * panel.
 * </p>
 *
 * <p>
 ...
 * <li>The hover listener for the tab area or the content area depending on
 * where the mouse pointer is located.
 * <li>The hover listener for the tab area components area if the mouse pointer
 * is over that area.
 * </ol>
 * When the tabbed panel is no longer hovered, the hover listenrs are called in
 * the reverse order.
 * </p>
 *
 * <p>
 * It is possible to specify different hover policies (
 * {@link TabbedPanelHoverPolicy}) in the TabbedPanelProperties that affects all
 * hover areas of the tabbed panel.
 * </p>
 *
...
public class TabbedPanel extends JPanel {
	...
	private DraggableComponentBox draggableComponentBox = new DraggableComponentBox(
			TabbedUIDefaults.getButtonIconSize(), false);
	...
	private class HoverablePanel extends HoverableShapedPanel {
		...
		public boolean acceptHover(ArrayList enterableHoverables) {
			return getHoverListener() == null ? false
					: TabbedHoverUtil.acceptTabbedPanelHover(properties.getHoverPolicy(), enterableHoverables,
	...
	private GridBagLayout tabAreaLayoutManager = new GridBagLayout() {
		public void layoutContainer(Container parent) {
			...
			// Overlap if tab area is too narrow to fit both
			// tabAreaComponentsPanel and draggableComponentBox
			...
			/*
			 * if (contentPanel != null) { int newSize = (tabAreaOrientation ==
			 * Direction.UP || tabAreaOrientation == Direction.DOWN) ?
			 * draggableComponentBox.getWidth() :
			 * draggableComponentBox.getHeight(); int newOuterSize =
			 * (tabAreaOrientation == Direction.UP || tabAreaOrientation ==
			 * Direction.DOWN) ? tabAreaContainer.getWidth() :
			 * tabAreaContainer.getHeight(); if (newOuterSize == outerSize &&
			 * newSize != size) { size = newSize; SwingUtilities.invokeLater(new
			 * Runnable() { public void run() { if (contentPanel.isShowing()) {
			 * contentPanel.repaint(); } } }); } outerSize = newOuterSize; }
			 */
	...
	private HoverableShapedPanel tabAreaContainer = new HoverablePanel(tabAreaLayoutManager,
			properties.getTabAreaProperties().getHoverListener()) {
	...
	private HoverableShapedPanel tabAreaComponentsPanel = new HoverablePanel(new DirectionLayout(),
			properties.getTabAreaComponentsProperties().getHoverListener()) {
		...
		public void changed(DraggableComponentBoxEvent event) {
			if (event.getDraggableComponentEvent() != null) {
				...
				if (type == DraggableComponentEvent.TYPE_PRESSED && properties.getHighlightPressedTab()) {
					...
					tabAreaContainer.repaint();
					selectedMoved = event.getDraggableComponent() == draggableComponentBox
	...
	private void updatePropertiesForTabAreaComponentsButtons(Map changes) {
			...
			if (m != null) {
				if (dropDownList != null) {
					...
					if (m.keySet().contains(ButtonProperties.FACTORY)) {
						b = properties.getButtonProperties().getTabDropDownListButtonProperties().getFactory()
								.createButton(TabbedPanel.this);
		...
		if (scrollButtonBox != null) {
			AbstractButton buttons[] = new AbstractButton[] { scrollButtonBox.getUpButton(),
					scrollButtonBox.getDownButton(), scrollButtonBox.getLeftButton(),
					scrollButtonBox.getRightButton() };

			ButtonProperties props[] = new ButtonProperties[] { buttonProps.getScrollUpButtonProperties(),
					buttonProps.getScrollDownButtonProperties(), buttonProps.getScrollLeftButtonProperties(),
	...
	private void updateAllDefaultValues() {
		...
		tabAreaComponentsPanel
				.setHorizontalFlip(tabAreaOrientation == Direction.DOWN || tabAreaOrientation == Direction.LEFT
	...
	private void updateProperties(Map changes) {
		...
		if (m != null) {
			...
			if (keySet.contains(TabbedPanelProperties.TAB_REORDER_ENABLED)
					|| m.keySet().contains(TabbedPanelProperties.ABORT_DRAG_KEY)
			...
			if (keySet.contains(TabbedPanelProperties.TAB_LAYOUT_POLICY) && getTabCount() > 1)
				draggableComponentBox.setScrollEnabled(
						((TabLayoutPolicy) ((ValueChange) m.get(TabbedPanelProperties.TAB_LAYOUT_POLICY))
								.getNewValue()) == TabLayoutPolicy.SCROLLING);
			...
			if (keySet.contains(TabbedPanelProperties.TAB_SCROLLING_OFFSET))
				draggableComponentBox.setScrollOffset(
						((Integer) ((ValueChange) m.get(TabbedPanelProperties.TAB_SCROLLING_OFFSET)).getNewValue())
								.intValue());

			if (keySet.contains(TabbedPanelProperties.ENSURE_SELECTED_VISIBLE))
				draggableComponentBox.setEnsureSelectedVisible(
						((Boolean) ((ValueChange) m.get(TabbedPanelProperties.ENSURE_SELECTED_VISIBLE)).getNewValue())
								.booleanValue());

			if (keySet.contains(TabbedPanelProperties.TAB_AREA_ORIENTATION)) {
				tabAreaOrientation = (Direction) ((ValueChange) m.get(TabbedPanelProperties.TAB_AREA_ORIENTATION))
						.getNewValue();
				updatePropertiesForTabAreaLayoutConstraints();
			...
			if (keySet.contains(TabbedPanelProperties.TAB_DEPTH_ORDER))
				draggableComponentBox.setDepthSortOrder(
						((TabDepthOrderPolicy) ((ValueChange) m.get(TabbedPanelProperties.TAB_DEPTH_ORDER))
								.getNewValue()) == TabDepthOrderPolicy.DESCENDING);

			if (keySet.contains(TabbedPanelProperties.AUTO_SELECT_TAB))
				draggableComponentBox.setAutoSelect(
						((Boolean) ((ValueChange) m.get(TabbedPanelProperties.AUTO_SELECT_TAB)).getNewValue())
								.booleanValue());

			/*
			 * if (keySet.contains(TabbedPanelProperties.HIGHLIGHT_PRESSED_TAB))
			 * { // Elsewhere }
			 *
			...
			if (keySet.contains(TabbedPanelProperties.SHADOW_ENABLED)
					|| m.keySet().contains(TabbedPanelProperties.SHADOW_STRENGTH)
					|| m.keySet().contains(TabbedPanelProperties.SHADOW_COLOR)
					|| m.keySet().contains(TabbedPanelProperties.SHADOW_BLEND_AREA_SIZE)
					|| m.keySet().contains(TabbedPanelProperties.SHADOW_SIZE)
					|| m.keySet().contains(TabbedPanelProperties.PAINT_TAB_AREA_SHADOW)) {
				shadowSize = properties.getShadowSize();
				componentsPanel.setBorder(contentPanel != null && properties.getShadowEnabled()
						? new EmptyBorder(0, 0, shadowSize, shadowSize) : null);
			}
	...
	private void updateIntelligentInsets(JComponent c, ComponentProperties props) {
		...
		if (insets != null) {
			...
			c.setBorder(b != null ? (Border) new CompoundBorder(b, new EmptyBorder(insets))
		...
		if (m != null) {
			if (m.keySet().contains(TabAreaComponentsProperties.HOVER_LISTENER)) {
				tabAreaComponentsPanel.setHoverListener(
						(HoverListener) ((ValueChange) m.get(TabAreaComponentsProperties.HOVER_LISTENER))
								.getNewValue());
			}
	...
	private void updatePropertiesForTabAreaLayoutConstraints() {
		...
		if (tabAreaOrientation == Direction.UP) {
			setTabAreaLayoutConstraints(draggableComponentBox, 0, 0, GridBagConstraints.HORIZONTAL, 1, 1,
					GridBagConstraints.SOUTH);
			setTabAreaLayoutConstraints(tabAreaComponentsPanel, 1, 0,
					stretch ? GridBagConstraints.VERTICAL : GridBagConstraints.NONE, 0, 1, GridBagConstraints.SOUTH);
			updateTabAreaComponentsPanel(Direction.RIGHT, 0, 1);
		} else if (tabAreaOrientation == Direction.DOWN) {
			setTabAreaLayoutConstraints(draggableComponentBox, 0, 0, GridBagConstraints.HORIZONTAL, 1, 1,
					GridBagConstraints.NORTH);
			setTabAreaLayoutConstraints(tabAreaComponentsPanel, 1, 0,
					stretch ? GridBagConstraints.VERTICAL : GridBagConstraints.NONE, 0, 0, GridBagConstraints.NORTH);
			updateTabAreaComponentsPanel(Direction.RIGHT, 0, 0);
		} else if (tabAreaOrientation == Direction.LEFT) {
			setTabAreaLayoutConstraints(draggableComponentBox, 0, 0, GridBagConstraints.VERTICAL, 1, 1,
					GridBagConstraints.EAST);
			setTabAreaLayoutConstraints(tabAreaComponentsPanel, 0, 1,
					stretch ? GridBagConstraints.HORIZONTAL : GridBagConstraints.NONE, 0, 0, GridBagConstraints.EAST);
			updateTabAreaComponentsPanel(Direction.DOWN, 0, 0);
		} else {
			setTabAreaLayoutConstraints(draggableComponentBox, 0, 0, GridBagConstraints.VERTICAL, 1, 1,
					GridBagConstraints.WEST);
			setTabAreaLayoutConstraints(tabAreaComponentsPanel, 0, 1,
					stretch ? GridBagConstraints.HORIZONTAL : GridBagConstraints.NONE, 0, 0, GridBagConstraints.WEST);
	 ...
	 * TabbedPanelProperties. The properties for the content area will not be
	 * used.
	 * </p>
	 *
	 * <p>
	 ...
	 * </p>
	 *
	 * @param contentAreaComponent
	 *            component to be used as content area component or null for no
	 ...
	 *            content area component
	 * @param useProperties
	 *            true if the properties for the content area should be used,
	 *            otherwise false
	 * @see TabbedPanelProperties
	 * @since ITP 1.4.0
	 */
	 ...
	 *
	 * @param p
	 *            the point to check. Must be relative to this tabbed panel.
	 ...
	 *            tab to be inserted
	 * @param p
	 *            the point to insert tab at. Must be relative to this tabbed
	 *            panel.
	 * @see #addTab
	 * @see TabbedPanelProperties
	 */
	 ...
	 *
	 * @param tab
	 *            tab to move. Tab must be a member (added/inserted) of this
	 *            tabbed panel.
	 * @param p
	 ...
	 *
	 * @param tab
	 *            tab to select. Tab must be a member (added/inserted) of this
	 *            tabbed panel.
	 */
	 ...
	 *            tab that should be highlighted or null if no tab should be
	 ...
	 *
	 * @param tab
	 *            tab to scroll into visible tab area
	 * @since ITP 1.4.0
	 */
	 ...
	 * orientation.
	 *
	 * The components will be laid out in a line and the direction will change
	 * depending on the tab area orientation. Tab drop down list and scroll
	 * buttons are also tab area components but those are handled automatically
	 * by the tabbed panel and are not affected by calling this method.
	 *
	 * @param tabAreaComponents
	 *            array of components, null for no components
	 * @since ITP 1.1.0
	 */
	...
	/**
	 * Gets if any tab area components i.e. scroll buttons etc are visible at
	 * the moment
	 *
	 * @return true if visible, otherwise false
	 * @since ITP 1.2.0
	 ...
	 * those are handled automatically by the tabbed panel and no references to
	 * them will be returned. This method only returns the components that have
	 * been set with the setTabAreaComponents method.
	 *
	...
	/*
	 * private void updateProperties(Map changes) {
	 * //componentsPanel.remove(draggableComponentBox); tabAreaOrientation =
	 * properties.getTabAreaOrientation(); updateTabArea();
	 * updateAllTabsProperties();
	 * 
	 * componentsPanel.add(tabAreaContainer,
	 * ComponentUtil.getBorderLayoutOrientation(tabAreaOrientation));
	 * 
	 * // Shadow shadowSize = properties.getShadowSize();
	 * componentsPanel.setBorder( contentPanel != null &&
	 * properties.getShadowEnabled() ? new EmptyBorder(0, 0, shadowSize,
	 * shadowSize) : null);
	 * 
		...
		if (newListVisiblePolicy != listVisiblePolicy || newListTabLayoutPolicy != listTabLayoutPolicy) {
			...
			if (newListVisiblePolicy == TabDropDownListVisiblePolicy.MORE_THAN_ONE_TAB
				...
				dropDownList = new TabDropDownList(this,
						properties.getButtonProperties().getTabDropDownListButtonProperties()
								.applyTo(properties.getButtonProperties().getTabDropDownListButtonProperties()
	...
	private void updateTabProperties(Tab tab) {
		...
		tab.getDraggableComponent()
				.setSelectOnMousePress(properties.getTabSelectTrigger() == TabSelectTrigger.MOUSE_PRESS);
	}
	...
	private void setTabAreaLayoutConstraints(JComponent c, int gridx, int gridy, int fill, double weightx,
	...
	private int calcScrollWidth() {
		...
		boolean includeDropDownWidth = dropDownList != null
				&& listVisiblePolicy == TabDropDownListVisiblePolicy.TABS_NOT_VISIBLE;
	...
	private int calcScrollHeight() {
		...
		int componentsPanelHeight = tabAreaComponentsPanel.isVisible()
				? ((int) tabAreaComponentsPanel.getPreferredSize().getHeight() - insetsHeight
						- (scrollButtonBox.isVisible()
			...
			if (scrollButtonBox != null) {
				scrollButtonBox.setButtons(
						properties.getButtonProperties().getScrollUpButtonProperties()
								.applyTo(properties.getButtonProperties().getScrollUpButtonProperties().getFactory()
										.createButton(this)),
						properties.getButtonProperties().getScrollDownButtonProperties()
								.applyTo(properties.getButtonProperties().getScrollDownButtonProperties().getFactory()
										.createButton(this)),
						properties.getButtonProperties().getScrollLeftButtonProperties()
								.applyTo(properties.getButtonProperties().getScrollLeftButtonProperties().getFactory()
										.createButton(this)),
						properties.getButtonProperties().getScrollRightButtonProperties()
								.applyTo(properties.getButtonProperties().getScrollRightButtonProperties().getFactory()
										.createButton(this)));
	...
	private void updatePanelOpaque() {
		if (!properties.getShadowEnabled() && properties.getTabAreaProperties().getShapedPanelProperties().getOpaque()
				&& (contentPanel == null ? true
						: properties.getContentPanelProperties().getShapedPanelProperties().getOpaque())) {
	...
	private class ShadowPanel extends HoverablePanel {
		...
		public void paint(Graphics g) {
			...
			new ShadowPainter(this, componentsPanel, highlightedTab, contentPanel, tabAreaComponentsPanel,
					tabAreaContainer, draggableComponentBox, properties.getTabAreaOrientation(),
					properties.getPaintTabAreaShadow(), shadowSize, properties.getShadowBlendAreaSize(),
					properties.getShadowColor(), properties.getShadowStrength(),
