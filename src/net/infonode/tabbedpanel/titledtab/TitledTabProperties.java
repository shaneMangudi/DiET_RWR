 ...
 *
 * <p>
 * A titled tab can have three states, normal, highlighted and disabled. Each
 * state is represented by a {@link TitledTabStateProperties} object containing
 * all properties that can be set for a state.
 * </p>
 *
 * <p>
 * By default the property values in the highlighted and disabled state are
 * references to corresponding values in the normal state. This means that if
 * you set a property value in the normal state, then highlighted and the
 * disabled state will use that property value if the property has not been set
 * in the highlighted or disabled state.
 * </p>
 *
 * <p>
 * Example:<br>
 * Setting the background color in the normal state means that normal,
 * highlighted and disabled state will use that color as background color. If
 * you set background color for highlighted state, then the highlighted state
 * will use that color regardless of the background color for the normal state.
 * </p>
 *
 * <p>
 * By default the tool tip text in all states is the same as the tab text in the
 * normal state. For example, if you change the tab text in the highlighted
 * state and want the tooltip to display the same text, you must set the "Tool
 * Tip Text" property {@link TitledTabStateProperties#TOOL_TIP_TEXT} in the
 * highlighted state.
 * </p>
 *
...
public class TitledTabProperties extends PropertyMapContainer {
	...
	public static final PropertyMapProperty NORMAL_PROPERTIES = new PropertyMapProperty(PROPERTIES, "Normal Properties",
			"Normal tab properties.", TitledTabStateProperties.PROPERTIES);
	...
	public static final PropertyMapProperty HIGHLIGHTED_PROPERTIES = new PropertyMapProperty(PROPERTIES,
			"Highlighted Properties", "Highlighted tab properties.", TitledTabStateProperties.PROPERTIES);
	...
	public static final PropertyMapProperty DISABLED_PROPERTIES = new PropertyMapProperty(PROPERTIES,
			"Disabled Properties", "Disabled tab properties.", TitledTabStateProperties.PROPERTIES);
	...
	public static final TitledTabSizePolicyProperty SIZE_POLICY = new TitledTabSizePolicyProperty(PROPERTIES,
			"Size Policy", "Tab size policy", PropertyMapValueHandler.INSTANCE);
	...
	public static final DimensionProviderProperty MINIMUM_SIZE_PROVIDER = new DimensionProviderProperty(PROPERTIES,
			"Minimum Size", "Tab minimum size.", PropertyMapValueHandler.INSTANCE);
	...
	public static final IntegerProperty HIGHLIGHTED_RAISED_AMOUNT = IntegerProperty.createPositive(PROPERTIES,
			"Highlighted Raised", "Number of raised pixels for highlighted tab.", 2, PropertyMapValueHandler.INSTANCE);
	...
	public static final BooleanProperty ENABLED = new BooleanProperty(PROPERTIES, "Enabled",
			"TitledTab enabled or disabled", PropertyMapValueHandler.INSTANCE);
	...
	static {
		...
		DEFAULT_VALUES.getNormalProperties().getMap().createRelativeRef(TitledTabStateProperties.TOOL_TIP_TEXT,
				DEFAULT_VALUES.getNormalProperties().getMap(), TitledTabStateProperties.TEXT);
...

		{
			...
			for (int i = 0; i < refProperties.length; i++) {
				DEFAULT_VALUES.getHighlightedProperties().getMap().createRelativeRef(refProperties[i],
						DEFAULT_VALUES.getNormalProperties().getMap(), refProperties[i]);
...

		{
			...
			for (int i = 0; i < refProperties.length; i++) {
				DEFAULT_VALUES.getHighlightedProperties().getComponentProperties().getMap().createRelativeRef(
						refProperties[i], DEFAULT_VALUES.getNormalProperties().getComponentProperties().getMap(),
						refProperties[i]);
				DEFAULT_VALUES.getDisabledProperties().getComponentProperties().getMap().createRelativeRef(
						refProperties[i], DEFAULT_VALUES.getNormalProperties().getComponentProperties().getMap(),
						refProperties[i]);
			}
		}

		{
			...
			for (int i = 0; i < refProperties.length; i++) {
				DEFAULT_VALUES.getHighlightedProperties().getShapedPanelProperties().getMap().createRelativeRef(
						refProperties[i], DEFAULT_VALUES.getNormalProperties().getShapedPanelProperties().getMap(),
						refProperties[i]);
				DEFAULT_VALUES.getDisabledProperties().getShapedPanelProperties().getMap().createRelativeRef(
						refProperties[i], DEFAULT_VALUES.getNormalProperties().getShapedPanelProperties().getMap(),
						refProperties[i]);
			}
		}
	...
	/**
	 * Constructs a TitledTabProperties object with the give object as property
	 * storage
	 *
	 * @param object
	...
	/**
	 * Constructs a TitledTabProperties object that inherits its properties from
	 * the given TitledTabProperties object
	 *
	 * @param inheritFrom
	...
	/**
	 * <p>
	 * Sets if this TitledTab should show its built-in focus marker when this
	 * tab has focus.
	 * </p>
	 *
	 * <p>
	 * <strong>Note:</strong> Disabling the focus marker is useful when for
	 * example creating a theme that draws its own focus marker.
	 * </p>
	 *
	 * @param value
	...
	/**
	 * <p>
	 * Gets if this TitledTab should show its built-in focus marker when this
	 * tab has focus.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * @param size
	 *            the minimum size dimension provider or null if tab's default
	 *            minimum size should be used instead
	 * @return this TitledTabProperties
	 */
	 ...
	 *
	 * @return the minimum size provider or null if default tab minimum size is
	 *         to be used instead
	 */
	...
	/**
	 * Sets how many pixels higher this TitledTab will be when it is in its
	 * highlighted state compared to its normal and disabled state
	 *
	 * @param amount
	 *            number of pixels
	 * @return this TitledTabProperties
	...
	/**
	 * Gets how many pixels higher this TitledTab will be when it is in its
	 * highlighted state compared to its normal and disabled state
	 *
	 ...
	 * 
	 * <p>
	 * <strong>Note:</strong> Calling {@link TitledTab#setEnabled(boolean)} will
	 * modify this property for the tab.
	 * </p>
	 *
	 * @param value
	 *            true for enabled, otherwise false
	...
	/**
	 * <p>
	 * Sets the hover listener that will be triggered when the tab is hovered by
	 * the mouse.
	 * </p>
	 *
	 * <p>
	 * The hovered titled tab will be the source of the hover event sent to the
	 * hover listener.
	 * </p>
	 *
	 * @param listener
	...
	/**
	 * <p>
	 * Gets the hover listener that will be triggered when the tab is hovered by
	 * the mouse.
	 * </p>
	 *
	 * <p>
		...
		PropertyMapManager.runBatch(new Runnable() {
			public void run() {
				...
				DEFAULT_VALUES.getNormalProperties().setIconTextGap(gap).setTextTitleComponentGap(gap)
						.setIconVisible(true).setTextVisible(true).setTitleComponentVisible(true)
						.getComponentProperties().setFont(TabbedUIDefaults.getFont())
	...
	private static void updateFunctionalProperties() {
		DEFAULT_VALUES.setEnabled(true).setFocusable(true).setFocusMarkerEnabled(true)
				.setSizePolicy(TitledTabSizePolicy.EQUAL_SIZE).setBorderSizePolicy(TitledTabBorderSizePolicy.EQUAL_SIZE)
				.setHighlightedRaised(2);

		DEFAULT_VALUES.getNormalProperties().setHorizontalAlignment(Alignment.LEFT)
				.setVerticalAlignment(Alignment.CENTER).setIconTextRelativeAlignment(Alignment.LEFT)
				.setTitleComponentTextRelativeAlignment(Alignment.RIGHT).setDirection(Direction.RIGHT);
	}
}
