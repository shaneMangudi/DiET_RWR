...
/**
 * TitledTabBorderSizePolicy defines how the insets for the titled tab should be
 * calculated based on the borders for the different tab states. If the states
 * have borders with different insets titled tab can use the same insets for
 * each state based on the maximum insets (top, left, bottom, right) for each
...
public final class TitledTabBorderSizePolicy extends Enum {
	 ...
	 * each state based on the maximum insets (top, left, bottom, right) for
	 * each state. The compensated insets will be added on the inside of the
	 * borders.
	 */
