 ...
 *
 * <p>
 * A tab can hold a content component. The content component will then be shown
 * in the content area of the TabbedPanel that the tab is a member of when the
 * tab is selected. If the tab doesn't have a content component, then the
 * TabbedPanel will not show any content in the content area, i.e. it will be
 * empty.
 * </p>
 *
 * <p>
 * The tab is basically a JPanel with a BorderLayout. The layout manager can be
 * changed using setLayout. Components and borders can be added and removed from
 * the tab. The tab can also be subclassed to create other types of tabs, see
 * {@link TitledTab}. <strong>In most cases {@link TitledTab} is the preferred
 * tab type to use because TitledTab adds support for a text, icon, looks
 * etc.</strong>
 * </p>
 *
 * <p>
 * The tab component will be shown in the tab area of a TabbedPanel after the
 * tab has become a member of that TabbedPanel by either adding or inserting it.
 * A tab can only be a member of one TabbedPanel at the same time.
 * </p>
 *
 * <p>
 * A tab can have different states when it is a member of a TabbedPanel:
 * <ul>
 * <li>Normal: This means that the tab is shown (and not selected) in the
 * TabbedPanel. The content component is not shown until the user selects the
 * tab.
 * <li>Highlighted: This means that for some reason the tab should be
 * highlighted in the TabbedPanel. Highlighted could mean that the user pressed
 * the tab with the mouse and has not yet released the mouse, i.e. it has not
 * been selected yet.
 * <li>Selected: This means that the tab is selected in the TabbedPanel. The
 * TabbedPanel will then show the Tab's content component (if any). A selected
 * tab will also be be highlighted before it is selected.
 * <li>Enabled: This means that the tab is enabled and can be selected,
 * highlighted dragged, moved etc.
...
public class Tab extends JPanel {
			...
			if (tabbedPanel != null) {
				...
				while (true) {
					index = (index + tabbedPanel.getTabCount()
							+ (e.getKeyCode() == incKey ? 1 : e.getKeyCode() == decKey ? -1 : 0))
	 ...
	 *            content component for this tab or null for no content
	 *            component.
	 * @see #setEventComponent
	 */
	 ...
	 *
	 * @return the content component for this tab or null if this Tab doesn't
	 ...
	 *
	 * @return the TabbedPanel or null if this tab is not a member of any
	 *         TabbedPanel
	 */
	 ...
	 *
	 * <p>
	 * If the tab is disabled, then the tab will not signal any events until it
	 * is enabled again.
	 * </p>
	 *
	 * @param enabled
	...
	/**
	 * <p>
	 * Selects this tab. A tab can only have the selected state if it is a
	 * member of a TabbedPanel.
	 * </p>
	 *
	 * <p>
	 * Setting selected to true means that this tab will be the selected tab in
	 * the TabbedPanel it is a member of. If this tab is the selected tab in the
	 * TabbedPanel then setting selected to false means there will be no
	 * selected tab in the TabbedPanel until another tab is selected.
	 * </p>
	 *
	 * @param selected
	...
	/**
	 * Returns if this tab is selected in the TabbedPanel that it is a member
	 * of.
	 *
	...
	/**
	 * Highlights this tab. This tab will be the highlighted tab in the
	 * TabbedPanel that it is member of.
	 *
	 * @param highlighted
	...
	/**
	 * Returns if this tab is highlighted in the TabbedPanel that it is a member
	 * of.
	 *
	 * @return true if highlighted, false if not highlighted or this tab is not
	 *         member of a TabbedPanel
	 */
	...
	/**
	 * <p>
	 * Sets the event component. An event component is a component in the tab
	 * that is used for internal listening to mouse events on the tab.
	 * </p>
	 *
	 * <p>
	 ...
	 *            a component in this tab that should be used for mouse event
	 *            listening
	 */
	...
	/**
	 * <p>
	 * Sets a list of event components. An event component is a component in the
	 ...
	 *            a list of components in this tab that should be used for mouse
	 *            event listening
	 */
	 ...
	 *
	 * @return focusable component or null if this tab doesn't have any
	 *         focusable component
	 */
	 ...
	 *            a component in this tab or null if no component should be
	 *            focusable
	 */
	 ...
	 *
	 * <p>
	 * This returns the shape of the tab. This can be be used by for example
	 * content borders in the tabbed panel so they can skip a gap where the tab
	 * intersects the tabbed panel content area.
	 * </p>
	 *
	 * @return the tab {@link Shape}, null if the tab has the normal component
	 *         rectangle shape
	 * @since ITP 1.2.0
	 */
	...
	/**
	 * Called by the tabbed panel when the tab becomes a member or is no longer
	 * a member of the tabbed panel
	 *
	 * @param tabbedPanel
	 *            tabbed panel that this tab is a member of or null if this tab
	 *            is no longer a member o a tabbed panel
	 */
