...
public class PropertyMapImpl implements PropertyMap {
			...
			for (ConstMapIterator iterator = changes.constIterator(); iterator.atEntry(); iterator.next()) {
				...
				if (propertyGroup.hasProperty(property)) {
					...
					if (currentValue == null || currentValue.getParent() != null) {
						...
						PropertyValue newValue = superValue == null ? null
								: superValue.getSubValue(PropertyMapImpl.this);
	...
	private boolean hasListener() {
		return hasTreeListener() || (listeners != null && listeners.size() > 0)
	...
	private PropertyMapRef getRelativePathTo(PropertyMapImpl propertyObject) {
		...
		return ref == null ? parent == null ? null
				: new CompositeMapRef(ParentMapRef.INSTANCE, parent.getRelativePathTo(propertyObject)) : ref;
	}

	public Object createRelativeRef(Property fromProperty, PropertyMap toObject, Property toProperty) {
		PropertyValue value = setValue(fromProperty, new PropertyRefValue(this, fromProperty,
				getRelativePathTo((PropertyMapImpl) toObject), toProperty, null));
	...
	public void addSuperMap(PropertyMap superMap) {
		...
		/*
		 * if (!propertyObjectImpl.propertyGroup.isA(propertyGroup)) throw new
		 * RuntimeException("Property group '" +
		 * propertyObjectImpl.propertyGroup + "' can't be assigned to group '" +
		 * propertyGroup + "'!");
		 */
		PropertyMapManager.getInstance().beginBatch();
	...
	private PropertyValue getValue(PropertyPath propertyPath) {
		return propertyPath.getTail() == null ? getValue(propertyPath.getProperty())
	...
	private PropertyValue internalSetValue(Property property, PropertyValue value) {
		PropertyValue oldValue = (PropertyValue) (value == null ? values.remove(property)
	...
	protected void firePropertyTreeValuesChanged(Map changes) {
		if (treeListeners != null) {
			PropertyMapTreeListener[] l = (PropertyMapTreeListener[]) treeListeners
					.toArray(new PropertyMapTreeListener[treeListeners.size()]);
	...
	void firePropertyValuesChanged(Map changes) {
		if (listeners != null) {
			PropertyMapListener[] l = (PropertyMapListener[]) listeners
					.toArray(new PropertyMapListener[listeners.size()]);
		...
		if (propertyChangeListeners != null) {
			for (Iterator iterator = changes.entrySet().iterator(); iterator.hasNext();) {
				...
				if (list != null) {
					...
					PropertyChangeListener[] l = (PropertyChangeListener[]) list
							.toArray(new PropertyChangeListener[list.size()]);
	...
	public void dump(Printer printer, Set printed) {
		...
		for (int i = 0; i < superMaps.size(); i++) {
			/*
			 * if (printed.contains(superMaps.get(i))) continue;
			 */
	...
	public void dumpSuperMaps(Printer printer) {
		...
		for (int i = 0; i < superMaps.size(); i++) {
			// if (superMap.getMap(i) != ((PropertyMapImpl)
			// superMaps.get(i)).map)
	...
	private boolean doValuesEqual(PropertyMapImpl propertyObject, boolean recursive) {
		for (ConstMapIterator iterator = map.constIterator(); iterator.atEntry(); iterator.next()) {
			...
			if (!Utils.equals(((PropertyValue) iterator.getValue()).get(this),
					propertyObject.getValue(property).get(this)))
				return false;
		}
		...
		if (recursive) {
			for (ConstMapIterator iterator = childMaps.constIterator(); iterator.atEntry(); iterator.next()) {
				((PropertyMapImpl) iterator.getValue()).doCopy(
						(PropertyMapImpl) map.getChildMap((PropertyMapProperty) iterator.getKey()), copySuperMaps,
						recursive, false);
			}
		}
	}
