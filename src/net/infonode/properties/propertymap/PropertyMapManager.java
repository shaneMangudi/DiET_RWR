...
/**
 * Utility class for performing multiple modifications to {@link PropertyMap}'s
 * and merging change notifications to optimize performance.
 *
...
public class PropertyMapManager {
	...
	void addMapChanges(PropertyMapImpl propertyMap, ConstMap mapChanges) {
		...
		for (ConstMapIterator iterator = mapChanges.constIterator(); iterator.atEntry(); iterator.next()) {
			...
			Object newValue = vc.getNewValue() == null ? null
					: ((PropertyValue) vc.getNewValue()).getWithDefault(propertyMap);
			Object value = map.get(key);
			Object oldValue = value == null
					? vc.getOldValue() == null ? null : ((PropertyValue) vc.getOldValue()).getWithDefault(propertyMap)
	...
	/**
	 * Executes a method inside a {@link #beginBatch()} - {@link #endBatch()}
	 * pair. See {@link #beginBatch()} for more information. It's safe to call
	 * other batch methods from inside {@link Runnable#run}.
	 *
	 * @param runnable
	...
	/**
	 * Begins a batch operation. This stores and merges all change notifications
	 * occuring in all property maps until {@link #endBatch} is called. Each
	 * call to this method MUST be followed by a call to {@link #endBatch}. This
	 * method can be called an unlimited number of times without calling
	 * {@link #endBatch} in between, but each call must have a corresponding
	 * call to {@link #endBatch}. Only when exiting from the outermost
	 * {@link #endBatch()} the changes be propagated to the listeners.
	 */
	...
	private void addTreeChanges(PropertyMapImpl map, PropertyMapImpl modifiedMap, HashMap changes,
			HashMap treeChanges) {
