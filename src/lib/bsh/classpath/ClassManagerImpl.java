...
import java.io.*;
import lib.bsh.classpath.BshClassPath.ClassSource;
import lib.bsh.classpath.BshClassPath.JarClassSource;
import lib.bsh.classpath.BshClassPath.GeneratedClassSource;
import lib.bsh.BshClassManager;
import lib.bsh.ClassPathException;
import lib.bsh.Interpreter; // for debug()
import lib.bsh.UtilEvalError;
...
public class ClassManagerImpl extends BshClassManager {
	...
	public Class classForName(String name) {
		...
		if ((c == null) && name.startsWith(BSH_PACKAGE)) {
			final ClassLoader myClassLoader = Interpreter.class.getClassLoader(); // is
																					// null
																					// if
																					// located
																					// in
		...
		if (c == null)
			try {
				...
				// fall through
				/*
				 * I disagree with letting this fall through -fschmidt } catch (
				 * NoClassDefFoundError e ) { // fall through
				 */
			}
	...
	*/
	...
	/*
	 * Impl Notes: We add the bytecode source and the "reload" the class, which
	 * causes the BshClassLoader to be initialized and create a
	 * DiscreteFilesClassLoader for the bytecode.
	 * 
	...
	public Class defineClass(String name, byte[] code) {
		...
		try {
			...
			throw new lib.bsh.InterpreterError("defineClass: " + e);
		}
