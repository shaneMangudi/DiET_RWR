...
public class BshClassLoader extends URLClassLoader {
	...
	public Class loadClass(String name, boolean resolve) throws ClassNotFoundException {
		...
		/*
		 * Check first for classes loaded through this loader. The VM will not
		 * allow a class to be loaded twice.
		 */
		...
		if (name.startsWith(ClassManagerImpl.BSH_PACKAGE)) {
			try {
				return lib.bsh.Interpreter.class.getClassLoader().loadClass(name);
	 ...
	 * 
	 * c = findLoadedClass(name); if null try if parent not null c =
	 * parent.loadClass(name, false); else c = findBootstrapClass(name); catch
	 * ClassNotFoundException c = findClass(name);
	 */
