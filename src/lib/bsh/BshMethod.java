...
public class BshMethod implements java.io.Serializable {
	...
	BshMethod(Method method, Object object) {
		this(method.getName(), method.getReturnType(), null/* paramNames */, method.getParameterTypes(),
	...
	Object invoke(Object[] argValues, Interpreter interpreter, CallStack callstack, SimpleNode callerInfo,
		...
		if (javaMethod != null)
			try {
				...
				throw new TargetError("Exception invoking imported object method.", e2, callerInfo, callstack,
						true/* isNative */ );
			}
		...
		for (int i = 0; i < numArgs; i++) {
			...
			if (paramTypes[i] != null) {
				try {
					...
					throw new EvalError("Invalid argument: " + "`" + paramNames[i] + "'" + " for method: " + name
			...
			else // untyped param
			{
				...
				if (argValues[i] == Primitive.VOID)
					throw new EvalError(
							"Undefined variable or class name, parameter: " + paramNames[i] + " to method: " + name,
