...
public class ClassWriter implements ClassVisitor {
	...
	static {
		...
		String s = "AAAAAAAAAAAAAAAABCKLLDDDDDEEEEEEEEEEEEEEEEEEEEAAAAAAAADDDDDEEEEEEEEE"
				...
				+ "AAAAAAAAAAAAAAAAAIIIIIIIIIIIIIIIIDNOAAAAAAGGGGGGGHAFBFAAFFAAQPIIJJII" + "IIIIIIIIIIIIIIII";
		 ...
		 * 
		 * // SBYTE_INSN instructions b[Constants.NEWARRAY] = SBYTE_INSN;
		 ...
		 * b[i] = IMPLVAR_INSN; }
		 * 
		 * // TYPE_INSN instructions b[Constants.NEW] = TYPE_INSN;
		 * b[Constants.ANEWARRAY] = TYPE_INSN; b[Constants.CHECKCAST] =
		 ...
		 * 
		 * // LABEL(W)_INSN instructions for (i = Constants.IFEQ; i <=
		 * Constants.JSR; ++i) { b[i] = LABEL_INSN; } b[Constants.IFNULL] =
		 * LABEL_INSN; b[Constants.IFNONNULL] = LABEL_INSN; b[200] =
		 * LABELW_INSN; // GOTO_W b[201] = LABELW_INSN; // JSR_W // temporary
		 * opcodes used internally by ASM - see Label and CodeWriter for (i =
		 * 202; i < 220; ++i) { b[i] = LABEL_INSN; }
		 * 
		 * // LDC(_W) instructions b[Constants.LDC] = LDC_INSN; b[19] =
		 * LDCW_INSN; // LDC_W b[20] = LDCW_INSN; // LDC2_W
		 * 
		 * // special instructions b[Constants.IINC] = IINC_INSN;
		 * b[Constants.TABLESWITCH] = TABL_INSN; b[Constants.LOOKUPSWITCH] =
		 * LOOK_INSN; b[Constants.MULTIANEWARRAY] = MANA_INSN; b[196] =
		 * WIDE_INSN; // WIDE
		 * 
	 ...
	 *            flag is <tt>true</tt>, then the arguments of the
