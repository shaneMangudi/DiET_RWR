...
public class ClientInterfaceEventStringPrettifier {
		...
		try {
			...
			for (int i = 0; i < clientinterfaceevents.size(); i++) {
				...
				if (cie.getType().equalsIgnoreCase("tefr")) {
					...
					int length = (int) cie.getValue("length");
					if (length != 0) {
						sValue = sValue + " " + superscript("" + timesinceprevious + "") + '\u02FF'
								+ (String) cie.getValue("textbeingdeleted");
					}
					...
					previousDisplayTime = cie.getClientTimeOfDisplay();
					sValue = sValue + " " + superscript("" + timesinceprevious + "")
							+ DefaultConversationController.config.recordeddata_startOfIsTypingByOther;
					...
					previousDisplayTime = cie.getClientTimeOfDisplay();
					sValue = sValue + " " + superscript("" + timesinceprevious + "")
							+ DefaultConversationController.config.recordeddata_endOfIsTypingByOther;
					...
					previousDisplayTime = cie.getClientTimeOfDisplay();
					sValue = sValue + " " + superscript("" + timesinceprevious + "")
							+ DefaultConversationController.config.recordeddata_mazegameOpenGates;
					...
					previousDisplayTime = cie.getClientTimeOfDisplay();
					sValue = sValue + " " + superscript("" + timesinceprevious + "")
							+ DefaultConversationController.config.recordeddata_mazegameCloseGates;
				}
		...
		try {
			...
			for (int i = 0; i < clientinterfaceevents.size(); i++) {
				...
				if (cie.getType().equalsIgnoreCase("tefr")) {
					...
					if (length != 0) {
						sValue = sValue + createLogBlankspace(timesinceprevious) + '\u02FF'
								+ ((String) cie.getValue("textbeingdeleted"));
					}
				} else if (cie.getType().equalsIgnoreCase("tefi")) {
					long timesinceprevious = 0;
	...
	public static String convertToDeletedString(String s) {
		...
		if (2 < 5)
			return Charset.forName("UTF-8").encode("a" + "\u0300").toString();
		String newstring = "";
	...
	// U+02FF Ë¿ cb bf MODIFIER LETTER LOW LEFT ARROW
	// U+0354 Í” cd 94 COMBINING LEFT ARROWHEAD BELOW
}
