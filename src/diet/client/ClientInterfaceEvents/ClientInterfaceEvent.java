...
public class ClientInterfaceEvent implements Serializable {
	...
	public ClientInterfaceEvent(String participantID, String participantUsername, String type, long clientTimeOfDisplay,
			AttribVal... args) {
		this.type = type;
		this.arguments = args;
	...
	public AttribVal getAttribVal(String attribname) {
		for (int i = 0; i < arguments.length; i++) {
			if (arguments[i].id.equalsIgnoreCase(attribname))
				return arguments[i];
		}
		return null;
	}
	...
	public boolean isStartOfIsTyping() {
		// System.err.println("TYPEIS:
		// "+ClientInterfaceEventTracker.changestatuslabel);
		// System.exit(03);
	...
	public boolean isEndOfIsTyping() {
		if (this.type.equalsIgnoreCase(ClientInterfaceEventTracker.changestatuslabel)) {
			AttribVal av = this.getAttribVal("serverid");
			if (((String) av.value).endsWith("nt"))
				return true;
			return false;
		}
		return false;
