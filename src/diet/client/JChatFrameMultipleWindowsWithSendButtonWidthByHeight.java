...
public class JChatFrameMultipleWindowsWithSendButtonWidthByHeight extends JChatFrame {
	...
	// JTextPane jTextEntryPane = new
	// JChatFrameMultipleWindowsWithSendButtonWidthByHeightTextEntryPane();
	// //new JTextPane();
	...
	JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument jcfd = new JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument(
			this);
	StyledDocumentStyleSettings styles;
	MutableAttributeSet selfSTYLE;

	public JChatFrameMultipleWindowsWithSendButtonWidthByHeight(ClientEventHandler clevh, int numberOfWindows,
			int mainWindowWidth, int mainWindowHeight, boolean isVertical, boolean hasStatusWindow, int windowOfOwnText,
			int textEntryWidth, int textEntryHeight, int maxCharlength, StyledDocumentStyleSettings styles) {
		super(clevh);

		selfSTYLE = styles.masSELF;
		changeInterfaceProperties("initialsetup", ClientInterfaceEventTracker.changeTextStyleOfTextFormulationWindow,
				selfSTYLE, null);
		...
		try {
			...
			jccw = new JCollectionOfChatWindows(numberOfWindows, mainWindowWidth, mainWindowHeight, isVertical,
					hasStatusWindow, windowOfOwnText, styles);
			cards.add(jccw, "jccw");
			...
			// jccw.setPreferredSize(new
			// Dimension(mainWindowWidth,mainWindowHeight));
		...
		// setStatusBarText_EnableDisable("<html><b><u>T</u>wo</b><br>lines</html>",
		// null, false);
	...
	public void testTForDebugTrigger(String s) {
		...
		if (s.startsWith(DefaultConversationController.config.debug_fakeTypingByClientSTARTTrigger))
			dr.startFakeKeyboardActivity();
		else if (s.startsWith(DefaultConversationController.config.debug_fakeTypingByClientSTOPTrigger))
			dr.stopFakeKeyboardActivity();
		// System.exit(-234);
	}
	...
	public void changeInterfaceProperties(final String uniqueIDGeneratedByServer, final int newInterfaceproperties,
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (newInterfaceproperties == ClientInterfaceEventTracker.clearTextEntryField) {
					...
					getClientEventHandler().reportInterfaceEvent(ClientInterfaceEventTracker.changeborder,
							timeOfDisplay, av1, av2, avServerId);
					...
					getClientEventHandler().reportInterfaceEvent(ClientInterfaceEventTracker.changestatuslabel,
							timeOfDisplay, av1, avServerId);
					...
					AttribVal av1 = new AttribVal("text", (String) value1);
					getClientEventHandler().reportInterfaceEvent(
							ClientInterfaceEventTracker.changestatuslabel_disabletextentry, timeOfDisplay, av1,
							avServerId);
					...
					AttribVal av1 = new AttribVal("text", (String) value1);
					getClientEventHandler().reportInterfaceEvent(
							ClientInterfaceEventTracker.changestatuslabel_enabletextentry, timeOfDisplay, av1,
							avServerId);

				} else if (newInterfaceproperties == ClientInterfaceEventTracker.enableDeletes) {
					JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument jcfd = (JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument) jTextEntryPane
							.getDocument();
	...
	private void setStatusBarText_EnableDisable(String newText, MutableAttributeSet mas, boolean setWindowEnabled) {
		...
		if (setWindowEnabled)
			this.jTextEntryPane.requestFocus();
	...
	public void appendWithCaretCheck(String idTOCONFIRMTOSERVER, String prefix, boolean showPrefix, String text,
			int windowNumber, Object attributeset) {
		...
		if (attributeset instanceof String) {
			...
			if (attributesetAsString.equalsIgnoreCase("SELF")) {
				SwingUtilities.invokeLater(new DoAppendTextWithCaretCheck(this, jtp, jsp, windowNumber,
						idTOCONFIRMTOSERVER, prefix, showPrefix, text, selfSTYLE, true));
			}
		} else {
			SwingUtilities.invokeLater(new DoAppendTextWithCaretCheck(this, jtp, jsp, windowNumber, idTOCONFIRMTOSERVER,
					prefix, showPrefix, text, attributeset, false));
		}
	...
	public void clearTextEntryField(boolean changeIsInitiatedByClient) {
		...
		// DoSetTextEntryField(jTextEntryPane,s));

		if (changeIsInitiatedByClient) {
			final JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument jcfdc = (JChatFrameMultipleWindowsWithSendButtonWidthByHeightDocument) this.jTextEntryPane
					.getDocument();
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					try {
	...
	class JCollectionOfChatWindows extends JPanel {
		...
		public JCollectionOfChatWindows(int numberOfWindows, int mainWindowWidth, int mainWindowHeight,
				boolean isVertical, boolean hasStatusWindow, int windowOfParticipant,
				StyledDocumentStyleSettings styles) {
			super();
			if (isVertical) {
			...
			for (int i = 0; i < numberOfWindows; i++) {
				...
				// scrollPane.setMaximumSize(new
				// Dimension(mainWindowWidth,mainWindowHeight));
				// scrollPane.getViewport().setMaximumSize(new
				// Dimension(mainWindowWidth,mainWindowHeight));
	...
	public class DoAppendTextWithCaretCheck implements Runnable {
		...
		public DoAppendTextWithCaretCheck(JChatFrameMultipleWindowsWithSendButtonWidthByHeight jcfmwwsbw, JTextPane jtp,
				JScrollPane jsp, int windowNumber, String idTOCONFIRM, String prefix, boolean showPrefix, String text,
				Object attributeset, boolean bySelf) {
			this.jtp = jtp;
			this.jsp = jsp;
			this.text = text;
		...
		private int countMatches(String input, String searchString) {
			int lastIndex = 0;
			int count = 0;

			while (lastIndex != -1) {
				lastIndex = input.indexOf(searchString, lastIndex);
			...
			try {
				if (attrset instanceof String) {
					...
					jtp.getStyledDocument().insertString(jtp.getDocument().getLength(), text,
							jtp.getStyledDocument().getStyle((String) attrset));
					textDisplayed = text;
				} else if (attrset instanceof MutableAttributeSet) {
					if (DefaultConversationController.config.client_turnDisplayLimit > 0) {
						String allText = jtp.getDocument().getText(0, jtp.getDocument().getLength());
						int matchCount = countMatches(allText, "\n");
						if (DefaultConversationController.config.client_turnDisplayLimit <= matchCount) {
							jtp.getStyledDocument().remove(0, allText.indexOf('\n') + 1);
						}
					}
					...
					jtp.getStyledDocument().insertString(
							jtp.getDocument().getLength(), prefix + text
									+ " THERE IS AN ERROR IN THE SCRIPT SPECIFYING THE FONT/STYLE OF THE TEXT" + "\n",
							null);
					textDisplayed = prefix + text
							+ " THERE IS AN ERROR IN THE SCRIPT SPECIFYING THE FONT/STYLE OF THE TEXT";
				}
				...
				if (isbyself) {
					jcfmwwsbw.getClientEventHandler().reportInterfaceEvent(
							diet.client.ClientInterfaceEvents.ClientInterfaceEventTracker.chatwindow_appendbyself_clearingtextentry,
							timeDisplayed, av1);
				} else {
					jcfmwwsbw.getClientEventHandler().reportInterfaceEvent(
							diet.client.ClientInterfaceEvents.ClientInterfaceEventTracker.chatwindow_appendbyotherparticipant,
							timeDisplayed, av1, av2, av3);
				}
	...
	class InputDocumentListener implements DocumentListener {
		...
		public void updateInsert(DocumentEvent e) {
			...
			try {
				...
				int insrtIndex = (documentlength - length) - offset; // The
																		// length
																		// of
																		// document
																		// is
																		...
																		// on
																		// insertion
				// System.err.println("DocLength: "+documentlength+" "+".
				// Offset: "+offset+". Length: "+length+". Text:
				// "+jTextAreaSource.getText().substring(offset,offset+length)+".
				// InsertIndex: "+insrtIndex);

				try {
					getClientEventHandler().textEntryDocumentHasChangedInsert(
							jTextPaneSource.getText().substring(offset, offset + length), insrtIndex, length,
							jTextPaneSource.getText());
				} catch (Exception eee) {
					getClientEventHandler().textEntryDocumentHasChangedInsert("", insrtIndex, length,
							jTextPaneSource.getText());
					getClientEventHandler().getCts().sendErrorMessage("ERROR INSIDE CHAT FRAME WITH INDICES");
				}
			} catch (Error e2) {
				System.err.println("OFFSET ERROR " + offset + " , " + jTextPaneSource.getText().length() + " "
						+ jTextPaneSource.getText() + " " + e2.getStackTrace());
		...
		public void updateRemove(DocumentEvent e) {
			...
			int documentlength = jTextEntryPane.getDocument().getLength();
			int insrtIndex = (documentlength + length) - offset; // The length
																	// of
																	// document
																	// is
																	// decreased
																	...
																	// on
																	// deletion
			// System.err.println("REMOVE: DocLength: "+documentlength+" "+".
			// Offset: "+offset+". Length: "+length+". InsertIndex:
			// "+insrtIndex);
			...
			try {
				...
				getClientEventHandler().getCts()
						.sendErrorMessage(ee.getMessage() + " ERROR INSIDE THE JCHATFRAME (REMOVE1)");
			}
			...
			try {
				...
				getClientEventHandler().getCts()
						.sendErrorMessage(e2.getMessage() + " ERROR INSIDE THE JCHATFRAME (REMOVE1)");
			}
