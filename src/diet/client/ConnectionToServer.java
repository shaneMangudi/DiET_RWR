 ...
 * messages between server and client, unpacking messages from the server and
 * calling associated methods in {@link ClientEventHandler}. Events from the
...
public class ConnectionToServer extends Thread {
    ...
    private ReferenceWithoutReferentsTaskJFrame referenceWithoutReferentsTaskJFrame = null;
    ...
    /**
     * Attempts to set up network connection with server. On establishing a
     * connection it prompts the participant for an email address and also a
     * username.
     * <p>
            ...
            if (m instanceof MessagePopupClientLogonEmailRequest && this.forceParticipantID) {
                if (this.forcedParticipantID == null || this.forcedParticipantID.length() == 0) {
                    JOptionPane.showMessageDialog(null,
                            "For some reason the program can't\n" + "find your ID so we can't log you in\n"
                                    + "Please contact the experimenter directly\n" + "If you wish to take part\n");
                    System.exit(-1);
                }
                emailrequest = this.forcedParticipantID;
                out.writeObject(new MessageClientLogon(emailrequest, "", ServerClientConsistency.getVersionID(), ina,
                        inPort, local_ina, local_port, socketAddress));
                ...
                if (DefaultConversationController.config.login_autologin) {
                    emailrequest = DefaultConversationController.autologinParticipantIDGenerator.getNext();
                    ...
                    emailrequest = JOptionPane.showInputDialog(
                            m2.getPrompt() + " (At least " + m2.getMinLength() + " characters)", m2.getOptionText());
                }
                ...
                if (DefaultConversationController.config.login_autologin) {
                    ...
                    out.writeObject(new MessageClientLogon(emailrequest, usernamerequest,
                            ServerClientConsistency.getVersionID(), ina, inPort, local_ina, local_port, socketAddress));
                } else if (m2.getAllowAlternatives()) {
                    System.out.println("ClientSetup4b4B1");
                    usernamerequest = JOptionPane.showInputDialog(m2.getPrompt(), m2.getOptionText());
                    ...
                    int n = JOptionPane.showOptionDialog(null,
                            "It looks as if you were logged on before\n\n" + "USERNAME: " + m2.getOptionText() + "\n"
                                    + "\n\n" + "Do you want to log back in again and continue?\n"
                                    + "(Any old windows from previous login will stop working\n",
                            "Previous login detected", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null,
                            options, options[0]); // default button title
                    if (n == 1)
                        return false;
                    else {
                        System.out.println("ClientSetup4b4B21");
                        out.writeObject(new MessageClientLogon(emailrequest, usernamerequest,
                                ServerClientConsistency.getVersionID(), ina, inPort, local_ina, local_port,
                                socketAddress));
                    }
                ...
                // This IF section should be moved to the
                // initialsetupofchattool() method
    ...
    /**
     * Determines which chat tool window is to be used and sets the chat window
     * accordingly
     *
     * @param m
     */
    public void initialsetupOfChatTool(final Message m) {
        ...
        if (m instanceof MessageClientSetupParametersWithSendButtonAndTextEntryWidthByHeight) {
            ...
            try {
                ...
                // jcfsw = new
                // JChatFrameMultipleWindowsWithSendButtonWidthByHeight(cEventHandler,mcspwbyh.getNoOfWindows(),mcspwbyh.getMainWindowWidth(),500,mcspwbyh.getAlignmentIsVertical(),mcspwbyh.getParticipantHasStatusWindow(),mcspwbyh.getParticipantsTextWindow(),mcspwbyh.getTextEntryWidth(),mcspwbyh.getTextEntryHeight(),
                // mcspwbyh.getWtsdsd());
    ...
    public void eat3000Messages() {
        for (int j = 0; j < 250; j++) {
            try {
                System.err.println("READING OBJECTS");
                System.err.flush();
                Object o = in.readObject();
    ...
    /**
     * This is the main method of the client which constantly checks for
     * incoming messages from the server. On receiving a message, it extracts
     * the information and calls the appropriate methods in
     * {@link ClientEventHandler} When changing this method, ensure that it
     * retains the capacity to capture exceptions and send them to the server
     *
     * @throws java.lang.Exception
     */
        ...
        while (gameInProgress) {
            try {
                ...
                if (m instanceof MessageChatTextToClient) {
                    ...
                    cEventHandler.appendWithCaretCheck(mct.getMessageID(), mct.getUsername(), mct.isPrefixUsername(),
                            mct.getText(), mct.getWindowNo(), mct.getStylenumber());

                    // System.out.println("Appending:
                    // "+mct.getWindowNo()+mct.getText());
                } else if (m instanceof MessageChangeClientInterfaceProperties) {
                    // System.out.println("Receiving
                    // MessageStatusLabelDisplayAndBlocked");
                    MessageChangeClientInterfaceProperties mccip = (MessageChangeClientInterfaceProperties) m;
                    cEventHandler.changeInterfaceProperties(mccip.getUniqueIDGeneratedByServer(),
                            mccip.getInterfaceproperties(), mccip.getValue(), mccip.getValue2());
                    // System.exit(-444442);
                    ...
                    if (m instanceof MessageNewMazeGame && cteh == null) {
                        ...
                        jfgridimagestimuli.changeSelection(mgsstc.getServerID(), mgsstc.getSelections(),
                                mgsstc.getInnerMostColour(), mgsstc.getInnerColour());
                        ...
                        jfgridtextstimuli.changeAllSelections(mgistc.getServerID(), mgistc.getInnerMostSColors(),
                                mgistc.getInnerColors());
                    } else {
                    ...
                    try {
                        ...
                        this.wiad.displayNEWWebpage(mdw.getId(), mdw.getHeader(), tx2, mdw.getWidth(), mdw.getHeight(),
                                mdw.isVScrollBar(), mdw.isJProgressBar(), mdw.isForceCourier());
                        // this.wiad.changeJProgressBar(mdw.getId(), "THE TXT
                        // IS:" +tx2+"----ANDALSO-"+tx+"----", Color.red, 59);
                    } catch (Exception e) {
                        e.printStackTrace();
                        // this.sendErrorMessage("ERRORDISPLAYINGNEWWEBPAGE", 0,
                        // false, new Vector(),null);
                    ...
                    try {
                        ...
                        String tx2 = tx.replaceAll("%%SERVERIPADDRESS%%", this.serverName);
                        System.err.println("TRYING TO DISPLAY-" + tx + "-----------" + tx2);
                        // this.wiad.changeJProgressBar(mdcw.getId(), "THE TXT
                        // IS:" +tx2+"----ANDALSO-"+tx+"----", Color.black, 59);

                        this.wiad.changeWebpage(mdcw.getId(), tx2, mdcw.getPrepend(), mdcw.getAppend(),
                                mdcw.getLengthOfTime());
                    ...
                    try {
                        ...
                        if (id.equalsIgnoreCase("CHATFRAME")) {
                            this.cEventHandler.getChatFrame().changeJProgressBar(mdcpb.gettext(), mdcpb.getColor(),
                                    mdcpb.getValue());
                        } else {
                            this.wiad.changeJProgressBar(mdcpb.getId(), mdcpb.gettext(), mdcpb.getColor(),
                                    mdcpb.getValue());
                        }
                    ...
                    if (jfgridimagestimuli == null) {
                        jfgridimagestimuli = new JFrameGridImagesStimuli(this, mgsstc.getROWS(), mgsstc.getCOLUMNS(),
                                mgsstc.getWidthHeight());
                    }
                    if (mgsstc.getSerializableImages() != null) {
                        if (mgsstc.getSerializableImages().size() > 0)
                            jfgridimagestimuli.setStimuliSet(mgsstc.getServerID(), mgsstc.getSerializableImages());
                    }
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        public void run() {
                            jfgridtextstimuli = new JFrameGridTextStimuli(ctss, mgtsi.getServerID(), mgtsi.getROWS(),
                                    mgtsi.getCOLUMNS(), mgtsi.getTexts(), mgtsi.getInnermostC(), mgtsi.getInnerC(),
                                    mgtsi.getWidth(), mgtsi.getHeight());
                    ...
                    if (jfsubliminalstimuli != null) {
                        jfsubliminalstimuli.displayStimulus(mssci.getFixation1time(), mssci.getStimulus1time(),
                                mssci.getBlankscreen1time(), mssci.getFixation2time(), mssci.getStimulus2time(),
                                mssci.getBlankscreen1time(), mssci.getStimulus1ID(), mssci.getStimulus2ID());
                    ...
                    if (jfsubliminalstimuli != null) {
                        jfsubliminalstimuli.displayText(mssdt.getText(), mssdt.getColour(),
                                mssdt.getNameOfBackgroundPanel(), mssdt.getPositionX(), mssdt.getPositionY(),
                                mssdt.getLengthOfTime());
                    }
                    ...
                    if (this.jfssi == null) {
                        ...
                        SwingUtilities.invokeLater(new Runnable() {
                            public void run() {
                                jfssi = new JFrameStimulusSingleImage(ctsForStatic, msidj.width, msidj.height);
                    ...
                    if (jfssi != null)
                        jfssi.displayImage(name, duration);
                    ...
                    if (blockIncomingMessages) {
                        /*
                         * System.out.println("ClientSetup4b4B22"); MessagePopup
						 * mp = (MessagePopup)m; String[] options =
						 * mp.getOptions(); String question = mp.getQuestion();
						 * String title = mp.getTitle(); long timeOfDisplay =
						 * new Date().getTime(); int n =
						 * JOptionPane.showOptionDialog(parentComponent,
						 * question, title, JOptionPane.YES_NO_CANCEL_OPTION,
						 * JOptionPane.QUESTION_MESSAGE, null, options,
						 * options[0]); long timeOfResponse = new
						 * Date().getTime(); String emailcopy = email;
						 * if(emailcopy==null)emailcopy="NOTSETONCLIENT"; String
						 * usernamecopy = username;
						 * if(usernamecopy==null)usernamecopy="NOTSETONCLIENT";
						 * String popupID = mp.getID();
						 * MessagePopupResponseFromClient mpRETURN = new
						 * MessagePopupResponseFromClient (popupID,emailcopy,
						 * usernamecopy, question, options, title,
						 * n,timeOfDisplay,timeOfResponse);
                        ...
                        Runnable rr = new Runnable() {
                            public void run() {
                                ...
                                int n = JOptionPane.showOptionDialog(parentComponent, question, title,
                                        JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, 5);
                                long timeOfResponse = new Date().getTime();
                                String popupID = mp.getID();
                                MessagePopupResponseFromClient mpRETURN = new MessagePopupResponseFromClient(popupID,
                                        emailcopy, usernamecopy, question, options, title, n, timeOfDisplay,
                                        timeOfResponse);
                ...
                this.cEventHandler.appendWithCaretCheck("", "NETWORK ERROR", true,
                        "Trying to reconnect....please don't type anything", 0, "SELF");
                e.printStackTrace();
                String possiblyNullusername = "not yet set";
                String possiblyNullemail = "Not yet set";
                if (this.email != null)
                    possiblyNullemail = this.email;
                if (this.username != null)
                    possiblyNullusername = this.username;
                diet.message.MessageErrorFromClient mefc = new MessageErrorFromClient(e.getMessage(), possiblyNullemail,
                        possiblyNullusername);
    ...
    public void run() {
        ...
        while (retryconnection) {
            ...
            try {
                ...
                // EMUI.println("HELLO","ObjectInput and ObjectOutput streams
                // created");
                ...
                int n2 = JOptionPaneTimeOut.showTimeoutDialog(null,
                        "\nThere was some connectivity problem" + "trying to connect to " + serverName
                                ...
                                + " and then try and log in again",
                        "Disconnected", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, options, options[0]);
                if (n2 == 1) {
                    System.exit(-5);
                    // JEMStarter.pullThePlug();
                    return;
        ...
        try {
            ...
            while (this != null) {
                ...
                this.cEventHandler.appendWithCaretCheck("", "NETWORK ERROR(2B)", true,
                        "Trying to reconnect....please don't type anything", 0, "SELF");
    ...
    public void tryToReconnectAutomatically() {
        this.cEventHandler.appendWithCaretCheck("", "NETWORK ERRORTRYINGTORECONNECT", true,
                "Trying to reconnect....please don't type anything", 0, "SELF");
        ...
        while (!hasreconnected) {
            ...
            try {
                ...
                this.cEventHandler.appendWithCaretCheck("", "MANAGED TO RECONNECT", true, "RECONNECTED TO SERVER.STEP1",
                        0, "SELF");
                boolean loginSuccessful = reEstablishloginToServerAndInitializeChattool();
                this.cEventHandler.appendWithCaretCheck("", "MANAGED TO RECONNECT FULLY", true, "PLEASE CONTINUE", 0,
                        "SELF");
                hasreconnected = true;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private boolean reEstablishloginToServerAndInitializeChattool() throws Exception {
        ...
        while (!loggedin) {
            ...
            if (m instanceof MessagePopupClientLogonEmailRequest && this.forceParticipantID) {
                ...
                out.writeObject(new MessageClientLogon(email, "", ServerClientConsistency.getVersionID(), ina, inPort,
                        local_ina, local_port, socketAddress));
            } else if (m instanceof MessagePopupClientLogonEmailRequest) {
                ...
                out.writeObject(new MessageClientLogon(email, username, ServerClientConsistency.getVersionID(), ina,
                        inPort, local_ina, local_port, socketAddress));
    ...
    public void reestablishInitialsetupOfChatTool(Message m) {
        ...
        if (m instanceof MessageClientSetupParametersWithSendButtonAndTextEntryWidthByHeight) {
            ...
            // cEventHandler.setLabelAndTextEntryEnabled(mcspwbyh.getParticipantsTextWindow(),"Normal
            // text",false,true);
        }
    ...
    public synchronized void sendMessage(Message m) {
        ...
        try {
            ...
            try {
                this.cEventHandler.appendWithCaretCheck("", "NETWORK ERROR", true,
                        "Error sending...trying to reset...please don't type anything...the screen might freeze for a bit",
                        0, "SELF");
    ...
    public boolean isCONNECTED() {
        ...
        if (cSocket.isInputShutdown())
            return false;
        if (cSocket.isOutputShutdown())
            return false;
     ...
     * @param keyPresses                                All the keypresses associated with the construction of this
     *                                                  turn
     */
    public synchronized void sendChatText(String text, long startOfTyping,
        ...
        MessageChatTextFromClient msct = new MessageChatTextFromClient(email, username, text, startOfTyping,
                currentTurnBeingConstructedHasBeenBlocked, keyPresses, clientInterfaceEvents);
    ...
    public void sendChatTextWYSIWYG(String textAtEndOfString, int offset, String[] priorChatText) {
        try {
            ...
            MessageWYSIWYGDocumentSyncFromClientInsert mwysfc = new MessageWYSIWYGDocumentSyncFromClientInsert(email,
                    username, textAtEndOfString, offset, 0, "SENDCHATTEXTWYSIWYGDEPRECATED", priorChatText);
    ...
    /**
     * Sends offset and length of text removed from WYSIWYG chat window
     * interface
     *
     * @param offset
     * @param length
     */
    public void sendWYSIWYGDocumentHasChangedRemove(int offset, int length, String textInWindow,
                                                    String[] priorTurnByOther) {
        try {
            MessageWYSIWYGDocumentSyncFromClientRemove mwysfc = new MessageWYSIWYGDocumentSyncFromClientRemove(email,
                    username, offset, length, textInWindow, priorTurnByOther);
            sendMessage(mwysfc);
    ...
    /**
     * Sends offset and length of chat text inserted in WYSIWG chat window
     * interface
     *
     * @param text
     * @param offset
    ...
    public void sendWYSIWYGDocumentHasChangedInsert(String textTyped, int offset, int length, String textInWindow,
        ...
        try {
            MessageWYSIWYGDocumentSyncFromClientInsert mwysfc = new MessageWYSIWYGDocumentSyncFromClientInsert(email,
                    username, textTyped, offset, length, textInWindow, priorTurnByOther);
            sendMessage(mwysfc);
        } catch (Exception e) {
            this.sendErrorMessage(e);
        }
    ...
    public void sendErrorMessage(Exception e) {
        try {
            ...
            diet.message.MessageErrorFromClient mefc = new MessageErrorFromClient(
                    "This is the error message FROM CLIENT: " + e.getMessage() + "\n " + error, e, possiblyNullemail,
                    possiblyNullusername);
            sendMessage(mefc);
        } catch (Exception ee) {
    ...
    public void sendErrorMessage(String s) {
        try {
            String possiblyNullemail = this.email + "";
            String possiblyNullusername = this.username + "";
            diet.message.MessageErrorFromClient mefc = new MessageErrorFromClient("Error message FROM CLIENT: " + s,
                    null, possiblyNullemail, possiblyNullusername);
            sendMessage(mefc);
        } catch (Exception e) {
            e.printStackTrace();
        }
