...
public class JFrameSubliminalStimuli extends JFrame {
	...
	public void displayStimulus(final long fixation1time, final long stimulus1time, final long blankscreen1time,
			final long fixation2time, final long stimulus2time, final long blankscreen2time, String stimulus1ID,
			String stimulus2ID) {
		...
		try {
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					// fixation1time + stimulus1time + blankscreen1time +
					// fixation2time + stimulus2time +blankscreen2time

					// try{Thread.sleep(individualinstructionstime);}catch
					// (Exception e){e.printStackTrace();}
					jpss.displayImage(biFixation1);
					actualFixation1Time = new Date().getTime();

					// this.displayText("HERE1..bifixation1", Color.black,
					// null,100, 100);
					try {
						Thread.sleep(fixation1time);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					// this.displayText("HERE2...bistimulus1", Color.black,
					// null,120, 120);
					try {
						Thread.sleep(stimulus1time);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					// this.displayText("HERE3...bifixation2", Color.black,
					// null,140, 140);
					try {
						Thread.sleep(fixation2time);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					// this.displayText("HERE4....biStimulus2", Color.black,
					// null,160, 160);
					try {
						Thread.sleep(stimulus2time);
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					// cts.sendClientEvent("INDIVIDUALSTIMULUS "+
					// actualFixation1Time+"_"+actualStimulus1Time+"_"+actualFixation2Time+"_"+actualStimulus2Time+"_"+actualQuestionTime);

					cts.cEventHandler.ciet.deprecated_addClientEvent("JFRAMESUBLIMINALSTIMULI", -999999, "actiontype",
							"INDIVIDUALSTIMULUS " + actualFixation1Time + "_" + actualStimulus1Time + "_"
									+ actualFixation2Time + "_" + actualStimulus2Time + "_" + actualQuestionTime);
	...
	private BufferedImage loadLocalFile(String localFileName) {
		try {
			...
			// File fil = new
			// File(s+File.separatorChar+"stimuli"+File.separatorChar+"faces"+File.separatorChar+"stimuli10");
			...
			// SerializableImage si = new
			// SerializableImage(bi,fileName[fileName.length-1]);
			return bi;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void displayText(final String text, final Color textColour, final String nameOfPanel, final int positionX,
			final int positionY, long lengthOfTime) {
		try {
			...
			cts.cEventHandler.ciet.deprecated_addClientEvent("JFRAMESUBLIMINALSTIMULI", -99999, "actiontype",
					"DISPLAYEDBUFFEREDIMAGE " + text + "_" + nameOfPanel + "_" + timeOfDisplayText + "_"
							+ lengthOfTime);
	...
	public void displayTextOLD(String text, Color textColour, String nameOfPanel, int positionX, int positionY,
			long lengthOfTime) {
		try {
			Graphics g = this.jpss.getGraphics();
			if (nameOfPanel != null) {
