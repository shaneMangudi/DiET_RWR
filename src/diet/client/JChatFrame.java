...
public class JChatFrame extends JFrame implements WindowListener {
	...
	public JChatFrame(ClientEventHandler clevh) throws HeadlessException {
		...
		// getRootPane().setBorder(BorderFactory.createMatteBorder(10, 10, 10,
		// 10, Color.LIGHT_GRAY));
	...
	/**
	 * Determines whether the display of cursor position and text highlighting
	 * is permitted in chat window
	 * 
	 * @param windowNumber
	 * @param cursorIsDisplayed
	 * @param selectionIsDisplayed
	 */
	public void wYSIWYGsetCursorAndSelectionAreDisplayedWYSIWYG(int windowNumber, boolean cursorIsDisplayed,
			boolean selectionIsDisplayed) {
		System.err.println("Error...setting WYSIWYG Cursor in superclass");
	}

	/**
	 * Relays whether request for conversational floor has been permitted /
	 * client has lost floor (e.g. through timeout determined by server) in
	 * order to block / allow text entry in the chat window /
	 * 
	 * @param status
	 * @param msg
	 */
	...
	/**
	 * Returns the window number of the window in which the participant's own
	 * text is displayed
	 * 
	...
	public void changeInterfaceProperties(String uniqueIDGeneratedByServer, int newInterfaceproperties, Object value1,
			Object value2) {
