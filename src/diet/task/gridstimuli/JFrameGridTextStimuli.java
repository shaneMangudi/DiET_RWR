...
public class JFrameGridTextStimuli extends JFrame implements ActionListener {
	...
	public JFrameGridTextStimuli(ConnectionToServer cts, long idofset, int rrows, int ccolumns, final String[][] texts,
			Color[][] innerMostc, Color[][] innerc, int width, int height) {
		this.cts = cts;
	...
	public void setTextSelected(JPanel jp, String text, long timeOfPress, Color innerColor, Color innerMostColor, int x,
			int y) {

		int index = -1;

		try {
			MessageGridTextStimuliSelectionFromClient mgss = new MessageGridTextStimuliSelectionFromClient(
					cts.getEmail(), cts.getUsername(), this.serverIDOfSet, index, text, timeOfPress, innerColor,
					innerMostColor);
	...
	public void actionPerformed(ActionEvent e) {
		MessageGridTextStimuliSelectionFromClient mgss = new MessageGridTextStimuliSelectionFromClient(cts.getEmail(),
				cts.getUsername(), serverIDOfSet, -9999, "SELECTBUTTON", e.getWhen(), null, null);
