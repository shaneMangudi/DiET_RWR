...
public class JFrameGridImagesStimuli extends JFrame implements ActionListener {
	...
	public void changeSelection(final long serverIDD, final Vector selections, final Color cInnermost,
			final Color cInner) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				serverIDOfSet = serverIDD;
	...
	public void setImageSelected(JPanel jp, SerializableImage si, long timeOfPress) {
		...
		try {
			MessageGridImageStimuliSelectionFromClient mgss = new MessageGridImageStimuliSelectionFromClient(
					cts.getEmail(), cts.getUsername(), this.serverIDOfSet, index, si.getName(), timeOfPress);
	...
	public void actionPerformed(ActionEvent e) {
		MessageGridImageStimuliSelectionFromClient mgss = new MessageGridImageStimuliSelectionFromClient(cts.getEmail(),
				cts.getUsername(), serverIDOfSet, -1, "Select", e.getWhen());
		cts.sendMessage(mgss);
	}
