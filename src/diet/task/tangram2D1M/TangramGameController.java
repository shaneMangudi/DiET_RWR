...
public class TangramGameController extends DefaultTaskController {
	...
	public TangramGameController(Conversation c, String experimentID) {
		...
		tangramLoader = new TangramLoader(System.getProperty("user.dir") + File.separator + "experimentresources"
				+ File.separator + "tangram_sets");// ,c.getDirectoryNameContainingAllSavedExperimentalData().toString()
		sIOWriting = new SetupIOTangramGameMoveWriting(
				c.getDirectoryNameContainingAllSavedExperimentalData().toString(), curTangramSetName);
	...
	public void participantJoinedConversation(Participant pJoined) {
		...
		// Vector v =
		// (Vector)c.getParticipants().getAllActiveParticipants().clone();
	...
	private boolean loadAndSendTangramsToClients() {
		try {
			...
			tgsUI.initializeJTabbedPane(pMatcher.getUsername(), pDirectorA.getUsername(), pDirectorB.getUsername(),
					curSequence, serverTangramsMatcher, serverTangramsDirector);
			MessageNewTangramsMatcher mntm = new MessageNewTangramsMatcher("server", "server", vRandomOrder,
					curTangramSetName, pMatcher.getUsername());
			MessageNewTangramsDirectorA mnta = new MessageNewTangramsDirectorA("server", "server", curTangrams,
					curTangramSetName, curSequence, pDirectorA.getUsername());
			MessageNewTangramsDirectorB mntb = new MessageNewTangramsDirectorB("server", "server", curTangrams,
					curTangramSetName, curSequence, pDirectorB.getUsername());
	...
	/**
	 * This method is called from Conversation.closeDown() Any threads
	 * instantiated in a custom TaskController object should be stoppable using
	 * this method.
	 */
