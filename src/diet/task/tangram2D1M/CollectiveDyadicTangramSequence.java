...
public class CollectiveDyadicTangramSequence extends TangramSequence {
		...
		for (int i = 0; i < seq.length; i++) {
			if (seq[i] == null) {
				if (i != 0 && i != seq.length - 1) {
					if (seq[i + 1].equals("A") || seq[i - 1].equals("A"))
						seq[i] = "B";
					else if (seq[i + 1].equals("B") || seq[i - 1].equals("B"))
						seq[i] = "A";
					else
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i <= 10; i++) {
			 ...
			 * String[] a={"AB", "AB", "A", "B", "NN"}; String[] b={"C", "MM"};
			 * String[] result=insertAt(1, a , b); for (int
			 * i=0;i<result.length;i++) { System.out.print(result[i]+ " "); }
			 */
	}
