...
public class MatcherTangramPanel extends TangramPanel implements MouseMotionListener, MouseListener {
	...
	public final static int distractorItemCount = 3; // Shouldn't be more than
														// emptySlotCount/2.
														// will fail otherwise
	...
	public MatcherTangramPanel(MatcherTangramFrame containingFrame, Vector<SerialTangram> v, String setName)
			throws IOException {
	...
	public void updateStatusTangramRevealed(int slotID) {
		...
		if (slotID > 0)
			components.elementAt(slotID - 1).enabled = false;
	...
	public void mousePressed(MouseEvent e) {
		...
		if (curDraggedSlotIndex < 0)
			return;
		else if (components.elementAt(curDraggedSlotIndex).isEmpty() || (this.curDraggedSlotIndex < this.emptySlotCount
				&& this.curDraggedSlotIndex >= this.currentActiveDropSlot)) {
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (curDroppedOnSlotIndex < 0) {
			...
			// ClientTangramMessageOutgoing.tangramReleased(curDraggedSlotIndex,
			// curDroppedOnSlotIndex, successful);
			return;
		} else {
			if (!components.elementAt(curDroppedOnSlotIndex).isEmpty()) {
				// JOptionPane.showMessageDialog(this, new String("Slot not
				// empty!"));
			...
			else if (curDroppedOnSlotIndex > this.currentActiveDropSlot && curDroppedOnSlotIndex < emptySlotCount) {
				...
				// System.out.println("tangram moved from: "+
				// curDraggedSlotIndex + "to: "+ curDroppedOnSlotIndex+". Name
				// is:"+droppedOnSlot.tangram.name);
				containingFrame.comms.tangramPlaced(currentActiveDropSlot, curDraggedSlotIndex, curDroppedOnSlotIndex,
						allPreviousTangramsInPlace(currentActiveDropSlot), droppedOnSlot.tangram.name);
				...
				if (allPreviousTangramsInPlace(this.emptySlotCount - 1)) {
					// JOptionPane.showMessageDialog(this, "Well Done! Moving to
					// next set");
	...
	private boolean allPreviousTangramsInPlace(int slotID) {
		...
		for (int i = 0; i <= slotID; i++) {
			if (components.elementAt(i).isEmpty())
				return false;
		}
		return true;
	...
	private void setCurDroppedOnSlotIndex(int x, int y) {
		...
		for (int i = 0; i < components.size(); i++) {
			...
			if ((x > current.x) && (x < current.x + current.clientWidth) && (y > current.y)
					&& (y < current.y + current.clientHeight))
...

	{
		...
		for (int i = 0; i < components.size(); i++) {
			...
			if ((x > cur.x) && (x < (cur.x + Slot.clientWidth - Slot.clientImageBorder)) && (y > cur.y)
					&& (y < cur.y + Slot.clientHeight - Slot.clientImageBorder)) {
