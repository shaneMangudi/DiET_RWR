...
public class TangramGameServerUI extends JFrame {
	...
	protected int[] computeWidthHeightOfFrame() {
		...
		int w;
		w = Slot.serverWidth * MatcherServerPanel.emptySlotCount / 2
				+ MatcherServerPanel.horiDistance * (MatcherServerPanel.emptySlotCount / 2 + 2);
		int h = Slot.serverHeight * (2 + 3) + MatcherServerPanel.vertiDistanceSourceTarget
				+ MatcherServerPanel.vertiDistance * (4 + 2) + 50;
		int[] result = { w, h };
	...
	public void initializeJTabbedPane(final String matcherName, final String directorAName, final String directorBName,
			final TangramSequence curSeq, final Vector<Tangram> matcherTangrams,
			final Vector<Tangram> directorTangrams) {
