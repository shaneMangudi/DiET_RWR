...
public class TangramLoader {
	...
	// precondition is that the server and client directories have the exact
	// same tangram set names
	// and tangram names except that the server images are half the size are
	// halved
	...
	public Vector[] getNextTangramSet() throws IOException {
		...
		for (int i = 0; i < tangramSetIDs.length; i++) {
			if (tangramSetIDs[i].equalsIgnoreCase(".svn"))
				continue;
			...
			if (!curClientDir.isDirectory())
				continue;
			if (previousSetIDs.contains(curClientDir.getName()))
				continue;
			...
			for (int j = 0; j < files.length; j++) {
				...
				if (curName.substring(curName.length() - 4, curName.length()).equalsIgnoreCase(".jpg")) {
					BufferedImage curClientImage = ImageIO
							.read(new File(curClientDir.getAbsolutePath() + File.separator + curName));
					BufferedImage curServerImage = ImageIO
							.read(new File(curServerDir.getAbsolutePath() + File.separator + curName));
					...
					SerialTangram curClientTangram = new SerialTangram(curName.substring(0, curName.length() - 4),
							imageData, w, h);
	...
	// to get the current set name this method should be called before
	// getNextTangramSet is called again
