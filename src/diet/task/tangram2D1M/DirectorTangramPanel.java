...
public class DirectorTangramPanel extends TangramPanel implements MouseMotionListener, MouseListener {
	...
	public DirectorTangramPanel(DirectorTangramFrame containingFrame, TangramSequence seq, char whichDirector,
			Vector<SerialTangram> v, String tangramSetName) throws IOException {
		super(containingFrame, v, tangramSetName);// containingFrame, v,
													// tangramSetName);
	...
	public void updateStatusTangramPlaced(int activeDropSlot, boolean directorMoveOn) {
		if (activeDropSlot == slotCount - 1 && directorMoveOn)
			return;
		if (directorMoveOn) {
			if (curSlotToBeRevealed == activeDropSlot)
				curSlotToBeRevealed++;
		...
		/*
		 * else {
		 * ((DirectorSlot)components.elementAt(curSlotToBeRevealed)).enabled=
		 * false;
		 * 
		 * }
		 */
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (slotClicked >= 0) {
			...
			if (clicked.tangram == null)
				return;
			else if (clicked.enabled) {
				// Show image and send message to enable corresponding matcher
				// slot
				clicked.showImage = true;
				containingFrame.comms.tangramRevealed(slotClicked, clicked.tangram.name);
			} else
				JOptionPane.showMessageDialog(this,
						new String("Matcher is not finished with the previous tangrams yet."));
			...
			// JOptionPane.showMessageDialog(this, new String("You cannot reveal
			// this tangram yet."));
			return;
		}
