...
public class ConfidenceTaskController extends DefaultTaskController {
	...
	public void displayPleaseWait() {
		cC.getConversation().subliminalstimuliset_displayText(pA, "", "pleasewait", Color.yellow, 0, 0, 0,
				cC.getDescriptionForP(pA) + " DISPLAYINGPLEASEWAIT");
		cC.getConversation().subliminalstimuliset_displayText(pB, "", "pleasewait", Color.yellow, 0, 0, 0,
				cC.getDescriptionForP(pB) + " DISPLAYINGPLEASEWAIT");
	}

	public synchronized void nextState_assignNEWSPEAKERS(Participant pA, Participant pB,
	...
	public synchronized void nextState_assignNEWSPEAKERS_STARTINGAGAIN(Participant pA, Participant pB,
			ConfidenceTaskControllerSequence ctcs) {
		this.pA = pA;
		this.pB = pB;
		this.ctcs = ctcs;
	...
	public void saveScoreForParticipant(Participant p) {
		...
		String score = "Total number of individual selections made " + this.getNumberOfIndividualSelectionsMade(p)
				...
				+ " IndividualCorrect: " + this.getIndividualSuccesses(p) + " IndividualFailure: "
				+ this.getIndividualFailures(p) + " Joint success AS SELECTOR: " + this.getJointSuccessesOfSelector(p)
				+ "   joint failures AS SELECTOR " + this.getJointFailuresOfSelector(p) + " Joint success OVERALLL: "
				+ this.getJointSuccessOfBoth(p) + " Joint failure OVERALL: " + this.getJointFailuresOfBoth(p);

		cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(p, cC.getDescriptionForP(p) + "_SCORES",
				p.getParticipantID() + p.getUsername() + "_" + score);
	...
	public ConfidenceTaskController(CCCONFIDENCE cC, ConfidenceTaskControllerSequence ctcs, String identifierOfCTC,
			String experimentalCondition, int numberOfIndividualTrialsPerStage, long stimulusTimeLONG) {
		this.cC = cC;
		this.ctcs = ctcs;
		...
		try {
			...
			// pwOUT = new PrintWriter(new BufferedWriter(new
		...
		// String indstimTEMP = "INDIVIDUALSTIMULUS
		// 1378895914803_1378895917303_1378895918403_1378895920903_1378895922003";
	...
	public String getID() {
		try {
			return "(" + pA.getParticipantID() + "," + pA.getUsername() + ")(" + pB.getParticipantID() + ","
					+ pB.getUsername() + ")";
		} catch (Exception e) {
			return "no ID yet";
		}
	...
	public Object[] getDataFromTrial() {
		...
		Object[] dataFromTrial = {
				...
				"Patch_is_in_first_or_second:", this.state1_PatchIsInFirstOrSecond, // saved
				"Patch_Index_in_hexagram:", this.state1_PatchIndexInHexagram, // saved
				"Patch_Intensity:", this.state1_PatchIntensity, // saved
				// saved

				"PA_ID:", this.pA.getParticipantID(), // saved
				"PA_Username:", this.pA.getUsername(), // saved

				"PA_TimeOnServerOfStartOfTrial:", this.pA__TimeOnServerOfCommandToClientToCarryOutNextTrial, // saved

				"PA_TimeOnServerOfINDIVIDUALSelection:", this.pA__TimeOnServerOfIndividualSelection, // saved

				"PA_TimeOnClientOfFixation1Time:", pA__TimeOnClientFixation1Time, // saved
				"PA_TimeOnClientOfStimulus1Time:", pA__TimeOnClientStimulus1Time, // saved
				"PA_TimeOnClientOfFixation2Time:", pA__TimeOnClientFixation2Time, // saved
				"PA_TimeOnClientOfStimulus2Time:", pA__TimeOnClientStimulus2Time, // saved

				"PA_TimeOnClientOfIndividualQuestion:", this.pA__TimeOnClientOfQuestionTime, // saved
																								// //prompt
																								// on
																								// client
				"PA_TimeOnClientOfIndividualSelection:", this.pA__TimeOnClientOfIndividualSelection, // saved
				"PA_IndividualSelection", this.pA__state1SelectionOfA, // saved

				"PA_TimeOnServerOfJointPrompt", pA__timeOnServerOfJointPrompt, // saved
				"PA_TimeOnServerOfJointSelection", pA__timeOnServerOfJointSelection, // saved

				"PA_TimeOnClientOfJointPrompt", pA__timeOnClientOfJointPrompt, // saved
				"PA_TimeOnClientOfJointSelection", pA__timeOnClientOfJointSelection, // saved
				"PA_JointSelection", pA__JointSelection, // saved
				"PA_TimeOnClientOfSelectionDoubleSlash", pA__TimeOnClientOfSelectionDoubleSlash,
				"PA_TimeOnServerOfSelectionDoubleSlash", pA__TimeOnServerOfSelectionDoubleSlash,

				"PB_ID:", this.pB.getParticipantID(), "PB_Username:", this.pB.getUsername(),

				"PB_TimeOnServerOfStartOfTrial:", this.pB__TimeOnServerOfCommandToClientToCarryOutNextTrial, // saved
				"PB_TimeOnServerOfINDIVIDUALSelection:", this.pB__TimeOnServerOfIndividualSelection, // saved

				"PB_TimeOnClientOfFixation1Time:", pB__TimeOnClientFixation1Time, // saved

				"PB_TimeOnClientOfStimulus1Time:", pB__TimeOnClientStimulus1Time, // saved

				"PB_TimeOnClientOfFixation2Time:", pB__TimeOnClientFixation2Time, // saved

				"PB_TimeOnClientOfStimulus2Time:", pB__TimeOnClientStimulus2Time, // saved

				"PB_TimeOnClientOfIndividualQuestion:", this.pB__TimeOnClientQuestionTime,

				"PB_TimeOnClientOfIndividualSelection:", this.pB__TimeOnClientOfIndividualSelection, // saved

				"PB_IndividualSelection", this.pB__state1SelectionOfB, // saved

				"PB_TimeOnServerOfJointPrompt", pB__timeOnServerOfJointPrompt, // saved

				"PB_TimeOnServerOfJointSelection", pB__timeOnServerOfJointSelection, // saved

				"PB_TimeOnClientOfJointPrompt", pB__timeOnClientOfJointPrompt,

				"PB_TimeOnClientOfJointSelection", pB__timeOnClientOfJointSelection, // saved

				"PB_JointSelection", pB__JointSelection, // saved

				"PB_TimeOnClientOfSelectionDoubleSlash", pB__TimeOnClientOfSelectionDoubleSlash,

				"PB_TimeOnServerOfSelectionDoubleSlash", pB__TimeOnServerOfSelectionDoubleSlash,

				participantWhoChoosesInJointTrialID, participantWhoChoosesInJointTrialUsername,
		...
		// A's reaction time measured as time from question mark appearance on
		// the screen and pressing "enter" after the answer
		// B's individual decision
		// B's reaction time measured as time from question mark appearance on
		// the screen and pressing "enter" after the answer
		// Joint decision (either the actual joint decision if they disagreed or
		// the response they both gave if they agreed)
		// Joint reaction time measured as time from prompt for joint decision
		// and pressing "enter" after the joint answer
	...
	public synchronized void saveState() {
		...
		cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(cC.getDescriptionForP(pA) + "INTERVENTION",
				identifierOfClass + "//" + experimentCondition + "//" + individualTrialNumber + "//" + stateAsString);

	}

	// PB_TimeOnServerOfJointSelection,PB_TimeOnClientOfJointPrompt,PB_TimeOnClientOfJointSelection
	// aren't stored, nor are the pA versions'
	...
	public void updateWithClientEvent_JointStimulusInfo(Participant p, String stimulusInfo) {
		...
		if (stimulusInfo.contains("jointquestionresponder")) {
			...
			if (p != this.participant__state2PersonWhoChooses) {
				cC.getConversation().printWln("ERROR299", p.getUsername() + "...." + stimulusInfo + "..."
						+ participant__state2PersonWhoChooses.getUsername());
				cC.getConversation().saveErrorLog("ERROR299" + p.getUsername() + "...." + stimulusInfo + "..."
						+ participant__state2PersonWhoChooses.getUsername());
				System.err.println("ERROR299" + p.getUsername() + "...." + stimulusInfo + "..."
						+ participant__state2PersonWhoChooses.getUsername());
				return;
			}
			...
			cC.getConversation().printWln("ERROR234", p.getUsername() + "...." + stimulusInfo);
			cC.getConversation().saveErrorLog("ERROR234" + p.getUsername() + "...." + stimulusInfo);
		...
		try {
			...
			if (p == pA) {
				...
				cC.getConversation().printWln("ERROR236", p.getUsername() + "...." + stimulusInfo);
				cC.getConversation().saveErrorLog("ERROR236" + p.getUsername() + "...." + stimulusInfo);
	...
	public String[] updateWithClientEvent_IndividualStimulusInfo(Participant recipientOfStimulus, String stimulusInfo) {
		...
		try {
			...
			if (recipientOfStimulus == this.pA) {
				...
				this.cC.getConversation().saveErrorLog("ERRROR2324324 " + recipientOfStimulus + "..." + stimulusInfo);
				Conversation.printWSln("Main",
						"REALLY SERIOUS ERROR " + 2324324
								+ " processing individualstimulusinfo from the wrong participant "
								+ recipientOfStimulus.getParticipantID() + ".." + recipientOfStimulus.getUsername());
			}

		} catch (Exception e) {
			e.printStackTrace();
		}
		String[] value = { actualFixation1Time, actualStimulus1Time, actualFixation2Time, actualStimulus2Time,
				actualQuestionTime };
		return value;
	}
	...
	public void processChatText(final Participant p, MessageChatTextFromClient mct) {
		...
		cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(p, cC.getDescriptionForP(p) + "TASKMOVE",
				text);
		...
		if (state == state0WAITINGFORSTART) {
			...
			if (p == pA & text.equalsIgnoreCase("/start")) {
				...
				cC.getConversation().deprecated_sendArtificialTurnToRecipient(p, "Thankyou...please wait for other participant",
						0);
			}
			...
			if (this.state0ParticipantAHasSentStart && this.state0ParticipantBHasSentStart) {
				...
				cC.getConversation().changeClientInterface_clearMainWindows(pA);
				cC.getConversation().changeClientInterface_clearMainWindows(pB);
			...
			if (new Date().getTime() < state1ProjectedTimeOfSendingResponse) {
				cC.getConversation().deprecated_sendArtificialTurnToRecipient(p,
						"****YOU SHOULD WAIT TILL YOU HAVE SEEN ALL THE IMAGES!***", 0,
						cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTTYPEDTOOSOON");
				return;
			}
			if (!(text.equalsIgnoreCase("/1") | (text.equalsIgnoreCase("/2")))) {
				cC.getConversation().deprecated_sendArtificialTurnToRecipient(p, "****YOU NEED TO TYPE EITHER  /1 or /2 ***", 0,
						cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTMADETYPE");
				return;
			}
			if (text.equalsIgnoreCase("/1") && p == pA) {
				...
				if (pB__state1SelectionOfB < 1) {
					cC.getConversation().subliminalstimuliset_displayText(pA, "", "individualdecisionmade", Color.BLACK, 20, 30,
							-9999, cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDCORRECT_SENTTOGUI");
			...
			if (text.equalsIgnoreCase("/1") && p == pB) {
				...
				if (pA__state1SelectionOfA < 1) {
					cC.getConversation().subliminalstimuliset_displayText(pB, "", "individualdecisionmade", Color.BLACK, 20, 30,
							-9999, cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDCORRECT_SENTTOGUI");
				}
			...
			// if(state1SelectionOfB<1)cC.getC().subliminalstimuliset_displayText(pA,
			// "","individualdecisionmade",Color.BLACK,
			// 20,30,1000,this.getID()+"INDIVIDUALSAMEANDCORRECT_SENTTOGUI");
			// if(state1SelectionOfA<1)cC.getC().subliminalstimuliset_displayText(pB,
			// "","individualdecisionmade",Color.BLACK,
			// 20,30,1000,this.getID()+"INDIVIDUALSAMEANDCORRECT_SENTTOGUI");

			if (pA__state1SelectionOfA != -1 && pB__state1SelectionOfB != -1) {
				if (pA__state1SelectionOfA == this.state1_PatchIsInFirstOrSecond
						&& pB__state1SelectionOfB == this.state1_PatchIsInFirstOrSecond) {
					cC.getConversation().deprecated_sendArtificialTurnToRecipient(pA, "****CORRECT***", 0,
							cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTMADETYPE_SENTTOCHATFRAME");
					cC.getConversation().deprecated_sendArtificialTurnToRecipient(pB, "****CORRECT***", 0,
							cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTMADETYPE_SENTTOCHATFRAME");
					cC.getConversation().subliminalstimuliset_displayText(pA, "", "individualdecisioncorrect", Color.BLACK, 20, 30,
							this.timeShowingEvaluationOfIndividualtrial,
							cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDCORRECT_SENTTOGUI");
					cC.getConversation().subliminalstimuliset_displayText(pB, "", "individualdecisioncorrect", Color.BLACK, 20, 30,
							this.timeShowingEvaluationOfIndividualtrial,
							cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDCORRECT_SENTTOGUI");
					...
					postTrialCalculations();
						...
						&& pA__state1SelectionOfA != state1_PatchIsInFirstOrSecond) {
					cC.getConversation().deprecated_sendArtificialTurnToRecipient(pA, "****WRONG***", 0,
							cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTMADETYPE_SENTTOCHATFRAME");
					cC.getConversation().deprecated_sendArtificialTurnToRecipient(pB, "****WRONG***", 0,
							cC.getDescriptionForP(p) + "TASKMOVE_PARTICIPANTMADETYPE_SENTTOCHATFRAME");
					cC.getConversation().subliminalstimuliset_displayText(pA, "", "individualdecisionwrong", Color.BLACK, 20, 30,
							this.timeShowingEvaluationOfIndividualtrial,
							cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDWRONG");
					cC.getConversation().subliminalstimuliset_displayText(pB, "", "individualdecisionwrong", Color.BLACK, 20, 30,
							this.timeShowingEvaluationOfIndividualtrial,
							cC.getDescriptionForP(p) + "INDIVIDUALSAMEANDWRONG");
					...
					if (pA__state1SelectionOfA == state1_PatchIsInFirstOrSecond) {
						cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pA,
								cC.getDescriptionForP(p) + "SELECTIONINFO",
								"MADE CORRECT SELECTION: " + pA__state1SelectionOfA);
						cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pB,
								cC.getDescriptionForP(p) + "SELECTIONINFO",
								"MADE WRONG SELECTION: " + pB__state1SelectionOfB);
						Conversation.printWSln("Main", cC.getDescriptionForP(p) + "Individual decisions are DIFFERENT: "
								+ pA.getUsername() + " is correct");
						this.updateIndividualScore(pA, true);
						this.updateIndividualScore(pB, false);
						this.updateIndividualSelectionMade(pA);
						this.updateIndividualSelectionMade(pB);
						...
						cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pA,
								cC.getDescriptionForP(p) + "SELECTIONINFO",
								"MADE WRONG SELECTION: " + pA__state1SelectionOfA);
						cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pB,
								cC.getDescriptionForP(p) + "SELECTIONINFO",
								"MADE CORRECT SELECTION: " + pB__state1SelectionOfB);
						Conversation.printWSln("Main", cC.getDescriptionForP(p) + "Individual decisions are DIFFERENT: "
								+ pB.getUsername() + " is correct");
						this.updateIndividualScore(pA, false);
						this.updateIndividualScore(pB, true);
						this.updateIndividualSelectionMade(pA);
						this.updateIndividualSelectionMade(pB);
					...
					cC.getConversation().changeClientInterface_enableTextDisplay(pA);
					cC.getConversation().changeClientInterface_enableTextDisplay(pB);
					...
					// SAVE INFO OF WHO MADE RIGHT DECISION AND WHO MADE WRONG
					// DECISION
					// Go to next state
					...
					Thread t = new Thread() {
						public void run() {
							cTC.cC.getConversation().subliminalstimuliset_displayText(pA, "", "different", Color.BLACK, 20, 30, 0,
									cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
							cTC.cC.getConversation().subliminalstimuliset_displayText(pB, "", "different", Color.BLACK, 20, 30, 0,
									cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");
							try {
								Thread.sleep(cTC.timeShowingEvaluationOfIndividualtrial);
							} catch (Exception e) {
								e.printStackTrace();
							}
							;
							if (pA == cTC.participant__state2PersonWhoChooses) {
								...
								// cTC.cC.getC().sendArtificialTurnFromApparentOriginToRecipient_NoUsername(pA,pA,
								// "**************WHAT IS YOUR JOINT
								// DECISION?****************",
								// cC.getDescriptionForP(p)+"TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
								// cTC.cC.getC().sendArtificialTurnFromApparentOriginToRecipient_NoUsername(pA,
								// pB, "**************WHAT IS YOUR JOINT
								// DECISION?****************",
								// cC.getDescriptionForP(p)+"TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");

								//

								cTC.cC.getConversation().deprecated_sendArtificialTurnToRecipient(pA,
										"**************WHAT IS YOUR JOINT DECISION?****************", 0,
										cC.styleManager.defaultFONTSETTINGSSELF, cC.getDescriptionForP(p)
												+ "TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
								cTC.cC.getConversation().deprecated_sendArtificialTurnToRecipient(pB,
										"**************WHAT IS YOUR JOINT DECISION?****************", 0,
										cC.styleManager.defaultFONTSETTINGSOTHER, cC.getDescriptionForP(p)
												+ "TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");

								//
								cTC.cC.getConversation().subliminalstimuliset_displayText(pA, "", "jointquestionresponder",
										Color.BLACK, 20, 30, 0,
										cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
								cTC.cC.getConversation().subliminalstimuliset_displayText(pB, "", "jointquestionnonresponder",
										Color.BLACK, 20, 30, 0,
										cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");
								...
								long timeOfJointPrompt = new Date().getTime();
								// cTC.cC.getC().sendArtificialTurnFromApparentOriginToRecipient_NoUsername(pB,pB,
								// "**************WHAT IS YOUR JOINT
								// DECISION?****************",
								// cC.getDescriptionForP(p)+"TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
								// cTC.cC.getC().sendArtificialTurnFromApparentOriginToRecipient_NoUsername(pB,pA,
								// "**************WHAT IS YOUR JOINT
								// DECISION?****************",cC.getDescriptionForP(p)+"TASKMOVE_PARTICIPANTMADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");
								//
								...
								//
								cTC.cC.getConversation().subliminalstimuliset_displayText(pB, "", "jointquestionresponder",
										Color.BLACK, 20, 30, 0,
										cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_MUSTMAKECHOICE");
								cTC.cC.getConversation().subliminalstimuliset_displayText(pA, "", "jointquestionnonresponder",
										Color.BLACK, 20, 30, 0,
										cC.getDescriptionForP(p) + "MADEDIFFERENTSELECTIONS_DOESNTMAKECHOICE");
								pA__timeOnServerOfJointPrompt = timeOfJointPrompt;
								pB__timeOnServerOfJointPrompt = timeOfJointPrompt;
			...
			if (p != this.participant__state2PersonWhoChooses) {
				cC.getConversation().deprecated_sendArtificialTurnToRecipient(p,
						"****You can't choose. Your partner has to choose***", 0,
						cC.getDescriptionForP(p) + "TASKMOVE_JOINTDECISION_WRONGPARTICIPANTTRIEDTOANSWER");
				return;
			} else if (p == this.participant__state2PersonWhoChooses) {
				String id = "";
				if (!(text.equalsIgnoreCase("/1") | (text.equalsIgnoreCase("/2")))) {
					cC.getConversation().deprecated_sendArtificialTurnToRecipient(p, "****YOU NEED TO TYPE EITHER  /1 or /2 ***",
							0, cC.getDescriptionForP(p) + "TASKMOVEJOINTDECISION_PARTICIPANTMADETYPEO");
					return;
				...
				cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(p,
						cC.getDescriptionForP(p) + "JOINTSELECTIONINFO", id);
	...
	public void updateDisplayPostJointDecision(String decision) {
		...
		if (decision.equalsIgnoreCase("/1") && state1_PatchIsInFirstOrSecond == 1)
			jointdecisionIsCorrect = true;
		if (decision.equalsIgnoreCase("/2") && state1_PatchIsInFirstOrSecond == 2)
			jointdecisionIsCorrect = true;

		boolean individualdecisionOfAIsOriginallyCorrect = false;
		if (state1_PatchIsInFirstOrSecond == 1 && pA__state1SelectionOfA == 1)
			individualdecisionOfAIsOriginallyCorrect = true;
		if (state1_PatchIsInFirstOrSecond == 2 && pA__state1SelectionOfA == 2)
			individualdecisionOfAIsOriginallyCorrect = true;

		boolean individualdecisionOfBIsOriginallyCorrect = false;
		if (state1_PatchIsInFirstOrSecond == 1 && pB__state1SelectionOfB == 1)
			individualdecisionOfBIsOriginallyCorrect = true;
		if (state1_PatchIsInFirstOrSecond == 2 && pB__state1SelectionOfB == 2)
			individualdecisionOfBIsOriginallyCorrect = true;
		...
		cC.getConversation().subliminalstimuliset_displayText(pA, "", idForParticipantA, Color.BLACK, 20, 30,
				this.timeShowingEvaluationOfJointtrial, cC.getDescriptionForP(pA) + "TOGUI_JOINTDECISIONCORRECT");
		cC.getConversation().subliminalstimuliset_displayText(pB, "", idForParticipantB, Color.BLACK, 20, 30,
				this.timeShowingEvaluationOfJointtrial, cC.getDescriptionForP(pB) + "TOGUI_JOINTDECISIONCORRECT");
		System.err.println(idForParticipantA);
		System.err.println(idForParticipantB);
		// System.exit(-5);
		// cC.getC().sendArtificialTurnToRecipient(pA, "",
		// 0,this.getID()+"TOCHATFRAME_JOINTDECISIONCORRECT");
		// cC.getC().sendArtificialTurnToRecipient(pB, "****CORRECT!***",
		// 0,this.getID()+"TOCHATFRAME_JOINTDECISIONCORRECT");
	...
	public void postTrialCalculations() {
		...
		// Conversation.printWSln("Main", "INDIVIDUAL TRIAL NUMBER:
		// "+this.individualTrialNumber + " Each stage has
		// "+this.individualTrialsPerStage);
		...
		if (this.individualTrialNumber < this.individualTrialsPerStage) {
			...
			// this.resetToStart();
			this.saveState();
			cC.getConversation().subliminalstimuliset_displayText(pA, "Stage 1 completed! Please take a break.",
					"backgroundorange", Color.BLACK, 20, 30, 0, cC.getDescriptionForP(pA));
			cC.getConversation().subliminalstimuliset_displayText(pB, "Stage 1 completed! Please take a break.",
					"backgroundorange", Color.BLACK, 20, 30, 0, cC.getDescriptionForP(pB));

			JOptionPane.showMessageDialog(null, "Stage 1 Completed. PRESS OK TO START STAGE 2", "Message 1 of 2",
					JOptionPane.PLAIN_MESSAGE);
			JOptionPane.showMessageDialog(null, "PRESS OK AGAIN TO START STAGE 2", "Message 2 of 2",
					JOptionPane.PLAIN_MESSAGE);
	...
	public void disableTextDisplayOfClients() {
		cC.getConversation().changeClientInterface_disableTextDisplay(pA);
		cC.getConversation().changeClientInterface_disableTextDisplay(pB);
	}
	...
	public void gotoSTATE1_SendNewStimulusSet() {
		...
		cC.getConversation().subliminalstimuliset_displaySet(pA, fixationtime, stimulustime, backgroundtime, fixationtime,
				stimulustime, backgroundtime, stimulus1ID, stimulus2ID,
				cC.getDescriptionForP(pA) + "TASKMOVE:STIMULUS (" + stimulus1ID + "," + stimulus2ID + ")");
		cC.getConversation().subliminalstimuliset_displaySet(pB, fixationtime, stimulustime, backgroundtime, fixationtime,
				stimulustime, backgroundtime, stimulus1ID, stimulus2ID,
				cC.getDescriptionForP(pB) + "TASKMOVE:STIMULUS (" + stimulus1ID + "," + stimulus2ID + ")");
	...
	public void initializeImages() {
		try {
			...
			fDIR = new File(s + File.separatorChar + "experimentresources" + File.separatorChar + "confidence"
					+ File.separatorChar + "stimuliset");
	...
	public void participantRejoinedConversation(Participant p) {
		cC.getConversation().subliminalstimuliset_SendSet(p, images, 800, 600, cC.getDescriptionForP(pA));
		if (pA != null)
			cC.getConversation().subliminalstimuliset_displayText(pA, "Resetting because participant logged back in",
					"backgroundwhite", Color.BLACK, 20, 30, 2000, cC.getDescriptionForP(pA));
		if (pB != null)
			cC.getConversation().subliminalstimuliset_displayText(pB, "Resetting because participant logged back in",
					"backgroundwhite", Color.BLACK, 20, 30, 2000, cC.getDescriptionForP(pB));
		cC.getConversation().deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(cC.getDescriptionForP(p),
				"CRASH RECOVERY " + p.getUsername() + "");
		this.gotoSTATE1_SendNewStimulusSet();
	}
	...
	public void participantJoinedConversation(Participant p) {
		...
		cC.getConversation().subliminalstimuliset_SendSet(p, images, 800, 600, cC.getDescriptionForP(p));
		cC.getConversation().subliminalstimuliset_displayText(p, "Please wait for the other participant to log in",
				"backgroundwhite", Color.BLACK, 20, 30, 0, cC.getDescriptionForP(p));
	}
	...
	public void sendStartMessageToParticipants() {
		...
		cC.getConversation().subliminalstimuliset_displayText(pA, "Please type '/start' ", "backgroundwhite", Color.BLACK, 20, 30,
				0, cC.getDescriptionForP(pA));
		cC.getConversation().subliminalstimuliset_displayText(pB, "Please type '/start' ", "backgroundwhite", Color.BLACK, 20, 30,
				0, cC.getDescriptionForP(pB));
	}
