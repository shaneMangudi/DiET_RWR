...
public class MazeGameIsOnSwitchCountdownController extends Thread {
	...
	public MazeGameIsOnSwitchCountdownController(MazeGameController2WAY mgc, Participant pOnSwitch, Participant pOther,
			long timeParticipantMustStayStill) {
	...
	public synchronized void checkIfHasBeenOnSwitchLongEnough() {
		if (!participantIsOnSwitch) {
			try {
				System.err.println(
						"CAN SLEEP WITHOUT TIMING IT - THE PARTICIPANT IS NOT ON THE SWITCH - SHOULD BE ABLE TO SLEEP TILL NOTIFIED");
				...
				if (participantIsOnSwitch)
					System.err.println("WOKEN UP....CORRECT....THE PARTICIPANT IS ON THE SWITCH");
				if (!participantIsOnSwitch)
					System.err.println("WOKEN UP.INCORRECT..THE PARTICIPANT ISN'T ON THE SWITCH BUT SHOULD BE!");
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else {
			...
			if (timeParticipantMustStayStill > 0)
				percentagecomplete = (((float) lengthOnSwitch) / ((float) timeParticipantMustStayStill)) * 100;
			mgc.mgcUI.changeProgressBarOfName(this.pOnSwitch.getUsername(), (int) percentagecomplete);

			// mgc.mgcUI.changeProgressBarOfName(this.pOnSwitch.getUsername(),(int)
			// percentagecomplete);
			System.err.println("Participant is presumably STILL ON SWITCH..has been on switch for: " + lengthOnSwitch
					+ " milliseconds....and the percentagecomplete:" + (percentagecomplete));
