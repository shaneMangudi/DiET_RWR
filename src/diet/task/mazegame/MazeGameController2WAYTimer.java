...
public class MazeGameController2WAYTimer extends Thread {
	...
	public synchronized void waitForNotification() {
		while (doTiming) {
			try {
				...
				if (hasTimedOut) {
					// It might be that in the time interval the participants
					// solved the maze. If this is the case then the method call
					// update below
					// doesn't actually set the timeout (see the method in
					// mgc...
				...
				if (mgcUI.mgc.isCompleted()) {
					...
					System.err.println("TESTING IF COMPLETED: FINISHEDMAZE - HASDETERMINED THAT THE MGC OF: "
							+ mgc.pDirector.getUsername() + " AND " + mgc.pMatcher.getUsername()
				...
				if (mgcUI.mgc.isTimedOut(mgcUI.mgc.mazeNumber)) {
					{
						MazeGameController2WAY mgc = mgcUI.mgc;
						System.err.println(
								"TESTING IF TIMED OUT - HASDETERMINED THAT THE MGC OF: " + mgc.pDirector.getUsername()
										+ " AND " + mgc.pMatcher.getUsername() + "...has timed out");
	...
	public boolean doCalculationsForTimeOut() throws Exception {
		...
		if (pauseTiming) {
			// timeOfLatestChange = timeOfLatestChange+
			// (timeOfEndOfSleep-timeOfStartOfSleep);
	...
	public void doCountdownToNextPartner(Participant a, Participant b, int steps, String message, String cvsPREFIX) {
		for (int k = steps; k > 0; k--) {
			try {
				...
				mgc.c.deprecatedsendArtificialTurnToRecipientWithEnforcedTextColour(a, message + " in " + k + " secs",
						0, mgc.c.getController().getStyleManager().defaultFONTSETTINGSSERVER, cvsPREFIX);
				mgc.c.deprecatedsendArtificialTurnToRecipientWithEnforcedTextColour(b, message + " in " + k + " secs",
						0, mgc.c.getController().getStyleManager().defaultFONTSETTINGSSERVER, cvsPREFIX);
				...
				mgc.c.changeClientInterface_clearMainWindows(a);
				mgc.c.changeClientInterface_clearMainWindows(b);
				mgc.c.deprecatedsendArtificialTurnToRecipientWithEnforcedTextColour(a, "Please start", 0,
						mgc.c.getController().getStyleManager().defaultFONTSETTINGSSERVER, cvsPREFIX);
				mgc.c.deprecatedsendArtificialTurnToRecipientWithEnforcedTextColour(b, "Please start", 0,
						mgc.c.getController().getStyleManager().defaultFONTSETTINGSSERVER, cvsPREFIX);

				Color config_client_backgroundcolour = new Color(
						mgc.c.getController().config.client_backgroundcolour_rgb[0],
						mgc.c.getController().config.client_backgroundcolour_rgb[1],
						mgc.c.getController().config.client_backgroundcolour_rgb[2]);
