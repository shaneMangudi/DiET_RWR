...
import diet.task.mazegame.MazeLink;
import diet.task.mazegame.MazeSquare;
...
public class MazeGameLoadMazesFromJarFile {
	...
	public Vector getMazeFromJar(String name) {
		// InputStream inp =
		// ClassLoader.getSystemClassLoader().getResourceAsStream("featurestoadd.odt");
		// InputStream inp =
		// ClassLoader.getSystemClassLoader().getResourceAsStream(name);
		InputStream inp = ClassLoader.getSystemResourceAsStream("mazegame" + name);
		...
		try {
			...
			for (int i = 0; i < v.size(); i++) {
				...
				if (s.startsWith("MAZE No:")) {
					...
					s = s.replace("MAZE No: ", "");
					s = s.replace("MAZE No:", ""); /// In case the setup file
													/// has a space missing
					id = "" + s;
	...
	public void getSetOf9MazesFromJar() {
		...
		String[] choices = { "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15",
		...
		if (cl1Mazes == null || cl2Mazes == null) {
			...
			CustomDialog
					.showDialog("For some reason the chat tool could not find the mazes in the configuration file.\n"
	...
	public void initializemazes() {
		/*
		 * for(int i=0;i<cl1Mazes.size();i++){ Object o1 =
		 * cl1Mazes.elementAt(i); Object o2 = cl2Mazes.elementAt(i); MazePair mp
		 * = new MazePair(o1,o2); mazePairs.add(mp);
		 * 
		 * } for(int i=0;i<mazePairs.size();i++){ MazePair mp =
		 * (MazePair)mazePairs.elementAt(i); cl1MazesRANDOMIZED.add(mp.cl1Maze);
		 * cl2MazesRANDOMIZED.add(mp.cl2Maze); }
		 */
	}
	...
	public void initializeRandomized() {
		 ...
		 * = new MazePair(o1,o2); mazePairs.add(mp);
		 * 
		 * } Vector mazePairsRandomized = new Vector(); mazePairsRandomized =
		 * VectorToolkit.randomSubset(mazePairs, mazePairs.size()); for(int
		 * i=0;i<mazePairsRandomized.size();i++){ MazePair mp =
		 * (MazePair)mazePairsRandomized.elementAt(i);
		 * cl1MazesRANDOMIZED.add(mp.cl1Maze);
		 * cl2MazesRANDOMIZED.add(mp.cl2Maze); }
		 */
	}
