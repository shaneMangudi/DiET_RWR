...
public class JSetupFrame extends JFrame {
	...
	private void jbInit() throws Exception {
		...
		JLabel jLabel0 = new JLabel(
				" Apologies for the atrocious layout/design/usability of this interface - it's old code from a phd project. Important: Do NOT use during an experiment");
...
class JSetupMazeSquareButtonHandler1 extends JFrame implements java.awt.event.ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "HAS TO BE NUMBERS IN TEXTFIELD", "HAS TO BE NUMBERS",
		...
		if (!currentmaze.squareExists(new Dimension(x1, y1))) {
			JOptionPane.showMessageDialog(this, "First Square DOESNT EXIST", "SQUARE DOESNT EXIST",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		...
		if (e.getSource().equals(jsp.jButton6)) {
			if (!currentmaze.squareExists(new Dimension(x2, y2))) {
				JOptionPane.showMessageDialog(this, "Second Square DOESNT EXIST", "SQUARE DOESNT EXIST",
						JOptionPane.WARNING_MESSAGE);
				return;
			}
...
class JSetupFrameEventHandler extends JFrame implements java.awt.event.ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(jsp.jButton00)) {
			...
			if (mazeindex < 0)
				mazeindex = 0;
			Maze m = new Maze(Integer.parseInt(jsp.jTextField1.getText()), Integer.parseInt(jsp.jTextField2.getText()),
					Double.parseDouble(jsp.jTextField4.getText()), Integer.parseInt(jsp.jTextField5.getText()));
			Maze m2 = m.getClone();
			...
			try {
				System.err.println("THE FOLDER IS:" + fc.getCurrentDirectory().getCanonicalPath());
			} catch (Exception eee) {
				eee.printStackTrace();
			}
