...
public class MazeGameController3WAY extends DefaultTaskController {
	...
	public MazeGameController3WAY(Conversation c) {
		...
		sIO = new SetupIO(
				System.getProperty("user.dir") + File.separator + "mazegame" + File.separator + "mazegamesetup",
	...
	public void loadAndSendMazesToClients() {
		try {
			...
			// Check to see if the experiment is actually re-starting a previous
			// one
			...
			mgcUI.initializeJTabbedPane(pDirector.getUsername(), this.pDirectorMazes, pMatcher1.getUsername(),
					pMatcher1Mazes, pMatcher2.getUsername(), pMatcher2Mazes);
		} catch (Exception e) {
	...
	synchronized public void processTaskMove(MessageTask mt, Participant origin) {
		...
		try {
			if (origin == pDirector) {
				if (mt instanceof MessageCursorUpdate) {
					...
					if (pMatcher1Maze.getMaze().isASwitch(pDirectorMaze.getMaze().getMinBounds(), mcu.newPos)) {
						...
						MessageChangeGateStatus mscg = new MessageChangeGateStatus("server", "server", true,
								pMatcher1.getUsername() + "|" + pMatcher2.getUsername());
				...
				if (mt instanceof MessageCursorUpdate) {
					...
					if (pDirectorMaze.getMaze().isASwitch(pMatcher1Maze.getMaze().getMinBounds(), mcu.newPos)
							&& pDirectorMaze.getMaze().isASwitch(pMatcher2Maze.getMaze().getMinBounds(),
									pMatcher2Maze.getCurrent())) {
						//
				...
				if (mt instanceof MessageCursorUpdate) {
					...
					if (pDirectorMaze.getMaze().isASwitch(pMatcher2Maze.getMaze().getMinBounds(), mcu.newPos)
							&& pDirectorMaze.getMaze().isASwitch(pMatcher1Maze.getMaze().getMinBounds(),
									pMatcher1Maze.getCurrent())) {
			...
			if (pDirectorMaze.getMaze().isCompleted() && pMatcher1Maze.getMaze().isCompleted()
					&& pMatcher2Maze.getMaze().isCompleted()) {
	...
	synchronized public boolean moveToMazeNo(int mazeNo) {
		...
		MessageNextMaze_SentAsIndex mnm = new MessageNextMaze_SentAsIndex("server", "server", mazeNo,
				"Moving to maze " + mazeNumber);
	...
	public boolean moveToNextMaze() {
		...
		pMatcher2Maze = new MazeWrap(
				(Maze) pMatcher2Mazes.elementAt(pMatcher2Mazes.indexOf(pMatcher2Maze.getMaze()) + 1));
		mazeNumber++;
		pDirectorPosition = new Dimension(0, 0);
		pMatcher1Position = new Dimension(0, 0);
		pMatcher2Position = new Dimension(0, 0);
