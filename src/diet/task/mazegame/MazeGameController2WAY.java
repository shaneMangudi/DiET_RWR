...
/**
 *
 * @author user This class should actually be part of diet.task.mazegame.server
 *         However, due to legacy support it has to be in this package as it
 *         uses non-public fields of other classes in this package.
 */
public class MazeGameController2WAY extends DefaultTaskController {
	...
	public void saveDataForMaze() {
		// c.saveDataToConversationHistory(pDirector, pMatcher.getUsername(),
		// new Date().getTime(), "MAZEINFO", "AbsMoveno:$"+participant1MoveNo+);
		...
		try {
			...
			" mazenumber:" + this.mazeNumber + " previousmazenumber: " + prevMazeNumberAsString + " mazestarttime:"
					+ this.startOfCurrentMaze + " mazestoptime:" + timeOfExitFromMaze;
			...
			String dataToSaveDirector = dataToSave + " participanttotalnumberofgamemoves:" + participant1MoveNo
					+ " participanttotalnumberofswitchtraversals:" + participant1NoOfTimesOnSwitch + " partnerid: "
					+ pMatcher.getParticipantID() + " partnerusername: " + pMatcher.getUsername();

			String dataToSaveMatcher = dataToSave + " participanttotalnumberofgamemoves:" + participant2MoveNo
					+ " participanttotalnumberofswitchtraversals:" + participant2NoOfTimesOnSwitch + " partnerid: "
					+ pDirector.getParticipantID() + " partnerusername: " + pDirector.getUsername();

			String subdialogueID = "";
			try {
				subdialogueID = this.c.getController().participantPartnering.getSubdialogueID(pDirector);
			} catch (Exception ee) {
				ee.printStackTrace();
			}

			c.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pDirector, "", new Date().getTime(), subdialogueID,
					dataToSaveDirector);
			c.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(pMatcher, "", new Date().getTime(), subdialogueID,
					dataToSaveMatcher);
	...
	public void saveDataForMazeNEWv4() {
		...
		c.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(c.getController().participantPartnering.getSubdialogueID(pDirector),
				"DATA", pDirector.getParticipantID(), pDirector.getUsername(), this.timeOfJointGoal,
				this.timeOfJointGoal, this.timeOfJointGoal, status + getMazeNo(), new Vector());
		c.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(c.getController().participantPartnering.getSubdialogueID(pMatcher),
				"DATA", pMatcher.getParticipantID(), pMatcher.getUsername(), this.timeOfJointGoal, this.timeOfJointGoal,
				this.timeOfJointGoal, status + getMazeNo(), new Vector());
	...
	public MazeGameController2WAY(Conversation c, File p1Mazes, File p2Mazes) {
		...
		try {
			...
			if (timeParticipantMustStayStill == -1) {
				MazeGameController2WAY.timeParticipantMustStayStill = CustomDialog.getLong(
						"This is a new option for the maze game...\n"
			...
			// sIO = new
			// SetupIO("C:\\svndiet\\chattool\\mazegame",c.getDirectoryNameContainingAllSavedExperimentalData().toString());
			sIO = new SetupIO(
					System.getProperty("user.dir") + File.separator + "experimentresources" + File.separator
							+ File.separator + "mazegame" + File.separator + "mazegamesetup",
					c.getDirectoryNameContainingAllSavedExperimentalData().toString());
			sIOWriting = new SetupIOMazeGameMoveWriting(
					c.getDirectoryNameContainingAllSavedExperimentalData().toString());
	...
	public void startNewMazeGame(Participant a, Participant b) {
		Conversation.printWSln("Main", "Mazegame controller is sending mazes to both clients " + a.getUsername()
				+ " and also to " + b.getUsername());
		pDirector = a;
		pMatcher = b;
		if (mgioswcDirector != null)
			mgioswcDirector.abort();
		if (mgioswcMatcher != null)
			mgioswcMatcher.abort();
		this.mgioswcDirector = new MazeGameIsOnSwitchCountdownController(this, pDirector, pMatcher,
				timeParticipantMustStayStill);
		this.mgioswcMatcher = new MazeGameIsOnSwitchCountdownController(this, pMatcher, pDirector,
				timeParticipantMustStayStill);
	...
	public void sendMazesToClients() {
		...
		mgcUI.initializeJTabbedPane(pDirector.getUsername(), pDirectorMazes, pMatcher.getUsername(),
				this.pMatcher1Mazes);// ,pMatcher2.getUsername(),pMatcher2Mazes);
	...
	public void reconnectParticipant(Participant pReloggingIn) {
		if (pReloggingIn == pDirector) {
			...
			c.newsendInstructionToParticipant(pMatcher,
					"Network error. Your partner has logged back in. Resetting to the start of the maze");

			if (Debug.debugmazegamereconnect)
				c.newsendInstructionToParticipant(pReloggingIn, "(2a) You are the director....and the director ID IS:"
			...
			c.newsendInstructionToParticipant(pDirector,
					"Network error. Your partner has logged back in. Resetting to the start of the maze");
			if (Debug.debugmazegamereconnect)
				c.newsendInstructionToParticipant(pReloggingIn, "(2b) You are the matcher....and the director ID IS:"
						+ pDirector.getParticipantID() + "....." + pDirector.getUsername());
		...
		/*
		 * if(Debug.debugmazegamereconnect) { try{ System.err.println("5");
		 * Thread.sleep(1000); System.err.println("4"); Thread.sleep(1000);
		 * System.err.println("3"); Thread.sleep(1000); System.err.println("2");
		 * Thread.sleep(1000); System.err.println("1"); Thread.sleep(1000);
		 * System.err.println("0"); Thread.sleep(1000);
		 * 
	...
	synchronized public boolean isAonOthersSwitch(String a) {
		if (this.pDirector.getUsername().equalsIgnoreCase(a)) {
			...
			Conversation.printWSln("Main", "Error in working out whether A is on Other's switch because " + a
					+ " is not (1) " + pDirector.getUsername() + " or(2)" + pMatcher.getUsername());
			return false;
		}
	}
	...
	synchronized public boolean isOtherSOnAswitch(String a) {
		try {
			if (this.pDirector.getUsername().equalsIgnoreCase(a)) {
				...
				Conversation.printWSln("Main", "Error in working out whether Other is On A's switch because " + a
						+ " is not (1) " + pDirector.getUsername() + " or(2)" + pMatcher.getUsername());
	...
	public synchronized boolean processMazeMove(MessageTask mt, Participant origin, boolean automaticallyGoTONextMaze) {
			...
			if (mcu.getMazeNo() != this.getMazeNo()) {
				Conversation.printWSln("Main",
						"Possible network lag and/or error. Experiment is receiving maze cursor updates from "
								...
								+ "but the current maze is: " + this.getMazeNo());
		...
		try {
			if (origin == pDirector) {
				if (mt instanceof MessageCursorUpdate) {
					...
					if (pMatcher1Maze.getMaze().isASwitch(pDirectorMaze.getMaze().getMinBounds(), mcu.newPos)) {
						...
						MessageChangeGateStatus mscg = new MessageChangeGateStatus("server", "server", false,
								pMatcher.getUsername());
				...
				if (mt instanceof MessageCursorUpdate) {
					...
					if (pDirectorMaze.getMaze().isASwitch(pMatcher1Maze.getMaze().getMinBounds(), mcu.newPos)) {
						...
						/*
						 * System.out.println(
						 * "Server detects client2 and client3 as being on client1's switch"
						 * ); MessageChangeGateStatus mscg = new
						 * MessageChangeGateStatus("server","server",true,
						 * pDirector.getUsername());
			...
			if (pDirectorMaze.getMaze().isCompleted() && pMatcher1Maze.getMaze().isCompleted()) {
				if (timeOfJointGoal < 0)
					this.timeOfJointGoal = new Date().getTime();
				...
				c.deprecated_saveAdditionalRowOfDataToSpreadsheetOfTurns(subdialogueID,
						"MAZECOMPLETE:" + "(" + this.mazeNumber + ")" + " (" + pDirector.getParticipantID() + ","
								+ pDirector.getUsername() + " (" + pMatcher.getParticipantID() + ", "
								+ pMatcher.getUsername() + timeOfJointGoal);

			}

			/// if(automaticallyGoTONextMaze&&
			/// pDirectorMaze.getMaze().isCompleted()&&pMatcher1Maze.getMaze().isCompleted()){
	...
	synchronized public boolean moveToMazeNo(int mazeNo, String text) {
		text = text + "!";
		if (mgioswcDirector != null)
			mgioswcDirector.updateIsOffSwitch();
		if (mgioswcMatcher != null)
			mgioswcMatcher.updateIsOffSwitch();
		if (mazeNo < 0)
			mazeNo = 0;
		if (mazeNo >= pDirectorMazes.size())
			mazeNo = pDirectorMazes.size() - 1;
		//
		resetCounters();
		if (mazeNumber >= pDirectorMazes.size() - 1 || mazeNumber >= pMatcher1Mazes.size() - 1) {
			...
			Conversation.printWSln("Main",
					"Experimenter attempted to go to next or previous maze that doesn't exist...so doing nothing");
			return false;
		}
		resetCounters();
		...
		// MessageNextMaze_SentAsIndex mnm= new
		// MessageNextMaze_SentAsIndex("server","server",mazeNo,"Moving to maze
		// "+mazeNo);
	...
	public boolean moveToNextMaze(String displayMessage) {
		...
		pDirectorMaze = new MazeWrap(
				(Maze) pDirectorMazes.elementAt(pDirectorMazes.indexOf(pDirectorMaze.getMaze()) + 1));
		pMatcher1Maze = new MazeWrap(
				(Maze) pMatcher1Mazes.elementAt(pMatcher1Mazes.indexOf(pMatcher1Maze.getMaze()) + 1));
		prevMazeNumber = mazeNumber;
		mazeNumber++;
		pDirectorPosition = new Dimension(0, 0);
		pMatcher1Position = new Dimension(0, 0);
		System.out.println("move to next maze EXPERIMENT IS TRUE");
		MessageNextMaze_SentAsIndex mnm = new MessageNextMaze_SentAsIndex("server", "server", mazeNumber,
				displayMessage);
		c.sendTaskMoveToParticipant(pDirector, mnm);
		c.sendTaskMoveToParticipant(pMatcher, mnm);
