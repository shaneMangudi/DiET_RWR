...
public class Maze implements Serializable {
			...
			if (sqr.x == d1.width && sqr.y == d1.height) {
				for (int j = 0; j < squares.size(); j++) {
					...
					if (sqr2.x == d2.width && sqr2.y == d2.height) {
						// are matched
						if (sqr.north != null && sqr.north.getLinkedSquare(sqr).equals(sqr2))
							sqr.north.isagate = !sqr.north.isagate;
						if (sqr.east != null && sqr.east.getLinkedSquare(sqr).equals(sqr2))
							sqr.east.isagate = !sqr.east.isagate;
						if (sqr.south != null && sqr.south.getLinkedSquare(sqr).equals(sqr2))
							sqr.south.isagate = !sqr.south.isagate;
						if (sqr.west != null && sqr.west.getLinkedSquare(sqr).equals(sqr2))
							sqr.west.isagate = !sqr.west.isagate;
						return;
					}
	...
	public Vector getLinks() {
		...
		for (int i = 0; i < squares.size(); i++) {
			...
			if (ms.north != null && !links.contains(ms.north))
				links.addElement(ms.north);
			if (ms.east != null && !links.contains(ms.east))
				links.addElement(ms.east);
			if (ms.south != null && !links.contains(ms.south))
				links.addElement(ms.south);
			if (ms.west != null && !links.contains(ms.west))
				links.addElement(ms.west);
		}
		return links;
	}
	...
	public Maze getClone() {
		...
		try {
			...
			System.out.println("cloning doesn't seem to work " + e.getMessage());
		}
		return m;
	}
	...
	public void constructRandomly(int x, int y, double duplicateprob) {
		...
		// GOT TO CHECK THAT WHEN YOU GROW, EVEN WHEN THE LINK IS NULL , THE NEW
		// MAZESQUARE MIGHT ALREADY BE THERE ONLY NOT CONNECTED
			...
			for (int i = 0; i < slength; i++) {
				...
				if (s == 0 && temp.north == null && this.getBounds().height <= y) {
					...
					if (mz == null) {
						...
						System.out.println("ADDING NORTH : FROM " + temp.x + " " + temp.y + " to " + temp1.x + " "
								+ temp1.y + " ");
					} else {
					...
					if (mz == null) {
						...
						squares.addElement(temp1);
						System.out.println(
								"ADDING east : FROM " + temp.x + " " + temp.y + " to " + temp1.x + " " + temp1.y + " ");
					} else {
						double isOK = r.nextDouble();
						if (isOK < duplicateprob) {
							MazeLink mzl = new MazeLink(mz, temp, false);
					...
					if (mz == null) {
						...
						squares.addElement(temp1);
						System.out.println("ADDING south : FROM " + temp.x + " " + temp.y + " to " + temp1.x + " "
								+ temp1.y + " ");
					} else {
						double isOK = r.nextDouble();
						if (isOK < duplicateprob) {
					...
					if (mz == null) {
						...
						squares.addElement(temp1);
						System.out.println(
								"ADDING west : FROM " + temp.x + " " + temp.y + " to " + temp1.x + " " + temp1.y + " ");
					} else {
						double isOK = r.nextDouble();
						if (isOK < duplicateprob) {
							MazeLink mzl = new MazeLink(mz, temp, false);
	...
	public boolean isASwitch(Dimension mazeBoundsOfOther, Dimension position) {
		...
		System.out.println("CONVERTED " + position.width + " ," + position.height + " to " + updatedPosition.width
	...
	public boolean isASwitch(Dimension sq) {
		...
		for (int i = 0; i < squares.size(); i++) {
			MazeSquare sqr = (MazeSquare) squares.elementAt(i);
			System.out.println("looking for switch status of " + sq.width + " " + sq.height + "at " + " MAZESQUARE"
					+ sqr.x + " " + sqr.y);
		...
		// System.exit( -1);
		System.out.println(
				"REALLY SHOULDN'T GET HERE . SHOULDNT . BUT MIGHT IF THE RESET MAZE THING DOESNT WORK PROPERLY");
		return false;
	}
