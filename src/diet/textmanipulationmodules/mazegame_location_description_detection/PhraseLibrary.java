...
public class PhraseLibrary {
	...
	Random r = new Random();
	String prefixPath = System.getProperty("user.dir") + File.separator + "experimentresources" + File.separator
			+ "categ";

	public PhraseLibrary() {
		prefixPath = System.getProperty("user.dir") + File.separator + "experimentresources" + File.separator + "categ"
				+ File.separator;
		loadRawPhrases();
		// jumblePhrases_Figural =makeCopyOfVector(rawPhrases_Figural);
		// jumblePhrases_Abstract = makeCopyOfVector(rawPhrases_Abstract);
		// jumblePhrases_Abstract_Coordinates =
		// makeCopyOfVector(rawPhrases_Abstract_Coordinates);
	...
	public void jumbleAllVectors() {
		// System.out.println("STARTING JUMBLING WITH ABSTRACT: SIZE IS
		// "+jumblePhrases_Abstract.size());
		...
		// System.out.println("EXITED JUMBLING WITH ABSTRACT: SIZE IS
		// "+jumblePhrases_Abstract.size());
	}
		...
		try {
			...
			while (readon) {
				...
				if (r == null) {
					...
					if (!vectorContains(rawPhrases_Figural, r) & !r.equalsIgnoreCase("right") & !r.endsWith(".")
							& !r.endsWith(",") & !r.endsWith(",") & !r.endsWith("?") & !r.endsWith("!")) {
	...
	public Vector decomposeAbstractFrags(String s) {
		...
		while (stz.hasMoreTokens()) {
			...
			if (token.indexOf("1st") >= 0 | token.indexOf("2nd") >= 0 | token.indexOf("3rd") >= 0
					| token.indexOf("4th") >= 0 | token.indexOf("5th") >= 0 | token.indexOf("6th") >= 0
					| token.indexOf("7th") >= 0 | token.indexOf("8th") >= 0 | token.indexOf("9th") >= 0
					| token.indexOf("10th") >= 0 | token.indexOf("1 st") >= 0 | token.indexOf("2 nd") >= 0
					| token.indexOf("3 rd") >= 0 | token.indexOf("4 th") >= 0 | token.indexOf("5 th") >= 0
					| token.indexOf("6 th") >= 0 | token.indexOf("7 th") >= 0 | token.indexOf("8 th") >= 0
	...
	public Vector decomposeAbstractFrags_Coord(String s) {
		...
		for (int i = 0; i < decomposedCommaSpace.size(); i++) {
			...
			StringTokenizer stztok = new StringTokenizer(stok,
