...
public class Phrase {
	...
	public void addPhrase(Vector v) {
		...
		if (v.size() == 1) {
			...
			// if it is, do nothing
			// if it isn't, create new phrase with v.elementAt(i) and add it to
			// tails in alphabetical order
			...
			// check to see if first element of v is contained in tails
			// if it is,
			// call addPhrase method on it with the rest of the vector as
			// argument.
			// if it isn't
			// create new Phrase with v.elementAt(i)
			// add it to tails in alphabetical order
			// call the addPhrase method on it with the rest of the vector as
			// argument
