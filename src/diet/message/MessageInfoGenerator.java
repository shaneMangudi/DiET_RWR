...
public class MessageInfoGenerator {
	...
	static public Vector[] getAllMethods(Message m) {
		...
		for (int i = 0; i < methods.length; i++) {
			...
			try {
				if (f.getGenericParameterTypes().length == 0 && !f.getName().endsWith("getAllParameters")
						&& (f.getName().startsWith("get") || f.getName().startsWith("is"))) {
					// System.out.println("TRYING WITH
					// "+m.getClass().toString()+": "+f.getName());
				...
				System.err.println(
						"ERROR ATTEMPTING TO REFLECT MESSAGE WITH : " + m.getClass().toString() + ": " + f.getName());
