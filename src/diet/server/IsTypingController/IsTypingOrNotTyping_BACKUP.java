...
public class IsTypingOrNotTyping_BACKUP extends Thread {
	...
	// VectorHashtableEnforcingUniqueness htPairsWhoAreInformedOfIsTyping = new
	// VectorHashtableEnforcingUniqueness();
	...
	private synchronized void informISDOING(Participant p) {
		...
		Vector v = cC.participantPartnering.getRecipients(p);
		for (int i = 0; i < v.size(); i++) {
			Participant pRecipient = (Participant) v.elementAt(i);
			cC.getConversation().changeClientInterface_changeBorderOnChatFrame(pRecipient, this.isDoingPixels, this.isDoingColour);
	...
	private synchronized void informISNOTDOING(Participant p) {
		...
		for (int i = 0; i < v.size(); i++) {
			Participant pRecipient = (Participant) v.elementAt(i);
			cC.getConversation().changeClientInterface_changeBorderOnChatFrame(pRecipient, this.isNotTypingPixels,
					this.isNotTypingColour);
