...
public class FakeTypingActivity extends Thread {
	...
	public synchronized void addIsTypingInfo(Participant p, long timeOfEvent) {
		...
		// System.err.println("INSERTING AT: "+indexToInsert+"....size
		// is"+fakeactivity.size());
			...
			try {
				if (this.fakeactivity.size() == 0)
					...
					if (durationTillNext > 0) {
						...
						if (durationTillNext < 100)
							durationTillNext = 100;
						// wait(5000);
						wait(durationTillNext + 1);
						long timeOfExiting = new Date().getTime();
						System.err.println("SLEEPING TIME IS:::" + (timeOfExiting - timeOfCalling)
								+ " but should have been::" + durationTillNext);
	...
	public synchronized void removeFakeTypingAfterThreshold(Participant p, long threshold) {
		...
		for (int i = 0; i < this.fakeactivity.size(); i++) {
			...
			if (ft.apparentOrigin == p && ft.timeOfTyping > threshold)
				vToRemove.addElement(ft);
		}
