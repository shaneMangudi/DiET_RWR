...
public class IsMovingOrNotMoving extends Thread {
	...
	private synchronized void informISDOING(Participant p) {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			// 6, Color.black);
			cC.getConversation().changeClientInterface_changeBorderOnMazeFrame(pRecipient, this.isDoingPixels, this.isDoingColour);
	...
	private synchronized void informISNOTDOING(Participant p) {
		...
		for (int i = 0; i < v.size(); i++) {
			...
			// cC.getC().changeClientInterface_changeBorderOnChatFrame(pRecipient,
			// 6, Color.white);
			cC.getConversation().changeClientInterface_changeBorderOnMazeFrame(pRecipient, this.isNotTypingPixels,
