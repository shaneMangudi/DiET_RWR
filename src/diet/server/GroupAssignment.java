...
/**
 *
 * Ensure that when participants connect to the server, all the participants are
 * stored in this file Ensure also that all the conversations that participants
 * are assigned to are also recorded here
 *
 * @author Greg
 */
public class GroupAssignment {
	...
	public synchronized boolean assignParticipantToConversation(Vector conversations, String participantID,
			String userName, ParticipantConnection participConnection) {
		// default behaviour: go through all conversations, checking if they are
		// active and checking to see if
		// they are "filled".
		// System.exit(-5);
		for (int i = 0; i < conversations.size(); i++) {
			...
			if (c.isConversationActive()) {
				...
				if (c.requestPermissionForNewParticipantToBeAddedToConversation(participantID))
					canAssign = true;
