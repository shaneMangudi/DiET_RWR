...
public class IntelligentSpreadsheetAndHeaderWriter extends Thread {
	...
	public void appendToHeaderFile(String s) {
		try {
			...
			FileWriter fooWriter = new FileWriter(fHEADER, true); // true to
																	// append
			fooWriter.write(s);
			fooWriter.close();
		} catch (Exception e) {
	...
	private void reestablishFile() {
		...
		while (!wasSuccessfulReestablishing) {
			...
			try {
				this.textOut = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fSPREADSHEET),
						Charset.forName("UTF-8").newEncoder()));
				if (fSPREADSHEET.canWrite())
					wasSuccessfulReestablishing = true;
			} catch (Exception e) {
	...
	public String processAttribVals(Vector v) {
		...
		for (int i = 0; i < this.vAttributeIDs.size(); i++) {
			...
			for (int j = 0; j < v.size(); j++) {
				...
				if (av.getID().equalsIgnoreCase(s)) {
					...
					row = row + av.getValAsString() + DefaultConversationController.config.recordeddata_CSVSeparator;
					break;
				}
			}
			if (!found) {
				row = row + DefaultConversationController.config.recordeddata_CSVSeparator;
			}
		}
		...
		for (int i = 0; i < vAttribValsClone.size(); i++) {
			...
			this.appendToHeaderFile(av.id + DefaultConversationController.config.recordeddata_CSVSeparator);
		}
