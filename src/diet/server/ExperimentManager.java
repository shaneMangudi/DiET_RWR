...
public class ExperimentManager extends Thread {
	...
	public ExperimentManager(EMUI em, int portNo) {
		...
		// ConnectionListenerWebSocket clws = new
		// ConnectionListenerWebSocket(this,200001);
	...
	public synchronized boolean activateParticipant(ParticipantConnection participConnection, String participantID,
			String username) {
		// Called by participantconnection during verification of login details
		// And then adds itself to the conversation
		boolean succ = this.ga.assignParticipantToConversation(allConversations, participantID, username,
				participConnection);
		if (!succ)
			return false;
	...
	public synchronized void startParticipantandJoinConversationDEPRECATED(ParticipantConnection participC,
	...
	public synchronized Object findPrevioususernameOrParticipantFromPreviousLoginsAndCheckParticipantIDISOK(
	...
	// Need to return a Conversation object so that the calling class can start
	...
	public void connectUIWithExperimentManager(Conversation c, ConversationUIManager convUI) {
		if (emui != null) {
	...
	public int getMinParticipantIDLength() {
		return DefaultConversationController.config.login_minimumlength_of_participantID;
	}
