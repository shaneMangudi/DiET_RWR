...
import diet.task.ProceduralComms.ProceduralCommsTaskController;
...
public class Grid2016 extends DefaultConversationController {
	...
	public void participantJoinedConversation(Participant p) {
		...
		if (conversation.getParticipants().getAllParticipants().size() == 2) {
			Participant a = (Participant) conversation.getParticipants().getAllParticipants().elementAt(0);
			Participant b = (Participant) conversation.getParticipants().getAllParticipants().elementAt(1);
			pctc.startGame(a, b);
		}
	...
	public void processTaskMove(MessageTask mt, Participant p) {
		super.processTaskMove(mt, p); // To change body of generated methods,
										// choose Tools | Templates.
		pctc.processTaskMove(mt, p);

		// c.gridTextStimuli_ChangeSelection(p, new Date().getTime(),
		// innerMostSelection, innerSelection);
	...
	public void processClientEvent(Participant origin, MessageClientInterfaceEvent mce) {
		super.processClientEvent(origin, mce); // To change body of generated
												// methods, choose Tools |
												// Templates.
		// System.err.println("PROCESSING CLIENT EVENT");
		// System.err.println("THE TYPE OF EVENT IS:
		// "+mce.getClientInterfaceEvent().getType());
