...
/**
 * This is the representation of individual turns in the conversation. It stores
 * the origin, the recipients, the keypresses, the individual words used and
 * their associated patterns of usage, the timing data and the parse tree (if
 * stored).
 * 
 * @author user
 */
public class Turn implements Serializable {
	...
	public String getIOAdditionalValues() {
		...
		if (this.additionalValues == null) {
			return DefaultConversationController.config.recordeddata_CSVSeparator;
		...
		try {
			for (int i = 0; i < this.additionalValues.size(); i++) {
				returnval = returnval + DefaultConversationController.config.recordeddata_CSVSeparator
						+ ((AttribVal) additionalValues.elementAt(i)).getVal();
			}
		} catch (Exception e) {
			e.printStackTrace();
			return DefaultConversationController.config.recordeddata_CSVSeparator + "error";
		}

		return returnval + DefaultConversationController.config.recordeddata_CSVSeparator;
	}
