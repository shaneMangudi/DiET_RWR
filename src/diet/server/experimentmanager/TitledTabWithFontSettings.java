 ...
 * 
 * <p>
 * This is a customized version of Infonode's implementation of TitledTab which
 * permits the Font of the TitledTab to be changed.
 * 
 * <p>
 * A TitledTab is a tab that has support for text, icon and a custom Swing
 * component (called title component). Titled tab supports several properties
 * that makes it possible to change the look (borders, colors, insets), layout
 * (up, down, left, right).
 * </p>
 *
 * <p>
 * Titled tab has a line based layout, i.e. the text, icon and title component
 * are laid out in a line. The layout of the tab can be rotated, i.e. the text
 * and the icon will be rotated 90, 180 or 270 degrees. The title component will
 * not be rotated but moved so that the line layout will persist.
 * </p>
 *
 * <p>
 ...
 * </ul>
 * Most of the properties for the tab can be configured for each of the tab
 * rendering states.
 * </p>
 *
 * <p>
 * <strong>Note:</strong> If only the normal state properties have been
 * configured, the highlighted and disabled state will automatically use the
 * same properties as for the normal state, see {@link TitledTabProperties} and
 * {@link TitledTabStateProperties}.
 * </p>
 *
 * <p>
 * TitledTab implements the {@link net.infonode.gui.icon.IconProvider} interface
 * and overloads toString() so that both text and icon for the normal state is
 * shown in the tab drop down list in a tabbed panel.
 * </p>
 *
 * <p>
 * TitledTab supports mouse hovering. A {@link HoverListener} can be set in the
 * {@link TitledTabProperties}. The hover listener receives a {@link HoverEvent}
 * when the mouse enters or exits the tab. The hover event's source will be the
 * affected titled tab.
 * </p>
 *
...
public class TitledTabWithFontSettings extends Tab implements IconProvider {
	...
	private class StatePanel extends SimplePanel {
		...
		private void updateLayout(TitledTabStateProperties stateProperties, boolean titleComponentChanged) {
			if (titleComponent != null && stateProperties.getTitleComponentVisible()) {
				...
				if (titleComponentPanel.getComponentCount() == 0
						|| (titleComponentPanel.getComponentCount() > 0
								&& titleComponentPanel.getComponent(0) != titleComponent)
						|| titleComponentChanged || gap != currentLayoutGap || alignment != currentLayoutAlignment
					...
					if (d == Direction.UP) {
						panel.add(titleComponentPanel,
								alignment == Alignment.LEFT ? BorderLayout.SOUTH : BorderLayout.NORTH);
						titleComponentPanel.setBorder(new EmptyBorder(alignment == Alignment.LEFT ? gap : 0, 0,
								alignment == Alignment.LEFT ? 0 : gap, 0));
					} else if (d == Direction.LEFT) {
						panel.add(titleComponentPanel,
								alignment == Alignment.LEFT ? BorderLayout.EAST : BorderLayout.WEST);
						titleComponentPanel.setBorder(new EmptyBorder(0, alignment == Alignment.LEFT ? gap : 0, 0,
								alignment == Alignment.LEFT ? 0 : gap));
					} else if (d == Direction.DOWN) {
						panel.add(titleComponentPanel,
								alignment == Alignment.LEFT ? BorderLayout.NORTH : BorderLayout.SOUTH);
						titleComponentPanel.setBorder(new EmptyBorder(alignment == Alignment.LEFT ? 0 : gap, 0,
								alignment == Alignment.LEFT ? gap : 0, 0));
					} else {
						panel.add(titleComponentPanel,
								alignment == Alignment.LEFT ? BorderLayout.WEST : BorderLayout.EAST);
						titleComponentPanel.setBorder(new EmptyBorder(0, alignment == Alignment.LEFT ? 0 : gap, 0,
								alignment == Alignment.LEFT ? gap : 0));
					}
					try {
						panel.revalidate();
					} catch (Exception e) {
		...
		public void updateShapedPanel(TitledTabStateProperties stateProperties) {
			...
			panel.setHorizontalFlip(tabAreaOrientation == Direction.DOWN || tabAreaOrientation == Direction.LEFT
		...
		public boolean updateState(Map changes, TitledTabStateProperties stateProperties) {
			...
			if (changes == null) {
				...
				label.setHorizontalAlignment(alignment == Alignment.LEFT ? JLabel.LEFT
						: alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.RIGHT);

				alignment = stateProperties.getVerticalAlignment();
				label.setVerticalAlignment(alignment == Alignment.TOP ? JLabel.TOP
						: alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.BOTTOM);
				...
				if (m != null) {
					...
					if (keySet.contains(TitledTabStateProperties.TEXT)
							|| keySet.contains(TitledTabStateProperties.TEXT_VISIBLE)) {
						label.setText(stateProperties.getTextVisible() ? stateProperties.getText() : null);
					}

					if (keySet.contains(TitledTabStateProperties.ICON)
							|| keySet.contains(TitledTabStateProperties.ICON_VISIBLE)) {
						icon = stateProperties.getIcon();
						label.setIcon(stateProperties.getIconVisible() ? stateProperties.getIcon() : null);
					}
					...
					if (keySet.contains(TitledTabStateProperties.ICON_TEXT_GAP)) {
						label.setIconTextGap(
								((Integer) ((ValueChange) m.get(TitledTabStateProperties.ICON_TEXT_GAP)).getNewValue())
										.intValue());
					}

					if (keySet.contains(TitledTabStateProperties.ICON_TEXT_RELATIVE_ALIGNMENT)) {
						Alignment alignment = (Alignment) ((ValueChange) m
								.get(TitledTabStateProperties.ICON_TEXT_RELATIVE_ALIGNMENT)).getNewValue();
						label.setHorizontalTextPosition(alignment == Alignment.LEFT ? JLabel.RIGHT : JLabel.LEFT);
					}

					if (keySet.contains(TitledTabStateProperties.HORIZONTAL_ALIGNMENT)) {
						Alignment alignment = (Alignment) ((ValueChange) m
								.get(TitledTabStateProperties.HORIZONTAL_ALIGNMENT)).getNewValue();
						label.setHorizontalAlignment(alignment == Alignment.LEFT ? JLabel.LEFT
								: alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.RIGHT);
					}
					...
					if (keySet.contains(TitledTabStateProperties.VERTICAL_ALIGNMENT)) {
						Alignment alignment = (Alignment) ((ValueChange) m
								.get(TitledTabStateProperties.VERTICAL_ALIGNMENT)).getNewValue();
						label.setVerticalAlignment(alignment == Alignment.TOP ? JLabel.TOP
								: alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.BOTTOM);
					}

					if (keySet.contains(TitledTabStateProperties.TOOL_TIP_TEXT)
							|| keySet.contains(TitledTabStateProperties.TOOL_TIP_ENABLED)) {
						toolTipText = stateProperties.getToolTipEnabled() ? stateProperties.getToolTipText() : null;
						if (toolTipText != null && toolTipText.length() == 0)
							toolTipText = null;
					...
					if (keySet.contains(TitledTabStateProperties.DIRECTION)
							...
							|| keySet.contains(TitledTabStateProperties.TEXT_TITLE_COMPONENT_GAP)
							|| keySet.contains(TitledTabStateProperties.ICON_VISIBLE)
							|| keySet.contains(TitledTabStateProperties.TEXT_VISIBLE)) {
						label.setDirection(stateProperties.getDirection());

						updateLayout(stateProperties,
								keySet.contains(TitledTabStateProperties.TITLE_COMPONENT_VISIBLE));
					}
	...
	/**
	 * Constructs a TitledTab with a text, icon, content component and title
	 * component.
	 *
	 * @param text
	 ...
	 *            content component or null for no content component
	...
	public TitledTabWithFontSettings(String text, Icon icon, JComponent contentComponent, JComponent titleComponent,
		...
		eventPanel.addMouseListener(new MouseAdapter() {
			...
			private void updateFocus(TabSelectTrigger trigger) {
				if (isEnabled() && properties.getFocusable() && getTabbedPanel() != null
						&& getTabbedPanel().getProperties().getTabSelectTrigger() == trigger) {
		...
		addTabListener(new TabAdapter() {
			...
			public void tabRemoved(TabRemovedEvent event) {
				PropertyMapWeakListenerManager.removeWeakPropertyChangeListener(
						event.getTabbedPanel().getProperties().getMap(), TabbedPanelProperties.TAB_AREA_ORIENTATION,
	 ...
	 *
	 * <p>
	 * This method is a convenience method for setting the same title component
	 * for all states.
	 * </p>
	 *
	 * @param titleComponent
	 ...
	 *
	 * <p>
	 * <strong>Note:</strong> This will only have effect if this TitledTab is
	 * enabled and a member of a tabbed panel.
	 * </p>
	 *
	 * @param highlighted
	 ...
	 * 
	 * <p>
	 * <strong>Note:</strong> since ITP 1.5.0 this method will change the
	 * enabled property in the {@link TitledTabProperties} for this tab.
	 * Enabled/disabled can be controlled by modifying the property or this
	 * method.
	 * </p>
	 *
	 * @param enabled
	 ...
	 *
	 * @return the Shape for the active rendering state, null if no special
	 *         shape
	 * @since ITP 1.2.0
	 */
	public Shape getShape() {
	...
	private void updateBorders() {
		...
		Insets maxInsets = properties.getBorderSizePolicy() == TitledTabBorderSizePolicy.INDIVIDUAL_SIZE ? null
		...
		Border innerNormalBorder = getInnerBorder(properties.getNormalProperties(), tabAreaOrientation, -normalLowered,
				maxInsets);
		Border innerHighlightBorder = getInnerBorder(properties.getHighlightedProperties(), tabAreaOrientation,
				raised - normalLowered, maxInsets);
		Border innerDisabledBorder = getInnerBorder(properties.getDisabledProperties(), tabAreaOrientation,
				-normalLowered, maxInsets);
	...
	private void doUpdateTab(Map changes) {
		...
		if (changes == null) {
			...
			if (m != null) {
				...
				if (keySet.contains(TitledTabProperties.HOVER_LISTENER)) {
					updateHoverListener(
							(HoverListener) ((ValueChange) m.get(TitledTabProperties.HOVER_LISTENER)).getNewValue());
				}

				if (keySet.contains(TitledTabProperties.SIZE_POLICY)) {
					layout.setUseSelectedComponentSize(
							((TitledTabSizePolicy) ((ValueChange) m.get(TitledTabProperties.SIZE_POLICY))
									.getNewValue()) == TitledTabSizePolicy.INDIVIDUAL_SIZE);
				}

				if (keySet.contains(TitledTabProperties.HIGHLIGHTED_RAISED_AMOUNT)
						|| keySet.contains(TitledTabProperties.BORDER_SIZE_POLICY)) {
					updateBorders = true;
				}
		...
		updateBorders = highlightedStatePanel.updateState(changes, properties.getHighlightedProperties())
				|| updateBorders;
	...
	private Border getInnerBorder(TitledTabStateProperties properties, Direction tabOrientation, int raised,
			Insets maxInsets) {
		...
		if (maxInsets != null)
			insets = InsetsUtil.add(insets,
					InsetsUtil.sub(maxInsets, getBorderInsets(properties.getComponentProperties().getBorder())));

		Border border = properties.getComponentProperties().getBorder();
		Border innerBorder = new EmptyBorder(InsetsUtil.add(insets,
				InsetsUtil.setInset(InsetsUtil.EMPTY_INSETS, tabOrientation.getOpposite(), raised)));
		return border == null ? innerBorder : new CompoundBorder(border, innerBorder);
	}

	private Direction getTabAreaOrientation() {
		return getTabbedPanel() == null ? lastTabAreaOrientation
				: getTabbedPanel().getProperties().getTabAreaOrientation();
	}
	...
	private MouseEvent convertMouseEvent(MouseEvent e) {
		...
		return new MouseEvent(TitledTabWithFontSettings.this, e.getID(), e.getWhen(), e.getModifiers(), (int) p.getX(),
				(int) p.getY(), e.getClickCount(), !e.isConsumed() && e.isPopupTrigger(), e.getButton());
	}
