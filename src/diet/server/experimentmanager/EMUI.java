...
public class EMUI {
	...
	public EMUI(final int portNumber) {
		...
		try {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					jmf = new JExperimentManagerMainFrame(thisEMUI,
							System.getProperty("user.dir") + File.separator + "data",
							"DiET: Server. Port no. " + "" + portNumber);
		...
		// int n = (Integer)expSett.getV("Number of participants per
		// conversation");
	...
	public void runExperiment(final String nameOfConversationController) {
		Thread r = new Thread() {
			public void run() {
				//// expSett.changeParameterValue("Experiment Data
				//// Folder",System.getProperty("user.dir")+"\\data\\Saved
				//// experimental data");
				//// expSett.changeParameterValue("Experiment Data
				//// Folder",System.getProperty("user.dir")+File.separator+"data"+File.separator+"Saved
				//// experimental data");
	...
	public void runExperimentLocally(final String nameOfConversationController) {
		Thread r = new Thread() {
			public void run() {
				...
				int n = (Integer) c.getController().config.login_numberOfParticipants;
	...
	public void displayConversationUI(Conversation c, ConversationUIManager convUI) {
		...
		this.jmf.addConvIOtoLeftTabbedPane(c.getController().config.param_experimentID, jtp);
		if (c == null)
			System.exit(-12341234);
		if (jtp == null)
			System.exit(-12341235);
