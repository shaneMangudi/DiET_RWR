...
public class JLeftTabbedPanel extends TabbedPanel {
	...
	// TabbedPanelTitledTabTheme[] themes = new TabbedPanelTitledTabTheme[]{new
	// DefaultTheme(), new LookAndFeelTheme(), new ClassicTheme(), new
	// BlueHighlightTheme(), new SmallFlatTheme(), new GradientTheme(),
	// new GradientTheme(true, true), new ShapedGradientTheme(), new
	// ShapedGradientTheme(
	// 0f,
	// 0f,
	...
	public JLeftTabbedPanel(EMUI em) {
		super();
		try {
			theme = new ShapedGradientTheme(0f, 0f, new FixedColorProvider(new Color(150, 150, 150)), null) {
				public String getName() {
	...
	public void displayPanelAddingIfNecessaryDeprecated(ConversationUIManager cUI) {
		TitledTabWithFontSettings tab = new TitledTabWithFontSettings(cUI.getConvID(), null, new JPanel(), null,
				fontSize);
	...
	public void displayPanelAddingIfNecessary(JComponent jc) {
		TitledTabWithFontSettings tab = new TitledTabWithFontSettings("Conversation", null, new JPanel(), null,
				fontSize);
		tab.getProperties().addSuperObject(titledTabProperties);
		super.addTab(tab);
	}
