 ...
 * @author arash
 * 
 *         This is an abstract class which should be sub-classed detailing the
 *         getExtraColumns and getExtraColumnNames methods. This class will
 *         generally collate all the turn.txt files found in the given
 *         experiment directories. The 2 abstract methods should be overridden
 *         in order to provide experiment-specific post-analyses of each turn,
 *         returning a pipe-separated string. The result of calling
 *         analyseAndCollate() will then produce a concatenation of all the
 *         turns.txt files, plus these additional columns, coded for each turn.
 *         . . .
 * 
 *
 */
public abstract class TurnsCollaterPostAnalysis {
	...
	public void analyseAndCollate() throws IOException {
		BufferedWriter out = new BufferedWriter(new FileWriter(this.outputDir.getAbsolutePath() + File.separator
				+ "data_output_of_" + this.getClass().getName() + "3.csv"));
		...
		for (File expDir : dirList) {
			...
			if (expDir.isDirectory()) {
				...
				File turnsFile = new File(
						this.expDataDir + File.separator + expDir.getName() + File.separator + "turns.txt");
	...
	/**
	 * These should return pipe-separated values. The string returned should
	 * start with the pipe(|). They should return the empty string if the class
	 * is only to collate the turns.txt files, without adding extra columns.
	 * NOTE: the previous turns passed to the method will not include the exp
	 * id. So all indeces-1!
	 * 
	 * @param previousTurns
