 ...
 *
 * <p>
 * <a id="JsonReaderExample1"/> The following example demonstrates how to read
 * an empty JSON array from a string:
 * 
 * <pre>
 * <code>
 ...
 * multiple reader instances with the same configuration. This the preferred way
 * to create multiple instances. A sample usage is shown in the following
 * example:
 * 
 * <pre>
 * <code>
...
public interface JsonReader extends /* Auto */Closeable {
	 ...
	 *             if a JSON object or array cannot be created due to i/o error
	 *             (IOException would be cause of JsonException)
	 * @throws javax.json.stream.JsonParsingException
	 *             if a JSON object or array cannot be created due to incorrect
	 *             representation
	 * @throws IllegalStateException
	 *             if read, readObject, readArray or close method is already
	 *             called
	 */
	JsonStructure read();

	/**
	 * Returns a JSON object that is represented in the input source. This
	 * method needs to be called only once for a reader instance.
	 *
	 ...
	 *             (IOException would be cause of JsonException)
	 * @throws javax.json.stream.JsonParsingException
	 *             if a JSON object cannot be created due to incorrect
	 *             representation
	 * @throws IllegalStateException
	 *             if read, readObject, readArray or close method is already
	 *             called
	...
	/**
	 * Returns a JSON array that is represented in the input source. This method
	 * needs to be called only once for a reader instance.
	 *
	 * @return a JSON array
	 * @throws JsonException
	 *             if a JSON array cannot be created due to i/o error
	 *             (IOException would be cause of JsonException)
	 * @throws javax.json.stream.JsonParsingException
	 *             if a JSON array cannot be created due to incorrect
	 *             representation
	 * @throws IllegalStateException
	 *             if read, readObject, readArray or close method is already
	 *             called
