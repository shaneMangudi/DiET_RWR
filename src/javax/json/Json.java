...
/**
 * Factory class for creating JSON processing objects. This class provides the
 ...
 * {@link JsonProvider#provider()}. This class uses the provider instance to
 * create JSON processing objects.
 *
 * <p>
 * The following example shows how to create a JSON parser to parse an empty
 * array:
 * 
 * <pre>
 * <code>
...
public class Json {
	...
	/**
	 * Creates a JSON parser from a byte stream. The character encoding of the
	 * stream is determined as specified in
	 * <a href="http://tools.ietf.org/rfc/rfc4627.txt">RFC 4627</a>.
	 *
	 * @param in
	 *            i/o stream from which JSON is to be read
	 ...
	 *
	 *         public static JsonParserFactory createParserFactory() { return
	 *         JsonProvider.provider().createParserFactory(); }
	 */

	/**
	 * Creates a parser factory for creating {@link JsonParser} objects. The
	 * factory is configured with the specified map of provider specific
	 ...
	 *
	 * @param config
	 *            a map of provider specific properties to configure the JSON
	...
	/**
	 * Creates a JSON reader from a byte stream. The character encoding of the
	...
	/**
	 * Creates a reader factory for creating {@link JsonReader} objects. The
	 * factory is configured with the specified map of provider specific
	 * configuration properties. Provider implementations should ignore any
	 * unsupported configuration properties specified in the map.
	 *
	...
	/**
	 * Creates a writer factory for creating {@link JsonWriter} objects. The
	 * factory is configured with the specified map of provider specific
	 * configuration properties. Provider implementations should ignore any
	 * unsupported configuration properties specified in the map.
	 *
	...
	/**
	 * Creates a builder factory for creating {@link JsonArrayBuilder} and
	 * {@link JsonObjectBuilder} objects. The factory is configured with the
	 ...
	 * @param config
	 *            a map of provider specific properties to configure the JSON
	 *            builders. The map may be empty or null
