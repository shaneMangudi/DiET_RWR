 ...
 * configure the created writer instances.
 *
 * <p>
 * {@link javax.json.JsonWriter} can also be created using {@link Json}'s
 * {@code createWriter} methods. If multiple writer instances are created, then
 * creating them using a writer factory is preferred.
 *
 * <p>
 * <b>For example:</b>
 * 
...
public interface JsonWriterFactory {
	 ...
	 * {@link JsonArray array} structure to the specified character stream. The
	 * writer is configured with the factory configuration.
	 *
	 * @param writer
	 ...
	 * properties that are used to configure the created JSON writer objects. If
	 * there are any specified configuration properties that are not supported
	 * by the provider, they won't be part of the returned map.
	 *
	 * @return a map of supported provider specific properties that are used to
	 *         configure the created writers. The map may be empty but not null.
	 */
