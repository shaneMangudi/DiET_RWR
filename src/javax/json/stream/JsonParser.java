...
/**
 * Provides forward, read-only access to JSON data in a streaming way. This is
 * the most efficient way for reading JSON data. The class
 * {@link javax.json.Json} contains methods to create parsers from input sources
 * ({@link java.io.InputStream} and {@link java.io.Reader}).
 *
 * <p>
 * The following example demonstrates how to create a parser from a string that
 * contains an empty JSON array:
 * 
 * <pre>
 * <code>
 ...
 * {@code JsonParser} instances. {@link JsonParserFactory} is preferred when
 * creating multiple parser instances. A sample usage is shown in the following
 * example:
 * 
 * <pre>
 * <code>
 ...
 * 
 * <p>
 * {@code JsonParser} parses JSON using the pull parsing programming model. In
 * this model the client code controls the thread and calls the method
 * {@code next()} to advance the parser to the next state after processing each
 * element. The parser can generate the following events: {@code START_OBJECT},
 * {@code END_OBJECT}, {@code START_ARRAY}, {@code END_ARRAY}, {@code KEY_NAME},
 * {@code VALUE_STRING}, {@code VALUE_NUMBER}, {@code VALUE_TRUE},
 * {@code VALUE_FALSE}, and {@code VALUE_NULL}.
 *
 * <p>
 * <b>For example</b>, for an empty JSON object ({ }), the parser generates the
 * event {@code START_OBJECT} with the first call to the method {@code next()}
 * and the event {@code END_OBJECT} with the second call to the method
 * {@code next()}. The following code demonstrates how to access these events:
 *
 * <pre>
 * <code>
 ...
 *
 * <p>
 * calls to the method {@code next()} result in parse events at the specified
 ...
 *
 * <p>
 * The methods {@code next()} and {@code hasNext()} enable iteration over parser
 * events to process JSON data. {@code JsonParser} provides get methods to
 * obtain the value at the current state of the parser. For example, the
...
public interface JsonParser extends /* Auto */Closeable {
	...
	enum Event {
		 ...
		 * methods to access the number value: {@link #getInt}, {@link #getLong}
	...
	/**
	 * Returns {@code true} if there are more parsing states. This method
	 * returns {@code false} if the parser reaches the end of the JSON text.
	 *
	 * @return {@code true} if there are more parsing states.
	 * @throws javax.json.JsonException
	 *             if an i/o error occurs (IOException would be cause of
	 *             JsonException)
	 * @throws JsonParsingException
	 *             if the parser encounters invalid JSON when advancing to next
	 *             state.
	 */
	boolean hasNext();
	...
	/**
	 * Returns a {@code String} for the name in a name/value pair, for a string
	 * value or a number value. This method should only be called when the
	 * parser state is {@link Event#KEY_NAME}, {@link Event#VALUE_STRING}, or
	 * {@link Event#VALUE_NUMBER}.
	 *
	 * @return a name when the parser state is {@link Event#KEY_NAME} a string
	 *         value when the parser state is {@link Event#VALUE_STRING} a
	 *         number value when the parser state is {@link Event#VALUE_NUMBER}
	 * @throws IllegalStateException
	 *             when the parser state is not {@code KEY_NAME},
	 *             {@code VALUE_STRING}, or {@code VALUE_NUMBER}
	 */
	String getString();

	/**
	 * Returns true if the JSON number at the current parser state is a integral
	 * number. A {@link BigDecimal} may be used to store the value internally
	 * and this method semantics are defined using its {@code scale()}. If the
	 ...
	 * @throws IllegalStateException
	 *             when the parser state is not {@code VALUE_NUMBER}
	 */
	boolean isIntegralNumber();

	/**
	 * Returns a JSON number as an integer. The returned value is equal to
	 ...
	 * method should only be called when the parser state is
	 * {@link Event#VALUE_NUMBER}.
	 *
	...
	/**
	 * Returns a JSON number as a long. The returned value is equal to
	 ...
	 * This method is only called when the parser state is
	 * {@link Event#VALUE_NUMBER}.
	 *
	...
	/**
	 * Returns a JSON number as a {@code BigDecimal}. The {@code BigDecimal} is
	 * created using {@code new BigDecimal(getString())}. This method should
	 * only called when the parser state is {@link Event#VALUE_NUMBER}.
	 *
	...
	/**
	 * Return the location that corresponds to the parser's current state in the
	 * JSON input source. The location information is only valid in the current
	 * parser state (or until the parser is advanced to a next state).
	 *
	 * @return a non-null location corresponding to the current parser state in
	 *         JSON input source
	 */
	 ...
	 *
	 * getJsonValue(JsonArray.class) is valid in the START_ARRAY state and moves
	 * the cursor to END_ARRAY.
	 *
