...
/**
 * Factory for creating {@link JsonParser} instances. If a factory instance is
 * configured with a configuration, the configuration applies to all parser
...
public interface JsonParserFactory {
	 ...
	 *
	 * @param reader
	 *            a i/o reader from which JSON is to be read
	 */
	JsonParser createParser(Reader reader);

	/**
	 * Creates a JSON parser from the specified byte stream. The character
	 * encoding of the stream is determined as specified in
	 ...
	 * stream are decoded to characters using the specified charset.
	 *
	 * @param in
	 *            i/o stream from which JSON is to be read
	 * @param charset
	 ...
	 * properties that are used to configure the JSON parsers. If there are any
	 * specified configuration properties that are not supported by the
	 * provider, they won't be part of the returned map.
	 *
	 * @return a map of supported provider specific properties that are used to
	 *         configure the created parsers. The map may be empty but not null
	 */
